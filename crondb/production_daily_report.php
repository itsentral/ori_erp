...
// WIP REPORT
$sqlHeader      = "SELECT a.*, b.id_milik FROM history_pro_header_cron_wip a LEFT JOIN production_detail b ON a.id_production_detail = b.id  WHERE DATE(a.status_date)='".$date."' ";
$Q_Awal			= $koneksi->query($sqlHeader);
...
if(!empty($restHeader)){
    $sqlDel1 = "DELETE FROM laporan_wip_per_bulan WHERE `date`='".$date."' ";
    $sqlDel2 = "DELETE FROM laporan_wip_per_hari WHERE `date`='".$date."' ";
    $koneksi->query($sqlDel1);
    $koneksi->query($sqlDel2);
