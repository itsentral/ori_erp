...
class Report_in_customer extends CI_Controller {
    ...
    public function get_data_json_spk_produksi_progress2(){
		...
		foreach($query->result_array() as $row)
		{
			...
			if(!empty($row['id_material'])){
				$nilai_unit = $row['qty_mat'] * $row['cost_book'];
			}

			$nestedData[]	= "<div align='right'>".number_format($nilai_unit,2)."</div>";
	...
	public function excel_report_subgudang2(){
	 ...
	 //GROUP
    public function get_data_json_data_erp_incustomer(){
		$controller			= ucfirst(strtolower($this->uri->segment(1)));
		$Arr_Akses			= getAcccesmenu($controller);
		$requestData	= $_REQUEST;
		$fetch			= $this->query_data_erp_incustomer(
			$requestData['tgl_awal'],
			$requestData['tgl_akhir'],
			$requestData['search']['value'],
			$requestData['order'][0]['column'],
	...
	public function costbook(){
		...
		$this->load->view('Report_new/Report_in_customer/costbook',$data);
	}
    ...
    public function get_data_json_fg_costbook(){
		...
		$Arr_Akses			= getAcccesmenu($controller);
		$requestData	= $_REQUEST;
		$fetch			= $this->query_data_incustomer_costbook(
			$requestData['tgl_awal'],
			$requestData['tgl_akhir'],
			$requestData['search']['value'],
			$requestData['order'][0]['column'],
		...
		foreach($query->result_array() as $row)
		{
			...
			if(!empty($idtrans)){
			$out = $this->db->query("SELECT COUNT(a.qty_ke) as qty_out FROM data_erp_in_customer a WHERE jenis='out' AND id_trans=$idtrans GROUP BY id_trans")->row();
			}
			if(!empty($out)){
			$qty_out = 	$out->qty_out;
			}else{
	...
	public function query_data_incustomer_costbook($tgl_awal,$tgl_akhir,$like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		$WHERE_DATE = "AND a.tanggal LIKE '".date('Y')."-".date('m')."%' ";
		if($tgl_awal != '0'){
			$WHERE_DATE = "AND (DATE( a.tanggal ) BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."' )";
		}
