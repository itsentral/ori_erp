...
class Approval_mutasi extends CI_Controller {
	...
	public function index()
    {
		...
		$so = $this->request_mutasi_model->get_data_pn_approve();
		$data = array(
			'title'			=> 'Approval Request Mutasi',
			'action'		=> 'index',
		...
		$this->load->view('Request_mutasi/approval',$data);
	...
	public function add_mutasi()
    {
		...
		$this->load->view('Request_mutasi/approval_mutasi',$data);
	...
	public function approve_mutasi(){
		...
		$Qry_Update	 = "UPDATE tr_request_mutasi SET approved_by= '$user', approved_on='$tanggal',status_approve='1' WHERE kd_mutasi='$kd_mutasi'";
	    ...
	    if($this->db->trans_status() === FALSE){
		   ...
		   history('Approve Request Mutasi '.$kd_mutasi.' with username : '.$data_session['ORI_User']['username']);
		}
	...
	public function save_jurnal_jv($kd_mutasi){
		...
		$Qry_Update	 = "UPDATE tr_request_mutasi_aktual SET jurnal1='$Nomor_JV' WHERE kd_mutasi='$kd_bayar'";
            $this->db->query($Qry_Update);
	...
	public function reject()
    {
		...
		$this->load->view('Request_mutasi/reject',$data);
	...
	public function save_reject(){
		...
		$Qry_Update	 = "UPDATE tr_request_mutasi SET status_approve='2', alasan='$alasan' WHERE kd_mutasi='$kd_mutasi'";
        $this->db->query($Qry_Update);
	    ...
	    if($this->db->trans_status() === FALSE){
		   ...
		   history('Reject Request Mutasi '.$kd_mutasi.' with username : '.$data_session['ORI_User']['username']);
		}
		echo json_encode($Arr_Return);
