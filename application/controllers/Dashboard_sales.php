...
class Dashboard_sales extends CI_Controller {	
	...
	public function index() {
		...
		$ttl_inv = $this->db->query("SELECT sum(total_invoice) ttl_inv from ( SELECT sum(total_invoice_idr) total_invoice FROM tr_invoice_header where year(tgl_invoice)='".date("Y")."'
		union
		select sum(total_invoice_idr) from tr_invoice_np_header where status=1 and year(tgl_invoice)='".date("Y")."'
		) ttl_inv")->row(); 
