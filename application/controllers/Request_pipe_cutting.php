...
class Request_pipe_cutting extends CI_Controller {
    ...
    public function index(){
		...
		$data_ipp = $this->db->group_by('id_bq')->get_where('so_cutting_header',array('app'=>'Y','sts_confirm'=>'N'))->result_array();
        
		$data = array(
			'title'			=> 'Confirm SPK Cutting',
			'action'		=> 'index',
		...
		$this->load->view('Request_pipe_cutting/index',$data);
	}
    ...
    public function server_side_spk_cutting(){
		...
		foreach($query->result_array() as $row)
		{
            ...
            $sts_confirm = ($row['sts_confirm'] == 'Y')?"<span class='badge bg-green'>Confirm</span>":"<span class='badge bg-blue'>Waiting</span>";
            ...
            $view = "";
				...
				if($row['app'] == 'Y' AND $row['sts_confirm'] == 'N'){
					$cutting = "&nbsp;<a href='".base_url('request_pipe_cutting/confirm/'.$row['id'])."' class='btn btn-sm btn-success' title='Confirm'><i class='fa fa-check'></i></a>";
				...
				if($row['cutting'] == 'Y'){
					$view = "<a href='".base_url('request_pipe_cutting/detail/'.$row['id'])."' class='btn btn-sm btn-default' title='View'><i class='fa fa-eye'></i></a>";
				}
	...
	public function query_spk_cutting($no_ipp, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		...
		$sql = "
			...
			FROM
				...
				(SELECT @row:=0) r
		    WHERE  1=1 ".$where." ".$where2." AND a.cutting = 'Y' AND a.app = 'Y' AND(
    ...
    public function detail($id=null){
		...
		$this->load->view('Request_pipe_cutting/detail',$data);
	}
    ...
    public function confirm($id=null){
        if($this->input->post()){
			...
			if ($this->db->trans_status() === FALSE){
				...
				history('Confirm spk cutting '.$id);
			}
            ...
            $data = array(
                'title'			=> 'Closing Cutting Pipe',
                'action'		=> 'index',
                'detail'		=> $detail,
                'row_group'		=> $data_Group,
                'akses_menu'	=> $Arr_Akses
            );
            $this->load->view('Request_pipe_cutting/confirm',$data);
        }
	}
