...
class Project_history extends CI_Controller {
    ...
    public function index(){
		...
		$data = array(
			'title'			=> 'Warehouse Project >> Detail Transaction',
			'action'		=> 'index',
		...
		$this->load->view('Project/transaction/index',$data);
	}
	...
	public function query_data_json_detil_transaction($material,$tgl_awal, $tgl_akhir,$like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		...
		$sql = "
			...
			FROM
				...
				(SELECT @row:=0) r
		    WHERE 1=1 ".$where_gudang." ".$where_daterange." AND (a.id_gudang = '".$id_gudang_project."' OR a.gudang LIKE 'C100%') AND (
	...
	public function download_excel_stok(){
		...
		$SQL = "SELECT a.*, b.material_name AS material_name_new FROM warehouse_rutin_history a LEFT JOIN con_nonmat_new b ON a.code_group = b.code_group AND b.deleted_date IS NULL WHERE 1=1 ".$where_gudang." ".$where_daterange."AND (a.id_gudang = '".$id_gudang_project."' OR a.gudang LIKE 'C100%') ORDER BY a.id DESC";
	...
	public function show_history(){
		...
		$data_html = $this->load->view('Project/transaction/show_history', $dataArr, TRUE);
