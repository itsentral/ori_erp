			...
			$nestedData[]	= "<div align='left'><span class='badge' style='background-color:".$class."'>".$status."</span></div>";
				...
				if($row['type'] == 'progress'){
					if($row['status'] == '10' OR $row['status'] == '11'){
						$create_progress = "<a href='".base_url('penagihan/create_progress_new'.$delivery.'/'.$row['id'])."' class='btn btn-sm btn-success'><i class='fa fa-pencil' title='Edit'></i></a>";
						if($row['no_faktur']!="") $approve="<a href='".base_url('penagihan/create_progress_new'.$delivery.'/'.$row['id'])."/approve' class='btn btn-sm btn-warning' title='Approve'><i class='fa fa-check-square-o'></i></a>";
					}
				}
