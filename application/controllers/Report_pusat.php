...
class Report_pusat extends CI_Controller {
    ...
    public function index(){
		...
		$data = array(
			'title'			=> 'Report Gudang Pusat',
			'action'		=> 'index',
			'row_group'		=> $data_Group,
			'akses_menu'	=> $Arr_Akses
		);

		$this->load->view('Report_new/Report_pusat/index',$data);
	}
    ...
    public function excel_report_subgudang(){
		...
		$sheet->setCellValue('A'.$Row, 'REPORT GUDANG PUSAT');
		...
		$sheet->setCellValue('E'.$NewRow, 'ID MATERIAL');
		$sheet->getStyle('E'.$NewRow.':E'.$NextRow)->applyFromArray($tableHeader);
		$sheet->mergeCells('E'.$NewRow.':E'.$NextRow);
		$sheet->getColumnDimension('E')->setWidth(10);

		$sheet->setCellValue('F'.$NewRow, 'NM MATERIAL');
        ...
        $sheet->setCellValue('H'.$NewRow, 'COST BOOK');
		...
		$sheet->setTitle('Report Gudang Pusat');
		...
		header('Content-Disposition: attachment;filename="report-gudang-pusat.xls"');
		//unduh file
	...
	public function excel_report_subgudang2(){
		...
		$sheet->setCellValue('A'.$Row, 'REPORT GUDANG PUSAT GROUP');
		$sheet->getStyle('A'.$Row.':'.$Col_Akhir.$NewRow)->applyFromArray($mainTitle);
		$sheet->mergeCells('A'.$Row.':'.$Col_Akhir.$NewRow);
		...
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		//ubah nama file saat diunduh
		header('Content-Disposition: attachment;filename="report-gudang-pusat-group.xls"');
		//unduh file
		$objWriter->save("php://output");
	}
