...
class Gudang_wg extends CI_Controller {
    ...
    public function index(){
		...
		$data = array(
			'title'			=> 'Gudang WIP '.ucfirst($this->uri->segment(3)),
			'action'		=> 'index',
			'row_group'		=> $data_Group,
			'tanda'			=> $tanda,
			// 'akses_menu'	=> $Arr_Akses
		);
		history('View data gudang wip');
		$this->load->view('Gudang_wg/index',$data);
	}
	...
	public function server_side_wip(){
		...
		foreach($query->result_array() as $row)
		{
			...
			if($date_filter==''){
				...
				$SUM_DETAIL = $this->db->select('SUM(material_terpakai) AS berat')->get_where('tmp_production_real_detail', array('catatan_programmer'=>$row['kode_spk'].'/'.$get_ID_PRO[0]->upload_date,'id_production_detail'=>$ID_PRO))->result();
				...
				$nestedData[]	= "<div align='right'>".number_format($TOT_MATERIAL * $row['qtyTotal'],4)."</div>";
				$nestedData[]	= "<div align='center'><button type='button' class='btn btn-sm btn-warning look_history' data-length='".$row['length']."'  data-length_awal='".$row['length_awal']."' data-kode_spk='".$row['kode_spk']."' data-id_production_detail='".$ID_PRO."' data-category='1' data-qty='".$row['tot_qty']."' data-tot_product='".$row['qtyTotal']."'><i class='fa fa-eye'></i></button></div>";
			}else{
				$nestedData[]	= "<div align='right'>".number_format($row['qtyTotal'],4)."</div>";
	...
	public function finish_good(){
		...
		history('View data gudang finish good');
		$this->load->view('Gudang_wg/finish_good',$data);
	}
	...
	public function server_side_fg(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$nestedData[]	= "<div align='left'>".$spec."</div>";
			if($FLAG == 'cutting'){
				$nestedData[]	= "<div align='left'>".$row['length']."</div>";
			}
	...
	public function query_data_fg($status, $date_filter, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		...
		if($date_filter == ''){
			...
			if($status == 'cutting'){
				$where = " AND d.id IS NOT NULL AND c.id_category='pipe' AND d.app = 'Y' AND f.qc_date IS NOT NULL AND (f.kode_delivery IS NULL AND f.spool_induk IS NULL) ";
				$LEFT_JOIN = " LEFT JOIN so_cutting_detail f ON d.id = f.id_header,";
				$FIELD_CUTTING = "f.length AS length_awal, GROUP_CONCAT(f.length_split  ORDER BY f.id ASC SEPARATOR '<br>')AS length, f.cutting_ke,";
				$LIKE_PRODUCT_CODE = "CONCAT(SUBSTRING_INDEX(a.product_code, '.', 1),'.', a.product_ke,'.' ,f.cutting_ke)";
	...
	public function ExcelWIPGudangFG($date_filter,$status){
		//membuat objek PHPExcel
		set_time_limit(0);
		ini_set('memory_limit','1024M');
		...
		if($date_filter == '0'){
			...
			$sql = "
				...
				WHERE 1=1 
					...
					".$where."
					".$where2."
				GROUP BY a.id_milik, a.no_spk ORDER BY a.id_produksi ASC
			";
		}
		else{
	...
	public function ExcelWIPGudangWIP($date_filter,$status){
		//membuat objek PHPExcel
		set_time_limit(0);
		ini_set('memory_limit','1024M');
		...
		if($date_filter == '0'){
			...
			$sql = "
				...
				WHERE 1=1
					AND DATE(a.hist_date) = '".$date_filter."'
					".$where."
				GROUP BY a.id_milik ORDER BY no_so ASC
			";
		}
		// ECHO $sql; exit;
		$restDetail1	= $this->db->query($sql)->result_array();
