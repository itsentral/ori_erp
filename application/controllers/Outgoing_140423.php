...
class Outgoing extends CI_Controller {
    ...
    public function server_side_outgoing(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$type = $IP_ID;
			if($type != 'non-so'){
				$type = get_name('so_number','so_number','id_bq',$IP_ID);
			}
	...
	public function outgoing_subgudang(){
		...
		$no_field	= $this->db->group_by('id_bq')->order_by('id_bq','asc')->select('id_bq, SUM(qty_out) AS qty_out, SUM(qty) AS qty, id_milik')->get_where('request_outgoing',array('category'=>'mat','approve'=>'P'))->result_array();
	...
	public function get_customer2(){
		...
		$option2 = '';
		if($field_joint == 'yes'){
			$getSPK = $this->db->group_by('id_milik, approve_date')->get_where('request_outgoing',array('id_bq'=>$tipe_out,'id_milik !='=>NULL))->result_array();
			// $option2 .= "<option value='0'>ALL SPK</option>";
			foreach ($getSPK as $key => $value) {
				$option2	.= "<option value='".$value['id_milik']."/".$value['approve_date']."'>SPK ".get_name('so_detail_header','no_spk','id',$value['id_milik'])." -  [".date('d-M-Y H:i:s',strtotime($value['approve_date']))."]</option>";
			}
		}
		...
		if($tipe_out != 'non-so'){
			if($field_joint == 'no'){
				...
				if($no_spk_field != '0'){
					...
					$sql = "SELECT a.* FROM request_outgoing a WHERE a.id_bq='".$tipe_out."' AND qty_out < qty AND category = 'mat' AND id_milik='".$EXPLODE_FJ[0]."' AND approve_date='".$EXPLODE_FJ[1]."' AND id_material <> '0'";
				}
				else{
					$sql = "SELECT a.* FROM request_outgoing a WHERE a.id_bq='".$tipe_out."' AND qty_out < qty AND category = 'mat'";
				}
