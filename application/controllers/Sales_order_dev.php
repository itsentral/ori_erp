...
class Sales_order_dev extends CI_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model('master_model');
		$this->load->model('sales_order_model_dev');
	...
	public function index(){
		$this->sales_order_model_dev->index();
	}
	
	public function server_side_so(){
		$this->sales_order_model_dev->get_data_json_so();
	}
	
	public function modal_detail_so(){
		$this->sales_order_model_dev->modal_detail_so();
	}
	
	public function modal_deal_so(){
		$this->sales_order_model_dev->modal_deal_so();
	}
	
	public function delete_sebagian_so(){
		$this->sales_order_model_dev->delete_sebagian_so();
	}
	
	public function delete_sebagian_so_mat(){
		$this->sales_order_model_dev->delete_sebagian_so_mat();
	}
	
	public function delete_sebagian_so_eng_pack_trans(){
		$this->sales_order_model_dev->delete_sebagian_so_eng_pack_trans();
	}

	public function add_sebagian_so_eng_pack_trans(){
		$this->sales_order_model_dev->add_sebagian_so_eng_pack_trans();
	}
	
	public function update_qty_so(){
		$this->sales_order_model_dev->update_qty_so();
	}
	
	public function insert_sales_order(){
		$this->sales_order_model_dev->insert_sales_order();
	}
	
	public function ajukan_so(){
		$this->sales_order_model_dev->ajukan_so();
	}
	
	public function deal_project(){
		$this->sales_order_model_dev->deal_project();
	}
	
	public function get_add(){
		$this->sales_order_model_dev->get_add();
	}
	
	public function print_sales_order(){
		$this->sales_order_model_dev->print_sales_order();
	}
	
	public function print_sales_order_ex_price(){
		$this->sales_order_model_dev->print_sales_order_ex_price();
	}
	
	public function print_sales_order_usd(){
		$this->sales_order_model_dev->print_sales_order_usd();
	}
	...
	public function approve_so(){
		$this->sales_order_model_dev->approve_so();
	}
	
	public function server_side_so_app(){
		$this->sales_order_model_dev->get_data_json_so_app();
	}
	
	public function modal_approve_so(){
		$this->sales_order_model_dev->modal_approve_so();
	}

	public function modal_approve_so_new(){
		$this->sales_order_model_dev->modal_approve_so_new();
	}
	
	public function process_approve_so(){
		$this->sales_order_model_dev->process_approve_so();
	}
	
	public function insert_sales_order_approve(){
		$this->sales_order_model_dev->insert_sales_order_approve();
	}
	...
	public function sales_invoice(){
		$controller			= ucfirst(strtolower($this->uri->segment(1))).'/sales_invoice';
		$Arr_Akses			= getAcccesmenu($controller);
		if($Arr_Akses['read'] !='1'){
			$this->session->set_flashdata("alert_data", "<div class=\"alert alert-warning\" id=\"flash-message\">You Don't Have Right To Access This Page, Please Contact Your Administrator....</div>");
			redirect(site_url('dashboard'));
	...
	public function data_side_sales_invoice(){
		$this->sales_order_model_dev->get_json_sales_invoice();
	}
