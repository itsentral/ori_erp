...
class Ledger_incustomer extends CI_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model('master_model');
		$this->load->model('Ledger_incustomer_model');
	...
	public function index(){
		...
		$data = array(
			'title'			=> 'Laporan Ledger In Customer',
			'action'		=> 'index',
			'data_menu'		=> $menu_akses,
			'akses_menu'	=> $Arr_Akses
		);

		$this->load->view('Report_new/Ledger_incustomer/index', $data);
	}
	...
	public function get_data_json(){
		...
		$fetch = $this->Ledger_incustomer_model->get_ledger_data($bulan, $tahun);

		echo json_encode($fetch);
	}

	public function excel_ledger_incustomer(){
		set_time_limit(0);
		...
		$sheet->setCellValue('A'.$Row, 'LAPORAN LEDGER IN CUSTOMER');
		...
		$objPHPExcel->getActiveSheet()->setTitle('Ledger In Customer');
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename="Ledger_InCustomer_'.$Arr_Bulan[(int)$bulan].'_'.$tahun.'.xlsx"');
