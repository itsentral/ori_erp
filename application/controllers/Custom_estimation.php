...
class Custom_estimation extends CI_Controller {
	...
	public function index(){
		if($this->input->post()){
			...
			//List Dropdown
			$ListProduct		= $this->db->query("SELECT * FROM product_parent WHERE type = 'custom' ORDER BY product_parent ASC")->result_array();
			...
			$this->load->view('Component_custom/custom_estimation', $data);
		}
	}

	public function modalAddDefaultx(){
	...
	public function pipe(){
		if($this->input->post()){
			...
			$getPar = $this->db->get_where('product_parent', array('id'=>$data['component_list']))->row();
			if ($getPar->product_parent == 'puddle flange') {
				$ins = 'PF';
			}
			if ($getPar->product_parent == 'Plate') {
				$ins = 'PL';
			}
			if ($getPar->product_parent == 'Rib') {
				$ins = 'RB';
			}
			if ($getPar->product_parent == 'Support') {
				$ins = 'SU';
			}
			if ($getPar->product_parent == 'Spectacle blind') {
				$ins = 'SB';
			}
			if ($getPar->product_parent == 'Spacer') {
				$ins = 'SC';
			}
			if ($getPar->product_parent == 'Saddle') {
				$ins = 'SD';
			}
			$arr_cari = array(
				'pressure'=>$pressure,
				...
				'waste_1'=>$data['disc_waste'],
				'waste_2'=>$data['square_waste'],
				'waste_3'=>$data['tri_waste'],
				'waste_4'=>$data['cyl_waste'],
				'thickness_1'=>$data['disc_thickness'],
				'thickness_2'=>$data['square_thickness'],
				'thickness_3'=>$data['tri_thickness'],
				'thickness_4'=>$data['cyl_thickness'],
				'max_1'=>$data['disc_max'],
				'max_2'=>$data['square_max'],
				'max_3'=>$data['tri_max'],
				'max_4'=>$data['cyl_max'],
				'min_1'=>$data['disc_min'],
				'min_2'=>$data['square_min'],
				'min_3'=>$data['tri_min'],
				'min_4'=>$data['cyl_min'],
				'dim1_1'=>$data['disc_in'],
				'dim1_2'=>$data['square_panjang'],
				'dim1_3'=>$data['tri_alas'],
				'dim1_4'=>$data['cyl_d'],
				'dim2_1'=>$data['disc_out'],
				'dim2_2'=>$data['square_lebar'],
				'dim2_3'=>$data['tri_tinggi'],
				'dim2_4'=>$data['cyl_l']
			);
			if ($data['cust'] == 'C100-1903000') {
				$kode_product	= $ins."-".$KdResinSystem."PN".$KdPressure."-".$KdLiner."-".$ket_plus;
				$arr_cari['cust'] = '';
			}else {
				$kode_product	= $ins."-".$KdResinSystem."PN".$KdPressure."-".$KdLiner."-".$cust.$ket_plus;
				$arr_cari['cust'] = $cust;
			}

			$getProNum = $this->db->get_where('component_header', $arr_cari)->num_rows();
			if ($getProNum) {
				$Arr_Kembali	= array(
					'pesan'		=>'Specifications are already in the list. Check again ...',
					'status'	=> 3
				...
				$getKod = $this->db->query("select * from component_header where id_product like '%$kode_product%' ORDER BY id_product DESC LIMIT 1")->row();
				if (count($getKod) == 0) {
					$num = 1;
				}else {
					$num = ((int) substr($getKod->id_product,-4))+1;
				}
				$kode_product	.= str_pad($num,4,"0");
				// echo $kode_product; exit;
				$srcType		= "SELECT id_product FROM component_header WHERE id_product='".$kode_product."' ";
				$NumRow			= $this->db->query($srcType)->num_rows();
				...
				if($NumRow > 0){
					...
					$qDefault		= "SELECT * FROM help_default WHERE product_parent = 'Custom' ";
					$getDefault		= $this->db->query($qDefault)->result();

					$ArrHeader	= array(
						'id_product'			=> $kode_product,
						'parent_product'		=> $getPar->product_parent,
						'nm_product'			=> $getPar->product_parent,
						'series'				=> $data['series']."-".$KdLiner,
						'standart_code'			=> $getDefault[0]->standart_code,
						'cust'					=> $cust,
						'ket_plus'				=> $ket_plus2,
					...
					$ArrFooter	= array(
							...
							'min'			=> $data['mix_lin_thickness'],
							'max'			=> $data['max_lin_thickness'],
							'hasil'			=> "OK"//$data['hasil_linier_thickness']
					);
					...
					$ArrFooter2	= array(
							...
							'min'			=> $data['mix_lin_thickness2'],
							'max'			=> $data['max_lin_thickness2'],
							'hasil'			=> "OK"//$data['hasil_linier_thickness2']
					);
					...
					$ArrFooter3	= array(
							...
							'min'			=> $data['mix_lin_thickness3'],
							'max'			=> $data['max_lin_thickness3'],
							'hasil'			=> "OK"//$data['hasil_linier_thickness3']
					);
	...
	public function getMaterialx(){
		...
		//TYPE VEIL
		if($id_category == 'TYP-0003' OR $id_category == 'TYP-0004' OR $id_category == 'TYP-0001' OR $id_category == 'TYP-0006'){
		...
		if($id_category == 'TYP-0005'){
			...
			if($NumMic != 0){
				$micron		=  $restMicron[0]->nilai_standard;
				$thickness	= (($micron/1000)/ $bw * $jumlah * (2 / 2.56)) + (($micron/1000)/ $bw * $jumlah * (2 / 1.2) * $resin);
			}
	...
	public function getMicronPlastic(){
		...
		$sqlMicron	= "SELECT * FROM raw_materials_bq_standard WHERE id_material ='".$id_material."' AND nm_standard='thickness' LIMIT 1 ";
	...
	public function getDataJSON2(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$nestedData[]	= "<div align='left'>".$row['nm_product']."<br>".$detail."</div>";
			$nestedData[]	= "<div align='center'>".$row['stiffness']."</div>";
	...
	public function queryDataJSON2($series, $komponen, $cust, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		$where_series = "";
		if(!empty($series)){
			$where_series = " AND a.series = '".$series."' ";
		}
		...
		$sql = "
			...
			WHERE 1=1
				...
				".$where_komponen."
				".$where_cust."
				AND a.deleted ='N' AND a.cust IS NOT NULL AND a.cust != '' AND (
				a.id_product LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.nm_product LIKE '%".$this->db->escape_like_str($like_value)."%'
	        )
		";
	...
	public function equalteemould(){
		if($this->input->post()){
			...
			$kode_product	= "T-".$KdResinSystem."PN".$KdPressure."DN".$KdDiameter."-".$KdLiner."-".$cust.$ket_plus;
			// echo $kode_product; exit;
			$srcType		= "SELECT id_product FROM component_header WHERE id_product='".$kode_product."' ";
			$NumRow			= $this->db->query($srcType)->num_rows();
	...
	public function equalteeslongsong(){
		if($this->input->post()){
			...
			$kode_product	= "Q-".$KdResinSystem."PN".$KdPressure."DN".$KdDiameter."-".$KdLiner."-".$cust;
			// echo $kode_product; exit;
			$srcType		= "SELECT id_product FROM component_header WHERE id_product='".$kode_product."' ";
			$NumRow			= $this->db->query($srcType)->num_rows();
	...
	public function equalteeslongsongXXXX(){
		if($this->input->post()){
			$data 			= $this->input->post();
			$data_session	= $this->session->userdata;
			$mY				=  date('ym');
	...
	public function reducerteeslongsongXXX(){
		if($this->input->post()){
			$data 			= $this->input->post();
			$data_session	= $this->session->userdata;
			$mY				=  date('ym');
			...
			$kode_product	= "L-".$KdResinSystem."PN".$KdPressure."DN".$KdDiameter."-".$KdLiner."-DN".$KdDiameter2."-".$cust;
			// echo $kode_product; exit;
			$srcType		= "SELECT id_product FROM component_header WHERE id_product='".$kode_product."' ";
			$NumRow			= $this->db->query($srcType)->num_rows();
	...
	public function flangeslongsongBEFORE(){
		if($this->input->post()){
			$data = $this->input->post();
			$data_session			= $this->session->userdata;
			$mY		=  date('ym');
	...
	public function flangeslongsongXXXXXX(){
		if($this->input->post()){
			$data = $this->input->post();
			$data_session			= $this->session->userdata;
			$mY		=  date('ym');
			...
			$kode_product	= "A-".$KdResinSystem."PN".$KdPressure."DN".$KdDiameter."-".$KdLiner."-".$cust;;
			// echo $kode_product; exit;
			$srcType		= "SELECT id_product FROM component_header WHERE id_product='".$kode_product."' ";
			$NumRow			= $this->db->query($srcType)->num_rows();
			...
			if($NumRow > 0){
				...
				foreach($ListDetail2_neck1 AS $val => $valx){
					...
					$dataMaterial2	= $this->db->query("SELECT nm_material, id_category, nm_category FROM raw_materials WHERE id_material='".$valx['id_material']."' LIMIT 1")->result_array();
					echo "SELECT nm_material, id_category, nm_category FROM raw_materials WHERE id_material='".$valx['id_material']."' LIMIT 1 <br>";
					$ArrDetail2_neck1[$val]['id_product'] 	= $kode_product;
					$ArrDetail2_neck1[$val]['detail_name'] 	= $data['detail_name2_neck1'];
					$ArrDetail2_neck1[$val]['acuhan'] 		= $data['acuhan_2_neck1'];
					$ArrDetail2_neck1[$val]['id_category'] 	= $dataMaterial2[0]['id_category'];
					$ArrDetail2_neck1[$val]['nm_category'] 	= $dataMaterial2[0]['nm_category'];
					$ArrDetail2_neck1[$val]['id_material'] 	= $valx['id_material'];
					$ArrDetail2_neck1[$val]['nm_material'] 	= $dataMaterial2[0]['nm_material'];
						$valueM							= (!empty($valx['value']))?$valx['value']:'';
					$ArrDetail2_neck1[$val]['value'] 			= $valueM;
						$thicknessM						= (!empty($valx['thickness']))?$valx['thickness']:'';
					$ArrDetail2_neck1[$val]['thickness'] 		= $thicknessM;
	...
	public function flange_mould_edit(){
		if($this->input->post()){
			...
			$this->load->view('Component_custom/edit/flange_mould_edit', $data);
		}
	}
	...
	public function flange_slongsong_edit(){
		if($this->input->post()){
			...
			$this->load->view('Component_custom/edit/flange_slongsong_edit', $data);
		}
	}
	...
	public function blindflange_edit(){
		if($this->input->post()){
			...
			if($this->db->trans_status() === FALSE){
				...
				history('Edit Est Blind Flange in custom code : '.$kode_product.' to '.$kode_product);
			}
			echo json_encode($Arr_Kembali);
		}
		else{
			...
			$this->load->view('Component_custom/edit/blindflange_edit', $data);
		}
	}
	...
	public function end_cap_edit(){
		if($this->input->post()){
			...
			if($this->db->trans_status() === FALSE){
				...
				history('Edit Est End Cap in custom code : '.$kode_product.' to '.$kode_product);
			}
			echo json_encode($Arr_Kembali);
		}
		else{
			...
			$this->load->view('Component_custom/edit/end_cap_edit', $data);
		}
	}
	...
	public function getDataJSONDefault(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$nestedData[]	= "	<div align='center'>
									...
									<button type='button' id='editDefault' data-id='".$row['id']."' class='btn btn-sm btn-primary' title='Edit Data' data-role='qtip'><i class='fa fa-edit'></i></button>
								</div>";
			$data[] = $nestedData;
            $urut1++;
            $urut2++;
	...
	public function modalAddDefault(){
		$this->load->view('Component_custom/modalChangeDefault');
	}
	...
	public function addDefaultSave(){
		...
		$getOld = $this->db->get_where('help_default', array('product_parent'=>'Custom'))->num_rows();

		//$getNum	= $this->db->query("SELECT * FROM help_default WHERE product_parent='".$data['komponen']."' AND diameter='".$data['diameter']."' AND standart_code='".$data['standart_code']."' ")->num_rows();

		//if($getNum < 1){
			$this->db->trans_start();
				if ($getOld != 0) {
					$this->db->where('product_parent','Custom')->delete('help_default');
				}
