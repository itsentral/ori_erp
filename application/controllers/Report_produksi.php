...
class Report_produksi extends CI_Controller {
    ...
    public function index(){
		...
		$data = array(
			'title'			=> 'Report Gudang Produksi',
			'action'		=> 'index',
			'row_group'		=> $data_Group,
			'akses_menu'	=> $Arr_Akses
		);

		$this->load->view('Report_new/Report_produksi/index',$data);
	}
	...
	public function query_data_spk_produksi_progress($tgl_awal,$tgl_akhir,$like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
        ...
        $sql = "SELECT 
					...
					b.id_gudang_dari,
					b.id_gudang_ke,
                    (SELECT z.price_book FROM price_book_produksi z WHERE z.id_material=a.id_material and z.updated_date <= a.update_date ORDER BY z.id DESC LIMIT 1) AS cost_book 
                FROM
                ...
                WHERE 1=1 ".$WHERE_DATE."
                    AND (b.id_gudang_dari IN (".$this->gudang.") OR b.id_gudang_ke IN (".$this->gudang."))
                    AND a.qty_oke > 0
    ...
    public function excel_report_subgudang(){
		...
		$sheet->setCellValue('A'.$Row, 'REPORT GUDANG PRODUKSI');
		...
		$sheet->setTitle('Report Gd.Produksi');
		...
		header('Content-Disposition: attachment;filename="report-gudang-produksi.xls"');
		//unduh file
	...
	public function excel_report_subgudang2(){
		...
		$sheet->setCellValue('A'.$Row, 'REPORT GD.PRODUKSI GROUP');
		$sheet->getStyle('A'.$Row.':'.$Col_Akhir.$NewRow)->applyFromArray($mainTitle);
		$sheet->mergeCells('A'.$Row.':'.$Col_Akhir.$NewRow);
		...
		$sheet->setCellValue('G'.$NewRow, 'ID TRANS');
		$sheet->getStyle('G'.$NewRow.':G'.$NextRow)->applyFromArray($tableHeader);
		$sheet->mergeCells('G'.$NewRow.':G'.$NextRow);
		$sheet->getColumnDimension('G')->setWidth(20);
        ...
        $sheet->setCellValue('I'.$NewRow, 'ID MATERIAL');
		$sheet->getStyle('I'.$NewRow.':I'.$NextRow)->applyFromArray($tableHeader);
		$sheet->mergeCells('I'.$NewRow.':I'.$NextRow);
		$sheet->getColumnDimension('I')->setWidth(20);
		...
		$sheet->setCellValue('L'.$NewRow, 'COST BOOK');
		...
		if($restDetail1){
			...
			foreach($restDetail1 as $key => $row){
                ...
                $awal_col++;
				$Cols			= getColsChar($awal_col);
				$sheet->setCellValue($Cols.$awal_row, $id_reff_jurnal);
				$sheet->getStyle($Cols.$awal_row)->applyFromArray($tableBodyLeft);
		...
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		//ubah nama file saat diunduh
		header('Content-Disposition: attachment;filename="report-produksi-group.xls"');
		//unduh file
		$objWriter->save("php://output");
	}

	//AKTUAL PRODUKSI
	public function get_data_json_spk_produksi_progress3(){
		$controller			= ucfirst(strtolower($this->uri->segment(1)));
		$Arr_Akses			= getAcccesmenu($controller);
		$requestData	= $_REQUEST;
		$fetch			= $this->query_data_spk_produksi_progress3(
			$requestData['tgl_awal'],
			$requestData['tgl_akhir'],
			$requestData['search']['value'],
			$requestData['order'][0]['column'],
	...
	public function query_data_spk_produksi_progress3($tgl_awal,$tgl_akhir,$like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		$WHERE_DATE = "AND a.updated_date LIKE '".date('Y')."-".date('m')."%' ";
		if($tgl_awal != '0'){
			$WHERE_DATE = "AND (DATE( a.updated_date ) BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."' )";
		}

        $sql = "SELECT 
                    (@row:=@row+1) AS nomor,
                    a.updated_date AS tanggal,
					CONCAT(a.kode_spk,'/',a.status_date) AS kode_trans,
					'laporan produksi' AS tipe,
					a.no_ipp,
					a.product,
					d.no_spk,
					a.id_material_aktual as id_material,
					e.nm_material,
					b.id_gudang,
					a.aktual_total AS qty,
                    (SELECT z.price_book FROM price_book_produksi z WHERE z.id_material=a.id_material and z.updated_date <= a.updated_date ORDER BY z.id DESC LIMIT 1) AS cost_book 
                FROM
                    production_spk_input a
					LEFT JOIN production_spk_parsial b ON a.kode_spk=b.kode_spk and a.updated_date=b.updated_date
					LEFT JOIN production_spk d ON a.kode_spk=d.kode_spk
					LEFT JOIN raw_materials e ON a.id_material_aktual = e.id_material,
                    (SELECT @row:=0) r
                WHERE 1=1 ".$WHERE_DATE."
                    AND a.aktual_total > 0
                    AND DATE(a.updated_date) > '2023-10-30'
                    AND (
	...
	public function excel_report_subgudang3(){
		...
		$sheet->setTitle('Report Produksi');
		//mulai menyimpan excel format xlsx, kalau ingin xls ganti Excel2007 menjadi Excel5
		$objWriter		= PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
		ob_end_clean();
		//sesuaikan headernya
