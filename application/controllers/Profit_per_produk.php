...
class Profit_per_produk extends CI_Controller {
    ...
    public function index(){
		...
		$sales_order = $this->db
                            ...
                            ->result_array();
		$data = array(
			'title'			=> 'Report Gross Profit Per Produk Lap.Produksi',
			'action'		=> 'add',
			'sales_order'	=> $sales_order
		);
		history('View data profit per produk');
		$this->load->view('Profit_per_produk/index',$data);
	}
    ...
    public function show_history(){
		...
		$data_html = $this->load->view('Profit_per_produk/detail', $dataArr, TRUE);
    ...
    public function download_excel(){
		...
		$sheet->setCellValue('A'.$Row, 'REPORT PROFIT GROSS PER PRODUK LAP PRODUKSI');
		...
		$sheet->setCellValue('H'.$NewRow, 'QTY PRODUKSI');
		$sheet->getStyle('H'.$NewRow.':H'.$NextRow)->applyFromArray($tableHeader);
		$sheet->mergeCells('H'.$NewRow.':H'.$NextRow);
		$sheet->getColumnDimension('H')->setWidth(10);

		$sheet->setCellValue('I'.$NewRow, 'NILAI PER PCS PRODUKSI (IDR)');
		...
		$sheet->setTitle('PROFIT GROSS PER PRODUK');
		...
		header('Content-Disposition: attachment;filename="profit-gross-per-produk-report-produksi-'.$no_so.'.xls"');
		//unduh file
