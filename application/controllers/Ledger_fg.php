...
class Ledger_fg extends CI_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model('master_model');
		$this->load->model('Ledger_fg_model');
	...
	public function index(){
		...
		$data = array(
			'title'			=> 'Laporan Ledger Finish Good',
			'action'		=> 'index',
			'data_menu'		=> $menu_akses,
			'akses_menu'	=> $Arr_Akses
		);

		$this->load->view('Report_new/Ledger_fg/index', $data);
	}
	...
	public function excel_ledger_fg(){
		set_time_limit(0);
		...
		$sheet->setCellValue('A'.$Row, 'LAPORAN LEDGER');
		...
		header('Content-Disposition: attachment;filename="Ledger_FG_'.$Arr_Bulan[(int)$bulan].'_'.$tahun.'.xlsx"');
