<?php
class Total_value_product extends CI_Controller {
    ...
    public function server_side_product_stock_wip(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$nestedData[]	= "<div align='center'>
								<button type='button' class='btn btn-sm btn-warning look_history' title='History' data-no_so='".strtoupper($row['no_so'])."' data-no_spk='".$row['no_spk']."' data-product='".$row['product']."' data-kode_trans='".str_replace($row['kode_trans'], " ", "_")."'><i class='fa fa-history'></i></button>
								</div>";
			$data[] = $nestedData;
            $urut1++;
            $urut2++;
	...
	public function query_data_json_product_stock_fg($gudang, $date_filter, $category, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
