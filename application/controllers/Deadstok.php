...
class Deadstok extends CI_Controller {
	...
	function jurnalFG($idmaterial,$idtrans){
		...
		$Date		    = date('Y-m-d'); 
			...
			$fg = $this->db->query("SELECT tanggal,keterangan,product,no_so,no_spk,id_trans, nilai_wip as wip, material as material, wip_direct as wip_direct, wip_indirect as wip_indirect,  wip_foh as wip_foh, wip_consumable as wip_consumable, nilai_unit as finishgood  FROM data_erp_fg WHERE no_spk ='".$idtrans."' AND tanggal ='".$Date."' AND keterangan='Deadstok to Finish Good' AND id_material='".$idmaterial."'")->result();
			...
			foreach($fg AS $data){
				...
				$coafg   		='1103-04-01';
					 ...
					 $det_Jurnaltes[]  = array(
					  ...
					  'debet'         => $finishgood,
					  'kredit'        => 0,
					  'jenis_jurnal'  => 'Deadstok to Finish Good',
