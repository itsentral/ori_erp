            ...
            $tgl1       		= $gethd->tgl_invoice;
                ...
                if($jenis_invoice=='progress'){
					...
					if($instalasi !='1')
					{
						...
						$wip = $this->db->query("SELECT tanggal,keterangan,product,no_so,no_spk,kode_trans,qty,id_trans, nilai_unit as finishgood  FROM data_erp_in_customer WHERE kode_delivery IN (".$kode_delivery.") AND jenis = 'out'")->result();
						...
						foreach($wip AS $data){
								...
								$this->db->query("UPDATE  warehouse_stock_incustomer SET qty = qty-1  WHERE no_so ='".$noso."' AND kode_trans ='".$kode_trans."'  AND no_spk ='".$nospk."' AND product ='".$nm_material."'");
								}
