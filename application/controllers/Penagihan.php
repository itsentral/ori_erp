...
//		$Nomor_JV				= $this->Jurnal_model->get_Nomor_Jurnal_Sales('101',$tgl);
		...
		if($this->db->trans_status() === FALSE or $db2->trans_status()=== FALSE){
		 ...
		 $Arr_Return		= array(
            ...
            $wip = $this->db->query("SELECT tanggal,keterangan,product,no_so,no_spk,id_trans, nilai_unit as finishgood  FROM data_erp_in_transit WHERE kode_delivery IN (".$kode_delivery.") AND jenis = 'out'")->result();
			...
			$stokfg = $this->db->query("SELECT
										...
										`data_erp_in_customer`.`nilai_wip` AS `nilai_wip`,
										`data_erp_in_customer`.`material` AS `material`,
										`data_erp_in_customer`.`wip_direct` AS `wip_direct`,
										`data_erp_in_customer`.`wip_indirect` AS `wip_indirect`,
										`data_erp_in_customer`.`wip_consumable` AS `wip_consumable`,
										`data_erp_in_customer`.`wip_foh` AS `wip_foh`,
