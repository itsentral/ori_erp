    ...
    public function get_data_json_spk_produksi_progress2(){
		...
		foreach($query->result_array() as $row)
		{
			...
			if($row['nilai_wip'] > 0 AND $qty > 0){
				$nilai_wip	= $row['nilai_wip'] / $qty;
			}
			$material	= 0;
			if($row['material'] > 0 AND $qty > 0){
				$material	= $row['material'] / $qty;
			}
			$wip_direct	= 0;
			if($row['wip_direct'] > 0 AND $qty > 0){
				$wip_direct	= $row['wip_direct'] / $qty;
			}
			$wip_indirect	= 0;
			if($row['wip_indirect'] > 0 AND $qty > 0){
				$wip_indirect	= $row['wip_indirect'] / $qty;
			}
			$wip_consumable	= 0;
			if($row['wip_consumable'] > 0 AND $qty > 0){
				$wip_consumable	= $row['wip_consumable'] / $qty;
			}
			$wip_foh	= 0;
			if($row['wip_foh'] > 0 AND $qty > 0){
				$wip_foh	= $row['wip_foh'] / $qty;
			}
			$nestedData[]	= "<div align='right'>".number_format($nilai_wip,2)."</div>";
			$nestedData[]	= "<div align='right'>".number_format($material,2)."</div>";
			$nestedData[]	= "<div align='right'>".number_format($wip_direct,2)."</div>";
			$nestedData[]	= "<div align='right'>".number_format($wip_indirect,2)."</div>";
			$nestedData[]	= "<div align='right'>".number_format($wip_consumable,2)."</div>";
			$nestedData[]	= "<div align='right'>".number_format($wip_foh,2)."</div>";
			$data[] = $nestedData;
            $urut1++;
            $urut2++;
		}
