...
class Report_fg extends CI_Controller {
    ...
    public function get_data_json_fg_costbook(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$out = $this->db->query("SELECT a.qty_ke as qty_out FROM data_erp_in_transit a WHERE jenis='in' AND id_trans=$idtrans  GROUP BY a.id_trans ORDER BY a.qty_ke DESC")->row();
