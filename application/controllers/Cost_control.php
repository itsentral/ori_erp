...
class Cost_control extends CI_Controller {
	...
	public function ExcelPerbandingan(){
		...
		$qHeader		= "SELECT * FROM so_component_header WHERE id_milik='".$id_milik."'";
		$restHeader		= $this->db->query($qHeader)->result_array();
			...
			if($row['qty_awal'] == $row['qty_akhir']){
				$nestedData[]	= "<div align='center'><span class='badge bg-blue'>".$row['product_ke']."</span></div>";
					...
					if($row['upload_real'] == 'Y' AND $row['upload_real2'] == 'Y'){
						...
						$btn3	= "&nbsp;<button type='button' class='btn btn-sm btn-success btn_download' title='Download Excel' data-id_produksi='".$row['id_produksi']."' data-id_milik='".$row['id_milik']."' data-qty_awal='".$row['qty_awal']."' data-qty_akhir='".$row['qty_akhir']."' data-qty='".$row['id']."' data-nm_product='".str_replace(' ','-',$row['id_category'])."'><i class='fa fa-file-excel-o'></i></button>";
					}
