...
class Ppic extends CI_Controller {
	...
	function jurnalOuttoWip($kode){
		...
		$noReffTrans = getReffTranStok('101');
			...
			$wip = $this->db->query("SELECT id, tanggal,keterangan,product,no_so,no_spk,id_trans, nilai_wip as wip, material as material, wip_direct as wip_direct, wip_indirect as wip_indirect,  wip_foh as wip_foh, wip_consumable as wip_consumable, nilai_wip as finishgood  FROM data_erp_wip_group WHERE kode_spool ='".$kode."' AND tanggal ='".$Date."' AND jenis='in spool'")->result();
			...
			foreach($wip AS $data){
				...
				$coafg   		='1103-04-01';
					...
					$this->db->query("UPDATE data_erp_wip_group SET reff_trans_stok = '$noReffTrans'  WHERE id ='".$data->id."' ");		
