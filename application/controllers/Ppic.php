...
class Ppic extends CI_Controller {
	...
	function jurnalOuttoWip($kode){
		...
		$Date		    = date('Y-m-d'); 
			...
			$wip = $this->db->query("SELECT tanggal,keterangan,product,no_so,no_spk,id_trans, nilai_wip as wip, material as material, wip_direct as wip_direct, wip_indirect as wip_indirect,  wip_foh as wip_foh, wip_consumable as wip_consumable, nilai_unit as finishgood  FROM data_erp_fg WHERE kode_spool ='".$kode."' AND tanggal ='".$Date."' AND jenis='in spool'")->result();
