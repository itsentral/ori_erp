...
function getEstimasiVsAktual($id_milik, $no_ipp, $qty, $id_production_detail) {
    ...
    $ArrayResult['est_mat']     = 0;
    $ArrayResult['act_mat']     = 0;
    $ArrayResult['est_price']   = 0;
    $ArrayResult['act_price']   = 0;
...
function getEstimasiVsAktualGroupProduct($id_milik, $no_ipp, $qty) {
    $CI =& get_instance();

    $restHeader		= $CI->db->get_where('so_component_header',array('id_milik'=>$id_milik))->result_array();
    if ($restHeader[0]['parent_product']!='branch joint' && $restHeader[0]['parent_product']!='field joint' && $restHeader[0]['parent_product']!='shop joint'){
    $qDetail1		= 	"
        (SELECT 'detail' AS tipe, a.id_detail, a.id_milik, a.id_bq, a.detail_name, a.nm_category, a.nm_material, sum(a.last_cost * ".$qty.") AS est_material, a.price_mat AS est_harga, b.material_terpakai AS real_material FROM so_component_detail a LEFT JOIN production_real_detail b ON a.id_detail = b.id_detail WHERE a.id_bq='BQ-".$no_ipp."'  AND a.id_milik='".$id_milik."' AND a.id_category NOT IN ( 'TYP-0001', 'TYP-0030' ) AND a.detail_name IN ('LINER THIKNESS / CB','GLASS','RESIN AND ADD') GROUP BY a.id_material)
        UNION
