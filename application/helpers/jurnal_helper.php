...
// cek deferred gudang project
	...
	function insert_jurnal_cutting($ArrData, $kode_pro){
		...
		$CI->jurnalOuttoWipcutting($kode_pro);
	}
	...
	function jurnalOuttoWipcutting($kode){
		...
		$Date		    = date('Y-m-d'); 
			...
			$jurnalwip = $CI->db->query("SELECT tanggal,keterangan,product,no_so,no_spk,id_trans, nilai_wip as wip, material as material, wip_direct as wip_direct, wip_indirect as wip_indirect,  wip_foh as wip_foh, wip_consumable as wip_consumable, nilai_wip as finishgood  FROM data_erp_wip_group WHERE id_trans ='".$kode."' AND tanggal ='".$Date."' AND jenis LIKE 'in cutting%'")->result();
			...
			$jurnalfg = $CI->db->query("SELECT tanggal,keterangan,product,no_so,no_spk,id_trans, nilai_wip as wip, material as material, wip_direct as wip_direct, wip_indirect as wip_indirect,  wip_foh as wip_foh, wip_consumable as wip_consumable, nilai_unit as finishgood  FROM data_erp_fg WHERE id_trans ='".$kode."' AND tanggal ='".$Date."' AND jenis LIKE 'out cutting%'")->result();
			
			// if(!empty($fg)){
            // $jurnalfg = $CI->db->query("SELECT tanggal,keterangan,product,no_so,no_spk,id_trans, nilai_wip as wip, material as material, wip_direct as wip_direct, wip_indirect as wip_indirect,  wip_foh as wip_foh, wip_consumable as wip_consumable, nilai_unit as finishgood  FROM data_erp_fg WHERE id_pro ='".$kode."' AND tanggal ='".$Date."' AND jenis LIKE 'out cutting%'")->result();
