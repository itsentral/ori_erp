		...
		<div class="form-group row">
			<div class="col-sm-12">
				<div class="text-center" id="title_periode" style="margin-bottom:10px;">
					<strong>LAPORAN LEDGER IN TRANSIT</strong><br>
...
$(document).ready(function(){
	...
	$('#btn_tampilkan').on('click', function(){
		...
		$('#title_periode').html('<strong>LAPORAN LEDGER IN TRANSIT</strong><br><strong>Periode : '+Arr_Bulan[bulanInt]+' '+tahun+'</strong>');
		$('#tbody_ledger').html('<tr><td colspan="7" class="text-center"><i class="fa fa-spinner fa-spin"></i> Loading...</td></tr>');

		$.ajax({
			url: '<?php echo site_url("Ledger_intransit/get_data_json"); ?>',
			type: 'GET',
	...
	$('#btn_download_excel').on('click', function(){
		var bulan = $('#bulan').val();
		var tahun = $('#tahun').val();
		window.location.href = '<?php echo site_url("Ledger_intransit/excel_ledger_intransit"); ?>/'+bulan+'/'+tahun;
	});
