            ...
            <div class="box box-primary">
                <div class="box-body">
                    ...
                    <table class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                ...
                                <th class='text-center'>Jadwal Pembayaran</th>
                        ...
                        <?php $total=0; $idd=0; $total_pembayaran = 0;
                        if(!empty($data_detail)){
                            foreach($data_detail AS $record){ 
                                $total += $record->nilai_bayar;
                                $PEMBAYARAN = $record->nilai_bayar + $record->ppn - $record->pph;
                                $total_pembayaran += $PEMBAYARAN;
                                ?>
                            <tr id='trd<?=$idd?>' class='delAll'>
                                <td>
                                    <input type="hidden" name="detail_id[]" id="raw_id_<?=$idd?>" value="<?=$record->id;?>"><?=($idd+1);?>
                                    <input type="hidden" id="uniq" value="<?=$idd?>">
                                ...
                                <td><input type="text" class="form-control text-center" name="tanggal[]" id="tanggal_<?=$idd;?>" value="<?=$record->tanggal;?>" readonly tabindex="-1"></td>
                                <td align='center'><?=date('d-M-Y',strtotime($record->tgl_bayar));?></td>
                                <td><input type="text" class="form-control text-right divide" name="nilai[]" id="nilai_<?=$idd;?>" value="<?=($record->nilai);?>" readonly tabindex="-1"></td>
                                ...
                                <td><input type="text" class="form-control text-right divide changeSum subtotal" data-uniq='<?=$idd;?>' name="nilai_bayar[]" id="nilai_bayar_<?=$idd;?>" value="<?=($record->nilai_bayar);?>" onkeyup="cektotal(<?=$idd;?>)"></td>
                                <td><input type="text" class="form-control text-right divide changeSum subppn" data-uniq='<?=$idd;?>' name="ppn[]" id="ppn_<?=$idd;?>" value="<?=$record->ppn?>" onkeyup="cektotal(<?=$idd;?>)"></td>
                                <td><input type="text" class="form-control text-right divide changeSum subpph" data-uniq='<?=$idd;?>' name="pph[]" id="pph_<?=$idd;?>" value="<?=$record->pph?>" onkeyup="cektotal(<?=$idd;?>)"></td>
                                <td><input type="text" class="form-control text-right divide" name="pembayaran[]" id="pembayaran_<?=$idd;?>" value="<?=$PEMBAYARAN?>" readonly></td>
                            </tr>
                            <?php
                                $idd++;
                            }
                        ...
                        <tfoot>
                            <tr>
                                <th class='text-right' colspan="7">TOTAL AKTUAL BAYAR</th>
                                <th><input type="text" class="form-control text-right divide" name="nilai_total_aktual" id="nilai_total_aktual" value="<?=($total);?>" readonly tabindex="-1"></th>
                                <th class='text-right' colspan="2">TOTAL PEMBAYARAN</th>
                                <th><input type="text" class="form-control text-right divide" name="nilai_total" id="nilai_total" value="<?=($total_pembayaran);?>" readonly tabindex="-1"></th>
                            </tr>
                        </tfoot>
                    </table>
                    ...
                    <div class="form-group ">
                        ...
                        <div class="col-sm-4 hidden">
                            <input type="text" class="form-control divide" id="nilai_pph" name="nilai_pph" value="<?php echo (isset($data->nilai_pph) ? $data->nilai_pph:0 ); ?>" readonly>
                        </div>
                    </div>
                ...
                <div class="box-footer">
                    <div class="form-group">
                        <div class="col-sm-offset-2 col-sm-10">
                            <?php
                            if(isset($data) AND empty($tanda)){
                                // if($data->status==1){
                                    echo '<button type="submit" name="save" class="btn btn-success btn-md" id="submit"><i class="fa fa-save">&nbsp;</i>Payment</button>';
                                // }
                            }
                            ?>
                            <a class="btn btn-danger btn-md" href='<?=base_url('pembayaran_periodik');?>'>Back</a>
                        </div>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
	var url_save = base_url+'pembayaran_periodik/save_data/';
	var url_approve = base_url+'pembayaran_periodik/approve/';
	var nomor=<?=$idcoa?>;
	<?php
	if($type=='view'){
	...
	$('.divide').autoNumeric('init', {mDec: '0', aPad: false});
...
//			data_save();
	...
	function cektotal(id){
        ...
        $('.changeSum').each(function(){
            uniq  = $(this).parent().parent().find('#uniq').val()
            // console.log($(this).parent().parent().find('#nilai_bayar_'+uniq).val());
            aktual  = Number($(this).parent().parent().find('#nilai_bayar_'+uniq).val().split(",").join(""))
            ppn     = Number($(this).parent().parent().find('#ppn_'+uniq).val().split(",").join(""))
            pph     = Number($(this).parent().parent().find('#pph_'+uniq).val().split(",").join(""))
            jumlah  = aktual + ppn - pph
            $(this).parent().parent().find('#pembayaran_'+uniq).val(number_format(jumlah))
            // nilai_aktual += aktual
        })
        // return false
		...
		$('.subtotal').each(function() {
            ...
            nilai_aktual += Number($(this).val().split(",").join(""));
		});
		$('.subppn').each(function() {
			sum += Number($(this).val().split(",").join(""));
			nilai_ppn += Number($(this).val().split(",").join(""));
		});
		$('.subpph').each(function() {
			sum -= Number($(this).val().split(",").join(""));
			nilai_pph += Number($(this).val().split(",").join(""));
		});
		$("#nilai_total_aktual").val(number_format(nilai_aktual));
		$("#nilai_ppn").val(number_format(nilai_ppn));
		$("#nilai_pph").val(number_format(nilai_pph));
        $("#nilai_total").val(number_format(sum));
	}
