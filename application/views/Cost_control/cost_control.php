...
<div class="box box-primary">
	<div class="box-header">
		...
		<div class="box-tool pull-right">
			<a href="<?php echo site_url('cost_control/excel_report_finish') ?>" target='_blank' class="btn btn-sm btn-primary" id='btn-add' style='float:right;margin-left:5px;'>
				<i class="fa fa-file-excel-o"></i> Download Excel
			</a>
			<a href="<?php echo site_url('cost_control/print_hasil_finish_project') ?>" target='_blank' class="btn btn-sm btn-success" id='btn-add' style='float:right;'>
		...
		<div class="box-tool pull-right">
			...
			<select id='status' name='status' class='form-control input-sm' style='min-width:200px;'>
				...
				<option value='FINISH' <?=$sel2;?>>FINISH SESUAI STANDARD</option>
				<option value='FINISH 2' <?=$sel3;?>>FINISH DIBAWAH STANDARD</option>
			</select>
		</div>
		...
		<table class="table table-bordered table-striped" id="my-grid" width='100%'>
			<thead> 
				<tr class='bg-blue'>  
					...
					<th width='5%' class="text-center">SO NUMBER</th> 
					<th width='5%' class="text-center">Selesai Produksi</th> 					
					...
					<th width='8%' class="text-center no-sort">Status</th>
...
<script>
	...
	$(document).on('click', '#detail_process_cost', function(e){
		...
		$.ajax({
			type:'POST',
			url: base_url+active_controller+'/modalDetailProcess/'+$(this).data('id_bq'),
	...
	$(document).on('click', '#detailBQ', function(e){
		...
		$.ajax({
			type:'POST',
			url: base_url+active_controller+'/modalDetailBQ/'+$(this).data('id_bq'),
			success:function(data){
				$("#ModalView").modal();
				$("#view").html(data);
	...
	$(document).on('click', '.view_total_cost', function(e){
		e.preventDefault();
		loading_spinner();
	...
	$(document).on('click', '#detailPlant', function(e){
		...
		$.ajax({
			type:'POST',
			url: base_url+active_controller+'/modalDetailPrice/'+$(this).data('id_produksi'),
			success:function(data){
				$("#ModalView").modal();
				$("#view").html(data);
	...
	$(document).on('click', '.MatDetail', function(e){
		...
		$.ajax({
			type:'POST',
			url: base_url+active_controller+'/modalDetailMat/'+$(this).data('id_product')+'/'+$(this).data('id_milik')+'/'+$(this).data('qty')+'/'+$(this).data('length'),
			success:function(data){
				$("#ModalView2").modal();
				$("#view2").html(data);
	...
	$(document).on('click', '.MatDetailCost', function(e){
		...
		$.ajax({
			type:'POST',
			url: base_url+active_controller+'/modal_detail_cost/'+$(this).data('id_product')+'/'+$(this).data('id_milik')+'/'+$(this).data('ipp')+'/'+$(this).data('qty'),
			success:function(data){
				$("#ModalView2").modal();
				$("#view2").html(data);
	...
	$(document).on('click', '#update_cost', function(){
		swal({
		  title: "Update Cost Control Finish ?",
		...
		function(isConfirm) {
			if (isConfirm) {
				...
				$('#spinnerx').show();
				$.ajax({
					url			: base_url+active_controller+'/insert_select_finish',
					type		: "POST",
					cache		: false,
					dataType	: 'json',
					processData	: false, 
					...
					success		: function(data){								
						if(data.status == 1){											
							...
							$('#spinnerx').hide();
							window.location.href = base_url + active_controller + '/cost_control';
						}
	...
	function DataTables(status = null){
		...
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/server_side_cost_control/cost_control',
				type: "post",
				data: function(d){
					d.status = status
				},
