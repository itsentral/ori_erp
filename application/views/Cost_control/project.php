	...
	<div class="box-header">
		...
		<div class="box-tool pull-right">
			...
			<select id='status' name='status' class='form-control input-sm' style='min-width:200px;'>
				...
				<option value='PROCESS PRODUCTION'>PROCESS PRODUCTION</option>
		...
		<table class="table table-bordered table-striped" id="my-grid" width='100%'>
			<thead>
				<tr class='bg-blue'>
					...
					<th width='12%' class="text-center no-sort">Total Material(Est)</th>
					<th width='12%' class="text-center no-sort">Total Cost(Est)</th>
					<th width='12%' class="text-center no-sort">Process Cost</th>
					<th width='6%' class="text-center no-sort">Rev</th>
					<th width='10%' class="text-center no-sort">Status</th>
					<th width='12%' class="text-center no-sort">Option</th>
				</tr>
			</thead>
			<tbody></tbody>
		</table>
...
<script>
	...
	$(document).on('click', '#detail_process_cost2', function(e){
		...
		loading_spinner();
		$("#head_title3").html("<b>DETAIL COST PROCESS BQ ["+$(this).data('id_bq')+"]</b>");
		$("#view3").load(base_url +'index.php/'+ active_controller+'/modalDetailProcess/'+$(this).data('id_bq')+'/'+$(this).data('id_product')+'/'+$(this).data('id'));
		$("#ModalView3").modal();
	});
	...
	function DataTables(status = null){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url +'index.php/'+active_controller+'/getDataJSON/project',
				type: "post",
				data: function(d){
					d.status = status
				},
