...
<script>
	...
	$(document).on('click', '#checkMaterial', function(){
		...
		function(isConfirm) {
			if (isConfirm) {
				...
				$.ajax({
					url			: base_url + active_controller+'/process_checking',
					type		: "POST",
					data		: formData,
					cache		: false,
					dataType	: 'json',
					...
					success		: function(data){								
						if(data.status == 1){											
							...
							window.location.href = base_url + active_controller+'/checking';
						}
	...
	function DataTables(no_po=null){
		var dataTable = $('#example1').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/server_side_checking',
				type: "post",
				data: function(d){
					d.no_po = no_po
				},
