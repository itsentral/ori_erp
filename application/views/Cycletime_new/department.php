...
<script>
	...
	$(document).on('click', '#add', function(e){
		...
		$("#view").load(base_url +'index.php/'+ active_controller+'/add_department');
		$("#ModalView").modal();
	});
	...
	$(document).on('click', '.edit', function(e){
		...
		$("#view").load(base_url +'index.php/'+ active_controller+'/add_department/'+id);
		$("#ModalView").modal();
	});

	$(document).on('click', '#save', function(){
		var nm_dept	= $("#nm_dept").val();
		...
		function(isConfirm) {
			if (isConfirm) {
				...
				var baseurl		= base_url + active_controller +'/add_department';
				$.ajax({
					url			: baseurl,
					type		: "POST",
					data		: formData,
					...
					success		: function(data){
						if(data.status == 1){
							...
							window.location.href = base_url + active_controller+'/department';
						}
	...
	function DataTables(){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url +'index.php/'+active_controller+'/data_side_department',
				type: "post",
				data: function(d){
