...
<body>
<?php
$header	= $this->db->query("SELECT * FROM tr_request_mutasi_admin WHERE kd_mutasi ='$kodebayar'")->row();
$matauang	= $this->db->query("SELECT * FROM currency WHERE kode ='$header->mata_uang'")->row();

	    $jenis = $header->jenis_transaksi;
		$transaksi = $header->transaksi;
		$jurnal1 = $header->jurnal1;
		$jurnal2 = $header->jurnal2;
		if($jenis =='keluar') {
		$judul1          ='BUKTI UANG KELUAR';
		}else{			
		$judul1          ='BUKTI UANG MASUK';	
		}
?>
<table width=800>
...
<tr><td><center><div style="font-size:18px;"><u><?= $judul1 ?> <br> <?= $jurnal1 ?> <br> <?= $jurnal2 ?> </u></div></center></td></tr>
    ...
    <br>
		...
		<tbody>
			<tr>
				...
				<td class="text-center"><?=number_format($transaksi,2)?></td>
			...
			<tr>
				...
				<td class="text-center">0.00</td>
				<td class="text-center"><?=number_format($transaksi,2)?></td>
				<td class="text-center"><?=$header->mata_uang.' '.$header->nilai.'x'.number_format($header->kurs,2)?></td>
				</td>
			</tr>
		</tbody>
