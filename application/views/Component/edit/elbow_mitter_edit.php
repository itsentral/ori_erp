	...
	<div class="box box-primary">
		...
		<div class="box-body">
			...
			<div class='form-group row'>
				...
				<label class='label-control col-sm-2'><b>Length Mitter <span class='text-red'>*</span></b></label>
				<div class='col-sm-2'>              
					<?php
						echo form_input(array('id'=>'panjang','name'=>'panjang','class'=>'form-control input-sm numberOnly','autocomplete'=>'off','placeholder'=>'Angle', 'readonly'=>'readonly'), floatval($header[0]->panjang));											
					?>	
				</div>
			</div>
			<!-- ====================================================================================================== -->
...
<script>	
	$(document).ready(function(){
		...
		// var pipeD 	= parseFloat($('#diameter').val());
		// var parent_product		= $('#parent_product').val();
		...
		// $.ajax({
			...
			// cache: false,
			// type: "POST",
			// data: "dim="+pipeD+"&std="+$('#standart_codex').val()+"&parent_product="+parent_product,
			// dataType: "json",
			// success: function(data){
				// $("#tamp").show();
				...
				// if(data.StNum == 1){
					// $.ajax({
						...
						// cache: false,
						// type: "POST",
						// data: "dim="+data.StDim+"&parent_product="+data.PrPdc+"&std="+data.Standart,
						// dataType: "json",
						// success: function(data){ 
							// $('#waste').val(data.waste);
							// $('#max_toleran').val(data.maxx);
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl		= base_url +'edit_standart/elbow_mitter_edit';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
