	...
	<div class="box box-primary">
		...
		<div class="box-body">
			...
			<div class='form-group row'>		 	 
				...
				<div class='col-sm-4'>
					<?php
						$idP	= substr($header[0]->id_product, 19,1);
							$exM = '';
							$exQ = '';
							$exA = '';
							$exL = '';
					...
					<select name='pipe_for' id='pipe_for' class='form-control input-sm'>
						<option value='M' <?= $exM;?>>ELBOW MITTER</option>
						<option value='Q' <?= $exQ;?>>EQUAL TEE SLONGSONG</option>
						<option value='A' <?= $exA;?>>FLANGE SLONGSONG</option>
						<option value='L' <?= $exL;?>>REDUCER TEE SLONGSONG</option>
					</select>
				</div>
			</div>
			<div class='form-group row'>		 	 
...
<script src="<?php echo base_url('application/views/Component/edit/pipe_slongsong_edit/change_all.js'); ?>"></script>
<script src="<?php echo base_url('application/views/Component/edit/pipe_slongsong_edit/change_material.js'); ?>"></script>
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl		= base_url + active_controller +'/pipe_slongsong_edit';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
