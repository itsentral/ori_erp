...
$(document).on('keyup', '.layer', function(){
	...
	var nomor 			= $(this).data('nomor');
	var nomorPlus		= getNum(nomor) + 1;
	var layer			= getNum($(this).val());
	var thick_hide		= getNum($(this).parent().parent().find("td:nth-child(2) #thickness_"+nomor).val());
	...
	var containing		= getNum($('#containing_'+nomorPlus).val());
...
$(document).on('keyup', '.perse', function(){
	var TotResin	= getNum($('#last_cost_10').val());
	
	var perse		= getNum($(this).val() / 100);
	var containing	= getNum($(this).parent().parent().find("td:nth-child(3) input").val());
...
$(document).on('keyup', '.layerStr', function(){
	...
	var nomorPlus		= getNum(nomor) + 1;
	var layer			= getNum($(this).val());
	var thick_hide		= getNum($(this).parent().parent().find("td:nth-child(2) #thicknessStr_"+nomor).val());
	var BW				= getNum($(this).parent().parent().find("td:nth-child(1) #bwStr_"+nomor).val());
	var Jumlah			= getNum($(this).parent().parent().find("td:nth-child(1) #jumlahStr_"+nomor).val());
	...
	// alert(thick_hide);
	var containing		= getNum($('#containingStr_'+nomorPlus).val());
...
$(document).on('keyup', '.perseStr', function(){
	var TotResin	= getNum($('#last_costStr_9').val());
...
$(document).on('keyup', '.layerEks', function(){
	...
	var nomorPlus		= getNum(nomor) + 1;
	var layer			= getNum($(this).val());
	var thick_hide		= getNum($(this).parent().parent().find("td:nth-child(2) #thicknessEks_"+nomor).val());
	var tot_thickness	= $(this).parent().parent().find("td:nth-child(6) input");
	var lastWeight		= $(this).parent().parent().find("td:nth-child(7) input");
	var HslTotThick		= layer * thick_hide;
	
	var containing		= getNum($('#containingEks_'+nomorPlus).val());
...
$(document).on('keyup', '.perseEks', function(){
	var TotResin	= getNum($('#last_costEks_9').val());
...
$(document).on('keyup', '.perseTc', function(){
	var LastCoat	= getNum($('#last_topcoat_1').val());
