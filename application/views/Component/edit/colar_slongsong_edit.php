		...
		$(document).on('change', '#standart_code', function(e){
			...
			$.ajax({
				...
				success: function(data){
					...
					$('#bwStrN1_9').val(data.str_faktor_rv_bw);
					$('#jumlahStN1r_9').val(data.str_faktor_rv_jb);

					$('#bwStrN1_11').val(data.str_faktor_rv_add_bw);
					$('#jumlahStN1r_11').val(data.str_faktor_rv_add_jb);
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl		= base_url +'edit_standart/colar_slongsong_edit';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
