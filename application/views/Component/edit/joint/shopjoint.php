		...
		<div class="box-body">
			...
			<div class='form-group row'>
				...
				<div class='col-sm-4'>
					...
					?>
					<select name='d1' id='d1' class='form-control input-sm' readonly>
				...
				<div class='col-sm-4'>
					<select name='cust' id='cust' class='form-control input-sm'>
						...
						foreach($standard AS $val => $valx){
							...
							echo "<option value='".$valx['id_customer']."' ".$active12.">".strtoupper($valx['nm_customer'])."</option>";
						}
						?>
					</select>
				</div>
			...
			<div class='form-group row'>
				...
				<div class='col-sm-2'>
					<?php
						echo form_input(array('type'=>'text','id'=>'pipe_thickness','name'=>'pipe_thickness','class'=>'form-control input-sm numberOnly','placeholder'=>'Pipe Thickness'),floatval($component_header->pipe_thickness));
					?>
				</div>
			...
			<div class='form-group row'>
				...
				<div class='col-sm-2'>
					<?php
						echo form_input(array('type'=>'text','id'=>'joint_thickness','name'=>'joint_thickness','class'=>'form-control input-sm','readonly'=>'readonly'),floatval($component_header->joint_thickness));
					?>
				</div>
				<label class='label-control col-sm-2'><b>Factor Thickness <span class='text-red'>*</span></b></label>
				<div class='col-sm-2'>
					<?php
						echo form_input(array('type'=>'text','id'=>'factor_thickness','name'=>'factor_thickness','class'=>'form-control input-sm'),floatval($component_header->factor_thickness));
					?>
				</div>
			</div>
				...
				<tbody>
					<tr>
						...
						<td>
							<?php
								$veil_res = 9/1;
								echo form_input(array('id'=>'veil_resin_content','name'=>'glass[resin_content][]','class'=>'form-control input-sm numberOnly','placeholder'=>'Resin Content'),floatval($getV->resin_content));
							?>
						</td>

						<td>
							<?php
								echo form_input(array('id'=>'veil_thickness','name'=>'glass[thickness][]','class'=>'form-control input-sm numberOnly','readonly'=>'readonly','placeholder'=>'Thickness'), number_format($getV->thickness,4));
							?>
						</td>

						<td>
							<?php
								echo form_input(array('id'=>'veil_material_weight','name'=>'glass[material_weight][]','class'=>'form-control input-sm numberOnly','readonly'=>'readonly','placeholder'=>'Material Weight'),number_format($getV->material_weight,4));
							?>
					...
					<tr>
						...
						<td>
							<?php
								$wr_res = 45/55;
								echo form_input(array('id'=>'wr_resin_content','name'=>'glass[resin_content][]','class'=>'form-control input-sm numberOnly','placeholder'=>'Resin Content','value'=>floatval($getW->resin_content)));
							?>
						</td>

						<td>
							<?php
								echo form_input(array('id'=>'wr_thickness','name'=>'glass[thickness][]','class'=>'form-control input-sm numberOnly','placeholder'=>'Thickness','readonly'=>'readonly','value'=>number_format($getW->thickness,4)));
							?>
						</td>

						<td>
							<?php
								echo form_input(array('id'=>'wr_material_weight','name'=>'glass[material_weight][]','class'=>'form-control input-sm numberOnly','placeholder'=>'Material Weight','readonly'=>'readonly','value'=>number_format($getW->material_weight,4)));
							?>
			...
			<table class="table table-striped table-bordered table-hover table-condensed" width="100%">
				<?php
					...
					foreach($component_add AS $val => $valx){
						$a++;
						echo "<tr>";
							echo "<td width='25%'>".$valx['nm_category']."</td>";
							echo "<td width='25%'>";
								// echo $this->master_model->get_select_detail($valx['id_category'],'ResinAdd['.$a.'][id_material]',$valx['nm_category'],$valx['id_material']);
								?>
								<select name="ResinAdd[<?=$a;?>][id_material]" id='id_material_<?=$a;?>' class='form-control input-sm'>
								<?php
									$ListAdd	= $this->db->query("SELECT * FROM raw_materials WHERE id_category='".$valx['id_category']."' ORDER BY nm_material ASC")->result_array();
									foreach($ListAdd AS $val2 => $valx2){
										$selx	= ($valx2['id_material'] == $valx['id_material'])?'selected':'';
										echo "<option value='".$valx2['id_material']."' ".$selx.">".$valx2['nm_material']."</option>";
									}
								 ?>
								</select>
								<?php
								// echo form_input(array('type'=>'hidden','id'=>'id_material_'.$a.'','name'=>'ResinAdd['.$a.'][id_material]'), $valx['id_material']);
								echo form_input(array('type'=>'hidden','id'=>'id_category_'.$a.'','name'=>'ResinAdd['.$a.'][id_category]'), $valx['id_category']);
								echo form_input(array('type'=>'hidden','id'=>'detail_name_'.$a.'','name'=>'ResinAdd['.$a.'][detail_name]'), $valx['detail_name']);
							echo "</td>";
							echo "<td width='25%'>".form_input(array('id'=>'perse','name'=>'ResinAdd['.$a.'][perse]','style'=>'text-align:center','class'=>'form-control input-sm numberOnly persen_resinadd', 'data-nomor'=>''.$a.'' ), floatval($valx['perse']))."</td>";
							echo "<td width='25%'>".form_input(array('id'=>'last_cost_'.$a.'','name'=>'ResinAdd['.$a.'][last_cost]','style'=>'text-align:right','class'=>'form-control input-sm numberOnly','readonly'=>'readonly'), $valx['last_cost'])."</td>";
						echo "</tr>";
					}
				?>
			</table>
