			...
			<div class='form-group row'>
				...
				<div class='col-sm-4'>
					<?php
						...
						//echo form_input(array('id'=>'d1','name'=>'d1','class'=>'form-control input-sm Hide','value'=>$component_header->diameter));
						echo form_input(array('id'=>'url_help','name'=>'url_help','class'=>'form-control input-sm Hide','value'=>$this->uri->segment(4)));
						
					?>
					<select name='d1_DIS' id='d1_DIS' class='form-control input-sm' disabled>
					...
					<?php for ($i=1; $i <= 20; $i++) {
								...
								foreach ($new as $key => $v) { $no++;$b++?>
									...
									<?php endif; ?>
										...
										<td >
											<input class="form-control input-sm titikAwal" type="text" id="o_width_<?=$no?>" name="o_width_<?=$no?>" value="">
										</td>
										...
										<td >
											<input class="form-control input-sm titikAwal" type="text" id="o_thickness1_<?=$no?>" name="o_thickness1_<?=$no?>" value="">
										</td>

										<td >
											<input class="form-control input-sm titikAwal" type="text" id="o_thickness2_<?=$no?>" name="o_thickness2_<?=$no?>" value="">
										</td>

										<?php	if ($no == 1) {?>
											<td rowspan="<?=count($new);?>" style="text-align:center;vertical-align:middle;border-bottom:2px solid #000" >
												<input class="form-control input-sm titikAwal" type="text" id="o_glasslength_<?=$i?>" name="o_glasslength_<?=$i?>" value="">
											</td>
										<?php	}elseif (($no % 7) == 0) {?>
											<td rowspan="<?=count($new);?>" style="text-align:center;vertical-align:middle;border-bottom:2px solid #000" >
												<input class="form-control input-sm titikAwal" type="text" id="o_glasslength_<?=$i?>" name="o_glasslength_<?=$i?>" value="">
										...
										<td >
											<input class="form-control input-sm titikAwal" type="text" id="o_veil_weight_<?=$no?>" name="o_veil_weight_<?=$no?>" value="">
										</td>

										<td >
											<input class="form-control input-sm titikAwal" type="text" id="o_csm_weight_<?=$no?>" name="o_csm_weight_<?=$no?>" value="">
										</td>

										<td >
											<input class="form-control input-sm titikAwal" type="text" id="o_wr_weight_<?=$no?>" name="o_wr_weight_<?=$no?>" value="">
										</td>
...
<script>
	...
	$(document).ready(function(){
		...
		$(document).on('click', '#calc', function(e){
			// document. getElementById("form_proses_bro").reset();
			$(".titikAwal").val(0);
			// return false;  
			std();
			OL_Function();
				...
				for (var s = 20; s >= 1; s--) {
					...
					for (var i = x; i >= 1; i--) {
						...
						if (i == x && s == 20) {
							...
							// console.log('#o_width_'+n+"____"+width);
//o width =============================================================================================>							
							...
							if 	(	($('#o_thickness1_'+parseInt(n+1)).val() == 0 && $('#o_glassconfiguration_'+n).val() == 'CSM')	||
										...
										($('#o_thickness1_'+parseInt(n+2)).val() == null && $('#o_glassconfiguration_'+parseInt(n+1)).val() == 'CSM')
											...
											var	width	=	ceilingxcl(kons().D4,50);
							}else if ( $('#o_width_'+getNum(n+1)).val() == null ) {
								var width = 0;
							}else if (kons().D4 < 601 || (kons().D4 - (Math.floor(kons().G4/5.5)*100))<550) {
								var width = $('#o_width_'+getNum(n+1)).val() - 50;
							}else {
								var width = $('#o_width_'+getNum(n+1)).val() - 100;
							}
							// console.log('#o_width_'+n+"____"+width);
//o width =============================================================================================>
							$('#o_width_'+n).val(width);
									...
									if (i%2 == 0) {
										if 	(	(($('#o_thickness1_'+(parseInt(n)+1)).val() == null || $('#o_thickness1_'+(parseInt(n)+1)).val() == 0) && $('#o_glassconfiguration_'+n).val() == 'CSM')	||
													(($('#o_thickness1_'+(parseInt(n)+2)).val() == null	|| $('#o_thickness1_'+(parseInt(n)+2)).val() == 0) && $('#o_glassconfiguration_'+(parseInt(n+1))).val() == 'CSM')
														...
														var	width	=	ceilingxcl(kons().D4,50);
										}else if (($('#o_width_'+(getNum(n)+1)).val() == null || $('#o_width_'+(getNum(n)+1)).val() == 0) || getNum($('#o_width_'+(getNum(n)+1)).val()) < 1) {
											var width = 0;
										}else {
											var width = getNum($('#o_width_'+(getNum(n)+1)).val()) - 50;
										}
										// console.log('#o_width_'+n+"____"+width);
//o width =============================================================================================>
										$('#o_width_'+n).val(width);
										...
										if 	(	(($('#o_thickness1_'+(parseInt(n)+1)).val() == null || $('#o_thickness1_'+(parseInt(n)+1)).val() == 0) && $('#o_glassconfiguration_'+n).val() == 'CSM')	||
													(($('#o_thickness1_'+(parseInt(n)+2)).val() == null	|| $('#o_thickness1_'+(parseInt(n)+2)).val() == 0) && $('#o_glassconfiguration_'+(parseInt(n+1))).val() == 'CSM')
														...
														var	width	=	ceilingxcl(kons().D4,50);
															...
															($('#o_width_'+(getNum(n)+1)).val() == null || $('#o_width_'+(getNum(n)+1)).val() == 0) ||
															getNum($('#o_width_'+(getNum(n)+1)).val()) < 1) {
											var width = 0;
										}else {
											var width = getNum($('#o_width_'+(getNum(n)+2)).val());
										}
										// console.log('#o_width_'+n+"____"+width);
//o width =============================================================================================>
										$('#o_width_'+n).val(width);
