	...
	<div class="box box-primary">
		...
		<div class="box-body">
					...
					<select name='top_typeList' id='top_typeList' class='form-control input-sm' disabled>
						...
						<?php
							foreach($product AS $val => $valx){
								$selx	= ($header[0]->diameter == $valx['value_d'] AND $header[0]->diameter2 == $valx['value_d2'])?'selected':'';
			...
			<div class='form-group row'>		 	 
				...
				<div class='col-sm-1'>              
					<?php
						echo form_input(array('type'=>'text','id'=>'panjang','name'=>'panjang','class'=>'form-control input-sm numberOnly','placeholder'=>'Length','readonly'=>'readonly'), floatval($header[0]->panjang));	
					?>	
				</div>
				<div class='col-sm-1'>              
					<?php
...
<script src="<?php echo base_url('application/views/Component/edit/javascript/con_ecc_reducer.js'); ?>"></script>
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl		= base_url +'edit_standart/concentric_reducer_edit';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
