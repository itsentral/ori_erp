...
<form action="#" method="POST" id="form_proses_bro">
	<div class="box box-primary">
		...
		<div class="box-body">
			...
			<!-- ================== ADD =============== -->
			<!--
			<table id="my-grid_glass" class="table table-striped table-bordered table-hover table-condensed" width="100%">
				<tbody id='detail_body_glass'>
				</tbody>
			</table>
			<br>
			<button type='button' name='add_glass' id='add_glass' class='btn btn-success btn-sm' style='width:100px; margin-left: 10px;'>Add Material</button>
			<input type='hidden' name='numberMax_glass' id='numberMax_glass' value='0'>&nbsp;
			<label style='color:red;'><b>Add Material Development Process, will not be saved</b></label>
			-->
			<!-- ================== END =============== -->
	...
	$(document).ready(function(){
		...
		//=====================================OUTSIDE LAMINATION=======================================//
			...
			function OL_Function(){ // STD
				...
				var maxGlass = $("#numberMax_glass").val();
				var aw;
				for(aw=1; aw <= maxGlass; aw++){
					var tipex = $("#id_category_topcoat_"+aw).val();
					if(tipex == 'TYP-0004'){
						var jumx = csm_wg;
					}
					if(tipex == 'TYP-0003'){
					...
					$("#last_glass_"+aw).val(jumx.toFixed(4));
					// console.log(tipex);
					// console.log(aw);
					// console.log(jumx);
				}
	...
	swal({
		...
		function(isConfirm) {
			if (isConfirm) {
				...
				var baseurl		= base_url + active_controller +'/shopjoint';
				$.ajax({
					url			: baseurl,
					type		: "POST",
					data		: formData,
