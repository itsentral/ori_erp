		...
		<div class="box-body">
			...
			<div class='form-group row'>		 	 
				...
				<div class='col-sm-3'>              
					...
					Bw<input type='text' name='ListDetail2[10][bw]' class='form-control input-sm' style='width:100px; margin-bottom: 5px;'  id='bw_rooving22' value='0'>	
					Benang<input type='text' name='ListDetail2[10][jumlah]' class='form-control input-sm'  style='width:100px' id='jumlah_rooving22' value='0'>	
				</div>
				<label class='col-sm-2' style='text-align:left; margin-top: 6px;'><u>Rs.Containing</u></label>
				<div class='col-sm-1'>              
...
<script>
	...
	$(document).ready(function(){	
		...
		$(document).on('keyup', '#bw_rooving22', function(){
			
			var bw_rooving22		= parseFloat($('#bw_rooving22').val());
			var jumlah_rooving22	= parseFloat($('#jumlah_rooving22').val());
			
			getBw(bw_rooving22, jumlah_rooving22);
		});
		
		$(document).on('keyup', '#jumlah_rooving22', function(){
		...
		$(document).on('change', '#series', function(){
			...
			if(isNaN(struktur)){ var struktur=0;}
		...
		$(document).on('keyup', '#acuhan_3', function(){
			...
			var external			= parseFloat($('#acuhan_3').val());
			var MinToleransi		= parseFloat($('#top_min_toleran').val());
			var MaxToleransi		= parseFloat($('#top_max_toleran').val());
			var tot_lin_thickness	= parseFloat($('#tot_lin_thickness').val());
			var tot_lin_thickness2	= parseFloat($('#tot_lin_thickness2').val());
			var tot_lin_thickness3	= parseFloat($('#tot_lin_thickness3').val());
		...
		// $(document).on('keyup', '#persen_additive', function(){
			...
			// var Hasil				= (parseFloat(nilai)/100) * parseFloat(hasil_resin_tot) * parseFloat(layer_additive);
		...
		$(document).on('click', '#simpan-bro', function(e){
			...
			var bw_rooving22 		= $('#bw_rooving22').val();
			var jumlah_rooving22	= $('#jumlah_rooving22').val();
			...
			if(bw_rooving22 == ''){
				swal({
				  title	: "Error Message!",
				  text	: 'Bandwidch Rooving Add Material [STRUKTURE THICKNESS] is Empty, please input first ...',
				  type	: "warning"
				});
				$('#simpan-bro').prop('disabled',false);
				return false;	
			...
			if(jumlah_rooving22 == ''){
				swal({
				  title	: "Error Message!",
				  text	: 'Benang Rooving Add Material [STRUKTURE THICKNESS] is Empty, please input first ...',
				  type	: "warning"
				});
				$('#simpan-bro').prop('disabled',false);
				return false;	
...
function Hasil(a, b, c, d, e, top_diameter, top_length, top_thickness, waste){
	...
	var Luas_Area_Rumus		= ((3.14/1000)*(top_diameter + topEST))*(top_length/1000)*(1+waste);
	...
	// var HasilAditive	= layer_additive * (persen_additive/100) * TotalResin;
	...
	// var persen_additive4 = parseFloat($("#persen_additive4").val());
	...
	// var HasilAddv42		= Addv4 * (persen_additive4/100) * HasilTopCoat;
...
function Hasil2(a, b, c, d, e, f, g, top_diameter, top_length, top_thickness, waste){
	...
	// var HasilAditive2	= layer_additive2 * (persen_additive2/100) * TotalResin2;
...
function getBw(bw_rooving22, jumlah_rooving22){
	var waste 			= parseFloat($('#waste').val());
	var top_length 		= parseFloat($('#top_length').val());
	var top_diameter 	= parseFloat($('#diameter').val());
	
	//Struktur Thickness
	var b			= ($('#layer_matcsm2').val() == '')?0:parseFloat($("#layer_matcsm2").val());
	var c			= ($('#layer_csm_add2').val() == '')?0:parseFloat($("#layer_csm_add2").val());
	var d			= ($('#layer_wr2').val() == '')?0:parseFloat($("#layer_wr2").val());
	var e			= ($('#layer_wr_add2').val() == '')?0:parseFloat($("#layer_wr_add2").val());
	var f			= ($('#layer_rooving21').val() == '')?0:parseFloat($("#layer_rooving21").val());
	var g			= ($('#layer_rooving22').val() == '')?0:parseFloat($("#layer_rooving22").val());
	var a			= parseFloat($('#tot_lin_thickness2').val());
	...
	var layer_resin21		= ($('#layer_resin21').val() == '')?0:parseFloat($('#layer_resin21').val());
	var layer_resin22		= ($('#layer_resin22').val() == '')?0:parseFloat($('#layer_resin22').val());
	var layer_resin23		= ($('#layer_resin23').val() == '')?0:parseFloat($('#layer_resin23').val());
	var layer_resin24		= ($('#layer_resin24').val() == '')?0:parseFloat($('#layer_resin24').val());
	var layer_resin25		= ($('#layer_resin25').val() == '')?0:parseFloat($('#layer_resin25').val());
	var layer_resin26		= ($('#layer_resin26').val() == '')?0:parseFloat($('#layer_resin26').val());
