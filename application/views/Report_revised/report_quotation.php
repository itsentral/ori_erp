...
<script>
	...
	$(document).on('click', '.detail_quo', function(e){
		...
		$("#view").load(base_url + active_controller+'/modal_detail_quotation/'+$(this).data('id_bq'));
		$("#ModalView").modal();
	});
	...
	$(document).on('click', '.detail_quotation', function(e){
		...
		$("#view2").load(base_url + active_controller+'/modal_detail_quotation_detail/'+$(this).data('id_bq')+'/'+$(this).data('rev'));
		$("#ModalView2").modal();
	});
	...
	function DataTables(){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/getDataJSON_quo', 
				type: "post",
				// data: function(d){
				// 	d.bulan = $('#bulan').val(),
                //     d.tahun = $('#tahun').val()
    ...
    function DataTables_quotation(id_bq=null){
		var dataTable = $('#my-grid2').DataTable({
			...
			"aLengthMenu": [[10, 20, 50, 100, 150], [10, 20, 50, 100, 150]],
			"ajax":{
				url : base_url + active_controller+'/getDataJSONDetail_quo', 
				type: "post",
				data: function(d){
					d.id_bq = $('#id_bq').val()
                    // d.tahun = $('#tahun').val()
	...
	function DataTables_quotation_detail(id_bq=null, rev=null){
		var dataTable = $('#my-grid3').DataTable({
			...
			"aLengthMenu": [[10, 20, 50, 100, 150], [10, 20, 50, 100, 150]],
			"ajax":{
				url : base_url + active_controller+'/getDataJSONDetail2_quo', 
				type: "post",
				data: function(d){
					d.id_bq = $('#id_bq').val(),
					d.rev = $('#rev').val()
