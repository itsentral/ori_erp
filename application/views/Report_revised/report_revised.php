...
<script>
	...
	$(document).on('click', '.detail_cos', function(e){
		...
		$("#view").load(base_url + active_controller+'/modalDetail/'+$(this).data('id_bq'));
		$("#ModalView").modal();
	});
	...
	$(document).on('click', '.detail_costing', function(e){
		...
		$("#view2").load(base_url + active_controller+'/modalDetail2/'+$(this).data('id_bq')+'/'+$(this).data('rev'));
		$("#ModalView2").modal();
	});
	...
	function DataTables_costing_detail(id_bq=null, rev=null){
		var dataTable = $('#my-grid3').DataTable({
			...
			"aLengthMenu": [[10, 20, 50, 100, 150], [10, 20, 50, 100, 150]],
			"ajax":{
				url : base_url +active_controller+'/getDataJSONDetail2', 
				type: "post",
				data: function(d){
					d.id_bq = $('#id_bq').val(),
					d.rev = $('#rev').val()
