		...
		<table class="table table-bordered table-striped" id="my-grid" width='100%'>
			<thead>
				<tr class='bg-blue'>
					...
					<th class="text-center" width='8%'>Max Rev</th>
...
<script>
	...
	$(document).on('click', '.detail', function(e){
		...
		$("#view").load(base_url + active_controller+'/modalDetail_costing/'+$(this).data('id_bq'));
		$("#ModalView").modal();
	});
	...
	$(document).on('click', '.detail_eng', function(e){
		...
		$("#view2").load(base_url + active_controller+'/modalDetail2_costing/'+$(this).data('id_bq')+'/'+$(this).data('rev'));
		$("#ModalView2").modal();
	});
	...
	function DataTables(){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/getDataJSON_costing', 
				type: "post",
				// data: function(d){
				// 	d.bulan = $('#bulan').val(),
                //     d.tahun = $('#tahun').val()
    ...
    function DataTables_engine(id_bq=null){
		var dataTable = $('#my-grid2').DataTable({
			...
			"aLengthMenu": [[10, 20, 50, 100, 150], [10, 20, 50, 100, 150]],
			"ajax":{
				url : base_url + active_controller+'/getDataJSONDetail_costing', 
				type: "post",
				data: function(d){
					d.id_bq = $('#id_bq').val()
                    // d.tahun = $('#tahun').val()
	...
	function DataTables_engine_detail(id_bq=null, rev=null){
		var dataTable = $('#my-grid3').DataTable({
			...
			"aLengthMenu": [[10, 20, 50, 100, 150], [10, 20, 50, 100, 150]],
			"ajax":{
				url : base_url + active_controller+'/getDataJSONDetail2_costing', 
				type: "post",
				data: function(d){
					d.id_bq = $('#id_bq').val(),
					d.rev = $('#rev').val()
