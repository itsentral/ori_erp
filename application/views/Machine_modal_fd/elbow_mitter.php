...
history('View edit product estimasi in final drawing elbow mitter : '.$id_bq.' / '.$id_milik.' / '.$idproduct);
// echo $idproduct;
...
$product			= $this->db->query("SELECT * FROM product WHERE parent_product='elbow mitter' AND deleted='N'")->result_array();
...
?>
<form action="#" method="POST" id="form_proses_elbowmitter" enctype="multipart/form-data"> 
<div class="box box-primary">
	<div class="box-body">	
		...
		<div class='form-group row'>
			...
			<label class='label-control col-sm-2'><b>Angle | Length Mitter <span class='text-red'>*</span></b></label>
			<div class='col-sm-2'>              
				<?php
					echo form_input(array('id'=>'angle','name'=>'angle','class'=>'form-control input-sm numberOnly','autocomplete'=>'off','placeholder'=>'Angle', 'maxlength'=>'4','readonly'=>'readonly'), floatval($header[0]->angle));										
				?>	
			</div>
			<div class='col-sm-2'>              
				<?php
					echo form_input(array('id'=>'panjang_m','name'=>'panjang','class'=>'form-control input-sm numberOnly','autocomplete'=>'off','placeholder'=>'Length Mitter','readonly'=>'readonly'), floatval($header[0]->panjang));											
				?>	
			</div>
		</div>
		<!-- ====================================================================================================== -->
		...
		<?php
			...
			echo "&nbsp;<button type='button' name='simpan-bro' style='min-width:100px; float: right; margin-right:10px;' id='simpan-bro-elbowmitter' class='btn btn-success'>Save</button>";
...
function LuasArea(){
	...
	var panjang	= getNum($('#panjang_m').val());
	// console.log(radius);
	var Luas_Area_Rumus		= ((3.14 * (diameter + (2 * topEST)) + overlap) * panjang / 1000000 * (1+waste));
...
function length_mitter(radius, angle){
	var tan_tan		= Math.tan(11.25 * 3.14 / 180);
	var hasil_tan   = (angle/90) * 8 * tan_tan * radius;
	// console.log(hasil_tan);
	$('#panjang_m').val(getNum(hasil_tan).toFixed(2));
}
...
$(document).on('keyup', '#angle', function(e){
	e.preventDefault();
	var radius	= parseFloat($('#radius').val());
	var angle = parseFloat($(this).val());
	
	length_mitter(radius, angle);
