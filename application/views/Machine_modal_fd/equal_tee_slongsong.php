...
history('View edit product estimasi in final drawing equal tee slongsong : '.$id_bq.' / '.$id_milik.' / '.$idproduct);
// echo $idproduct;
...
$product			= $this->db->query("SELECT * FROM product WHERE parent_product='equal tee slongsong' AND deleted='N'")->result_array();
...
?>
<form action="#" method="POST" id="form_proses_equalteeslongsong" enctype="multipart/form-data"> 
<div class="box box-primary">
	<div class="box-body">		
		...
		<div class='form-group row'>		 	 
			...
			<div class='col-sm-1'>              
				<?php 
					echo form_input(array('id'=>'panjang_m','name'=>'panjang','class'=>'form-control input-sm numberOnly','autocomplete'=>'off','placeholder'=>'Length','readonly'=>'readonly'), floatval($header[0]->panjang));				
				?>
			</div>
			<div class='col-sm-1'>              
				<?php 
		...
		<div class='form-group row'>
			...
			<div class='col-sm-2'>              
				<?php
					echo form_input(array('id'=>'wrap_length','name'=>'wrap_length','class'=>'form-control input-sm numberOnly','autocomplete'=>'off','placeholder'=>'Butt & Wrap Length'), floatval($header[0]->wrap_length));											
				?>	
			</div>
			<div class='col-sm-2'>              
				<?php
					echo form_input(array('id'=>'high','name'=>'high','class'=>'form-control input-sm numberOnly','placeholder'=>'High', 'readonly'=>'readonly'), floatval($header[0]->high));				
				?>	
			</div>	
		</div>
		<!-- ====================================================================================================== -->
		...
		<?php
			...
			echo "&nbsp;<button type='button' name='simpan-bro' style='min-width:100px; float: right; margin-right:10px;' id='simpan-bro-equalteeslongsong' class='btn btn-success'>Save</button>";
...
function LuasArea1(){
	...
	var panjang			= ceiling(wrap_length + (diameter + 2 * top_tebal_design) + 50, 10);
	var waste			= parseFloat($("#waste").val()) / 100;
	var top_tebal_est	= Estimasi();
	
	var LuasAreaOne		= 3.14 * (diameter + top_tebal_est)/1000 * (panjang / 1000) * (1+waste);
	
	if(isNaN(LuasAreaOne)){var LuasAreaOne = 0;}
	if(isNaN(panjang)){var panjang = 0;}
	$("#area").val(LuasAreaOne.toFixed(4));
	$("#panjang_m").val(Math.round(panjang / 10) * 10);
