...
$sqlBef = "SELECT * FROM so_detail_header in final drawing WHERE id_bq='".$id_bq."' AND id='".$id_milik."' LIMIT 1 ";
$restBef = $this->db->query($sqlBef)->result();
$idproduct = $restBef[0]->id_product;

history('View edit product estimasi reducer tee slongsong : '.$id_bq.' / '.$id_milik.' / '.$idproduct);
// echo $idproduct;
...
$product			= $this->db->query("SELECT * FROM product WHERE parent_product='reducer tee slongsong' AND deleted='N'")->result_array();
...
?>
<form action="#" method="POST" id="form_proses_bro_reducerteeslongsong" enctype="multipart/form-data"> 
<div class="box box-primary">
	<div class="box-body">	
		...
		<div class='form-group row'>		 	 
			...
			<div class='col-sm-4'>              
				<?php
					...
					echo form_input(array('id'=>'diameter2','name'=>'diameter2','class'=>'Hide','readonly'=>'readonly'),$header[0]->diameter2);
					...
					echo form_input(array('id'=>'help_url','name'=>'help_url','class'=>'Hide','readonly'=>'readonly'), $id_url); 
				?>
			</div>
		...
		<div class='form-group row'>		 	 
			...
			<label class='label-control col-sm-2'><b>Waste | Length | (Min Max)</b></label> 
			<div class='col-sm-1'>              
				<?php
					...
					echo form_input(array('type'=>'text','id'=>'area2','name'=>'area2','class'=>'HideCost'), $header[0]->area2);
			...
			<div class='col-sm-1'>              
				<?php 
					echo form_input(array('id'=>'panjang_m','name'=>'panjang','class'=>'form-control input-sm numberOnly','autocomplete'=>'off','placeholder'=>'Length'), floatval($header[0]->panjang));				
				?>
			</div>
			<div class='col-sm-1'>              
				<?php 
		...
		<div class='form-group row'>
			<label class='label-control col-sm-2'><b>Butt & Wrap Length 1 | 2 | High<span class='text-red'>*</span></b></label>
			<div class='col-sm-1'>              
				<?php
					echo form_input(array('id'=>'wrap_length','name'=>'wrap_length','class'=>'form-control input-sm numberOnly','autocomplete'=>'off','placeholder'=>'B&W 1'), floatval($header[0]->wrap_length));											
				?>	
			</div>
			<div class='col-sm-1'>              
				<?php
					echo form_input(array('id'=>'wrap_length2','name'=>'wrap_length2','class'=>'form-control input-sm numberOnly','autocomplete'=>'off','placeholder'=>'B&W 2'), floatval($header[0]->wrap_length2));											
				?>	
			</div>
			<div class='col-sm-2'>              
				<?php
					echo form_input(array('id'=>'high','name'=>'high','class'=>'form-control input-sm numberOnly','placeholder'=>'High'), floatval($header[0]->high));				
				?>	
			</div>	
		</div>
		<!-- ====================================================================================================== -->
		...
		<?php
			echo "&nbsp;<button type='button' style='min-width:100px; float: right; margin-right:10px;' id='simpan-bro-reducerteeslongsong' class='btn btn-success'>Save</button>";
		?>
		<br>			
	</div>	
</div>
...
<script>	
	$(document).ready(function(){
		...
		$(document).on('keyup', '#wrap_length2', function(){
			High();
			ChangeLuasArea();
		});
		
		$(document).on('keyup', '#panjang_m', function(){
			ChangeLuasArea();
		});
		
		$(document).on('keyup', '#high', function(){
			ChangeLuasArea();
		});
...
function ceiling($number, $significance = 1){ 
	return ( parseFloat($number) && parseFloat($significance) ) ? (Math.ceil($number/$significance)*$significance) : false;
}
	...
	function Panjang(){
		...
		var top_length			= ceiling(wrap_length + (diameter2 + 2 * top_tebal_est) + 50, 10);
		
		if(isNaN(top_length)){var top_length = 0;}
		$("#panjang_m").val(Math.round(top_length / 10) * 10);
	}
	...
	function LuasArea1(){
		...
		var diameter2			= parseFloat($("#diameter2").val());
		var top_length			= parseFloat($("#panjang_m").val());
		// var top_length			= ceiling(wrap_length + (diameter2 + 2 * top_tebal_est) + 50, 10);
		...
		// $("#panjang").val(Math.round(top_length / 10) * 10);
		  
		return LuasAreaOne;
	}
	
	function High(){
		var wrap_length2		= parseFloat($("#wrap_length2").val());
		var diameter			= parseFloat($("#diameter").val());
		var top_tebal_est		= Estimasi();
		var high				= ceiling(0.5 * (wrap_length2 + diameter + 2 * top_tebal_est) + 25, 10);
	...
	function LuasArea2(){
		...
		var LuasAreaTwo			= 3.14 * (diameter2 + top_tebal_est)/1000 * (high / 1000) * (1+waste);
...
function ChangeLuasArea(){
	...
	var LuasArea1x = LuasArea1();
	var LuasArea2x = LuasArea2();
	
	var LuasAreaX		= LuasArea1x + LuasArea2x;
