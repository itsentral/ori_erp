...
history('View edit product estimasi in final drawing pipe : '.$id_bq.' / '.$id_milik.' / '.$idproduct);
// echo $idproduct;
//berdasarkan diamater
$header				= $this->db->query("SELECT a.* FROM so_component_header a WHERE a.id_bq='".$id_bq."' AND a.id_milik='".$id_milik."' AND a.id_product='".$idproduct."' LIMIT 1 ")->result();
...
?>
<form action="#" method="POST" id="form_proses_bro_pipe" enctype="multipart/form-data"> 
<div class="box box-primary">
	<div class="box-body">	
		<?php
		echo form_input(array('id'=>'id_product','name'=>'id_product','class'=>'form-control input-sm Hide','style'=>'width:40%;font-size:18px;background-color: #e8ea6a;font-weight: bold;','readonly'=>'readonly'),$header[0]->id_product);
		echo form_input(array('id'=>'parent_productx','name'=>'parent_productx','class'=>'form-control input-sm Hide','style'=>'width:40%;font-size:18px;background-color: #e8ea6a;font-weight: bold;','readonly'=>'readonly'),$header[0]->parent_product);
		...
		<div class='form-group row'>		 	 
			<label class='label-control col-sm-2'><b>Length Pipe<span class='text-red'>*</span></b></label>
			<div class='col-sm-4'>
				<?php
					echo form_input(array('type'=>'text','id'=>'lengthx','name'=>'lengthx','class'=>'form-control input-sm numberOnly','autocomplete'=>'off','placeholder'=>'Pipe Length','readonly'=>'readonly'), floatval($header[0]->panjang - 400));				
				?>
				<span style='color:red;'><b>* <u>Pastikan panjang pipa sesuai panjang di BQ/Spesifikasi</u></b></span>
			</div>
		</div>
		<!-- ====================================================================================================== -->
		<!-- ============================================LINER THICKNESS=========================================== -->
		...
		<?php
			...
			echo "&nbsp;<button type='button' style='min-width:100px; float: right; margin-right:10px;' id='simpan-bro-pipe' class='btn btn-success'>Save</button>";
...
$(document).on('keyup', '.perseEksC', function(){
	var TotResin	= parseFloat($('#last_costEks_9').val());
	
	var perse		= parseFloat($(this).val());
	var containing	= parseFloat(($(this).parent().parent().find("td:nth-child(4) input").val())/ 100);
	var lastWeight	= $(this).parent().parent().find("td:nth-child(5) input");
