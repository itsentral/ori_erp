...
<script>
	$(document).ready(function(){
		...
		$(document).on('click', '#inputReal1New', function(e){
			...
			loading_spinner();
			$("#head_title").html("<b>UPDATE REAL PRODUCTION FIRST ["+$(this).data('id_product')+"]</b>");
			$("#view").load(base_url +'index.php/'+ active_controller+'/modalReal1New/'+$(this).data('id_product')+'/'+$(this).data('id_produksi')+'/'+$(this).data('id_producktion')+'/'+$(this).data('id_milik'));
			$("#ModalView").modal(); 
		});
		...
		$(document).on('click', '#inputReal3New', function(e){
			...
			$("#view").load(base_url +'index.php/'+ active_controller+'/modalReal3New/'+$(this).data('id_product')+'/'+$(this).data('id_produksi')+'/'+$(this).data('id_producktion')+'/'+$(this).data('id_milik'));
			$("#ModalView").modal();
		});
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						loading_spinner();
						var formData 	=new FormData($('#form_proses_bro')[0]);
						var baseurl=base_url + active_controller +'/UpdateRealMat2New';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
							...
							success		: function(data){
								if(data.status == 1){
									...
									window.location.href = base_url + active_controller+"/updateRealNew/"+data.produksi;
								}
								else if(data.status == 2){
									swal({
									  title	: "Save Failed!",
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						loading_spinner();
						var formData 	=new FormData($('#form_proses_bro')[0]); 
						var baseurl=base_url + active_controller +'/UpdateRealMat1New';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
	...
	function DataTables(){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url +'index.php/'+active_controller+'/getDataJSONUP',
				type: "post",
				data: function(d){
