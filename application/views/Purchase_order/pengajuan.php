...
<script>
	...
	$(document).on('click', '.ajukan', function(e){
		e.preventDefault();
		loading_spinner();
		$("#head_title2").html("<b>PEMILIHAN SUPPLIER ["+$(this).data('no_rfq')+"]</b>");
		$.ajax({
			type:'POST',
			url: base_url + active_controller+'/modal_pemilihan/'+$(this).data('no_rfq')+'/'+$(this).data('status'),
			success:function(data){
				$("#ModalView2").modal();
				$("#view2").html(data);
	...
	$(document).on('click', '#saveAju', function(){
		...
		function(isConfirm) {
			if (isConfirm) {
				...
				$.ajax({
					url			: base_url + active_controller+'/modal_pemilihan',
					type		: "POST",
					data		: formData,
					cache		: false,
					dataType	: 'json',
