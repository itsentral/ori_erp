...
<?=form_open('purchase/jurnal_save',array('id'=>'frm_data','name'=>'frm_data','role'=>'form','class'=>'form-horizontal',));?>
...
<div class="tab-content">
	<div class="tab-pane active">
		...
		foreach($data AS $record){ 
			...
			$numb++;
			$total_debet=($total_debet+$record->debet);
			$total_kredit=($total_kredit+$record->kredit);
			?>
			<tr>
		...
		<?php }
		$link='';
		if($jenis_jurnal=='JV041' || $jenis_jurnal=='JV053') $link='jurnal_invoice';
		?>
			<tr>
				<td colspan=2></td>
				<td><input type="text" class="form-control divide" id="ttl_debet" name="ttl_debet" value="<?=$total_debet;?>" required></td>
				<td><input type="text" class="form-control divide" id="ttl_kredit" name="ttl_kredit" value="<?=$total_kredit;?>" required></td>
			</tr>
			</tbody>
			</table>
			</div>
			<div class="box-footer">
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-10">
						...
						<a href="<?=base_url("purchase/".$link)?>" class="btn btn-warning btn-sm"><i class="fa fa-reply">&nbsp;</i>Kembali</a>
					</div>
				</div>
			</div>
		</div>
