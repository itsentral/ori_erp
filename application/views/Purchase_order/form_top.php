<?php
if(!empty($data_rfq[0]->currency)){
	$kurs=$data_rfq[0]->currency;
}else{
}
$kurs=1;
?>
<div class="box-body"> 
	<div class='note hidden'>
		<p>
			...
			Kurs sesuai <?=(!empty($data_rfq[0]->no_rfq)?$data_rfq[0]->no_rfq:'');?>, yang diinput di table perbandingan<br>
			<span style='color:green;font-size: 18px;'><b>1 USD = <?=number_format($kurs,2);?> IDR</b></span><br>
		</p>
	</div>
	<br>
	...
	<div class='form-group row'>
		...
		<div class='col-sm-3'>
			<select id='current' name='current' class='form-control input-sm' readonly style="pointer-events: none;">
				<?php
				$kurs_mata_uang = (!empty($data[0]->mata_uang))?$data[0]->mata_uang:'';
			...
			<input type='hidden' id='kurs' value='<?=$kurs;?>'>
		</div>
	</div>
	<div class='form-group row'>
			...
			<table id="my-grid" class="table table-bordered table-condensed" width="100%">
				...
				<tbody>
					...
					foreach($result AS $val => $valx){ $no++;
						...
						echo "<tr>";
							echo "<td align='left'>".strtoupper($valx['nm_material'])."
							...
							echo "<td align='right'><input name='detail[".$no."][qty]' id='qty_".$no."' class='form-control text-right input-md  ch_qty' value='".number_format($qty_p)."' data-decimal='.' data-thousand='' data-precision='0' data-allow-zero='' readonly></td>";
			...
			<table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
				<thead id='head_table'>
					<tr class='bg-blue'>
						...
						<th class="text-center" width='11%'>DPP + PPN <!--(IDR)--></th>
						...
						<th class="text-center" width='10%'>Nilai Inv</th>
						<th class="text-center" width='10%'>PPN Inv</th>
					...
					if(!empty($data_top)){
						foreach($data_top AS $val => $valx){ $id++;
							...
							echo "<tr class='header_".$id."'>";
								...
								echo "<td align='left' class='hidden'><input type='text' id='usd_".$id."' name='detail_po[".$id."][value_usd]' value='".number_format($valx['value_usd'],2)."' class='form-control input-md text-right maskM sum_tot_usd' readonly tabindex='-1' ".$styledisabled."></td>";
								echo "<td align='left'><input type='text' id='idr_".$id."' name='detail_po[".$id."][value_idr]' value='".number_format($valx['value_idr'],2)."' class='form-control input-md text-right maskM sum_tot_idr' ".$styledisabled."></td>";
								...
								echo "<td align='left'><input type='text' name='detail_po[".$id."][no_invoice]' value='".$valx['invoice_no']."' class='form-control input-md text-center datepicker' readonly></td>";
								echo "<td align='left'><input type='text' name='detail_po[".$id."][nilai_invoice]' value='".$valx['invoice_total']."' class='form-control input-md text-center datepicker' readonly></td>";
								...
								if($styledisabled=="" && $valx['group_top']=='progress' && $data[0]->total_terima_barang_idr > 0 && $valx['invoice_no'] ==''){
								...
								}elseif($styledisabled=="" && $valx['group_top']=='uang muka' && $data[0]->total_terima_barang_idr < 1 && $valx['invoice_no'] ==''){
									
									
									echo '<br /><a class="receive btn btn-xs btn-success" href="'.base_url('purchase/invoice_receive/'.$valx['id']).'" title="Receive Invoice"> <i class="fa fa-newspaper-o"></i> Receive Invoice</a>';	
									
													
								}elseif($styledisabled=="" && $valx['group_top']=='progress' && $data[0]->total_terima_barang_idr > 0 && $valx['invoice_no'] !=''){
								   ...
								   echo '<br /><a class="create1 btn btn-xs btn-primary" href="'.base_url('purchase/request_payment/'.$valx['id']).'" title="Request Payment"> <i class="fa fa-money"></i> Request Payment</a>';
								    ...
								    echo '<br /><a class="receive btn btn-xs btn-success" href="'.base_url('purchase/invoice_receive/'.$valx['id']).'" title="Receive Invoice"> <i class="fa fa-newspaper-o"></i> View Receive Inv</a>';	
								}elseif($styledisabled !="" && $valx['group_top']=='uang muka'){
								    ...
								    echo '<br /><a class="receive btn btn-xs btn-success" href="'.base_url('purchase/invoice_receive/'.$valx['id']).'" title="Receive Invoice"> <i class="fa fa-newspaper-o"></i> View Receive Inv</a>';									
								}elseif($styledisabled !="" && $valx['group_top']=='progress' ){
									echo '<a class="btn btn-xs btn-default" href="'.base_url('purchase/print_request/'.$valx['id']).'" title="Print Request" target="_blank"> <i class="fa fa-print"></i> Print Request</a>';
									echo '<br /><a class="receive btn btn-xs btn-success" href="'.base_url('purchase/invoice_receive/'.$valx['id']).'" title="Receive Invoice"> <i class="fa fa-newspaper-o"></i> View Receive Inv</a>';				
								}
								echo "</td>";
