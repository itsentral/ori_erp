        ...
        <table class="table table-sm table-bordered table-striped" id="my-grid2" width='100%'>
            <thead>
                <tr class='bg-blue'>
                    ...
                    <th class="text-center no-sort">Qty WIP</th>
                    <th class="text-center no-sort">Qty Order</th>
                    <th class="text-center no-sort">Qty Pass</th>
...
<script>
	$(document).ready(function(){
		...
		$(document).on('click', '.look_history', function(e){
            ...
            $("#view").load(base_url + active_controller + '/detail_berat/'+$(this).data('kode_spk')+'/'+$(this).data('id_production_detail')+'/'+$(this).data('category')+'/'+$(this).data('qty')+'/'+status+'/'+$(this).data('length')+'/'+$(this).data('length_awal')+'/'+$(this).data('tot_product'));
            $("#ModalView").modal();
        });
		...
		$(document).on('click', '#download_excel', function(e){
			...
			var Links		= base_url + active_controller+'/ExcelWIPGudangWIP/'+date_filter_+'/'+status;
	...
	function DataTables2(status=null, date_filter=null){
		var dataTable = $('#my-grid2').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/server_side_wip',
				type: "post",
				data: function(d){
					d.status = status,
