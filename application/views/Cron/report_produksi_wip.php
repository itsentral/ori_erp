		...
		<table class="table table-bordered table-striped" id="my-grid">
			<thead>
				<tr class='bg-blue'>
					...
					<th class="text-center valignmid" rowspan='2'>Tanggal Close</th>
					...
					<th class="text-center no-sort valignmid" rowspan='2'>Aktual Price (Rp)</th>
					<th class="text-center no-sort valignmid" rowspan='2'>Kurs</th>
					<th class="text-center no-sort valignmid" rowspan='2'>Price Rp / Kurs ($)</th>
...
<script>
	...
	$(document).on('click', '#excel_report', function(e){
		...
		var Link	= base_url + active_controller +'/excel_project_wip/'+bulan+'/'+tahun;
		window.open(Link);
	});
	...
	$(document).on('click', '.detail', function(e){
		...
		$("#view").load(base_url + active_controller+'/modalDetail_WIP/'+$(this).data('tanggal'));
		$("#ModalView").modal();
	});
	...
	function DataTables(bulan = null, tahun = null){
		var dataTable = $('#my-grid').DataTable({ 
			...
			"ajax":{
				url : base_url + active_controller+'/getDataJSON_WIP', 
				type: "post",
				data: function(d){
					d.bulan = $('#bulan').val(),
                    d.tahun = $('#tahun').val()
