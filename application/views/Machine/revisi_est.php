...
<div class="box box-primary">
	<div class="box-header">
		...
		<div class="box-tool pull-right">	
			<!-- <a href="<?php echo site_url('machine/revisi_quo') ?>" class="btn btn-md btn-primary" id='btn-add'>
				Revised BQ 
			</a> -->
		</div>
	</div>
...
<!--
<script src="<?php echo base_url('application/views/Component/edit/pipe_edit/change_all.js'); ?>"></script>
<script src="<?php echo base_url('application/views/Component/edit/pipe_edit/change_material.js'); ?>"></script>
-->
...
<script>
	...
	// $('#ModalView3').on("hidden.bs.modal-dialog", function (e) { 
	...
	$(document).on('click', '#editBQ', function(e){
		...
		$("#view").load(base_url +'index.php/'+ active_controller+'/modalEstBQ/'+$(this).data('id_bq')+'/'+$(this).data('ciri'));
		$("#ModalView").modal();
	});
	...
	$(document).on('click', '#editX', function(e){
		...
		$("#view3").load(base_url +'index.php/'+ active_controller+'/modalEditX/'+$(this).data('id_bq')+'/'+$(this).data('id_milik')+'/'+$('#pembeda').val());
		$("#ModalView3").modal();
	});
	...
	$(document).on('click', '#ajuAppBQ', function(){
		...
		function(isConfirm) {
			if (isConfirm) {
				loading_spinner();
				$.ajax({
					url			: base_url+'index.php/'+active_controller+'/ajukanAppBQEst/'+bq,
					type		: "POST",
					cache		: false,
					dataType	: 'json',
					processData	: false, 
	...
	$(document).on('click', '#simpan-bro-custom', function(e){
		...
		if(top_tebal_design == '' || top_tebal_design == null || top_tebal_design == 0){
			swal({
			  title	: "Error Message!",
			  text	: 'Thickness custom Design is Empty, please input first ...',
			  type	: "warning"
			});
	...
	function DataTables(){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url +'index.php/'+active_controller+'/getDataJSONEstRev',
				type: "post",
				data: function(d){
