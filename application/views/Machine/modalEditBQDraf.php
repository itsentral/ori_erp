...
$qBQHeader		= "SELECT * FROM draf_bq_header WHERE id_bq = '".$id_bq."' ";
$restBQHeader	= $this->db->query($qBQHeader)->result_array();

$qBQdetHeader 	= "SELECT * FROM draf_bq_detail_header WHERE id_bq = '".$id_bq."' ORDER BY id ASC";
...
$ListSeries		= $this->db->query("SELECT * FROM draf_bq_detail_header WHERE id_bq = '".$id_bq."' GROUP BY series")->result_array();
	$dtListArray = array();
...
?>
		...
		<table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
			<thead id='head_table'>
				<tr class='bg-blue'>
					...
					<th class="text-center" style='vertical-align:middle;' width='6%'>Unit Dev</th>
				...
				<?php
					...
					foreach($qBQdetRest AS $val => $valx){
						...
						<tr id='tr_<?= $no;?>' <?= $plusStyle;?>>
							...
							<td align='center'><?= $valx['series']; ?><input type='hidden' name='DetailBq[<?= $no;?>][series]' value='<?= $valx['series'];?>'></td>
							<td align='center'><?= $valx['id_delivery']; ?><input type='hidden' name='DetailBq[<?= $no;?>][id_delivery]' value='<?= $valx['id_delivery'];?>'></td>
							<td align='center'><?= $valx['sub_delivery']; ?><input type='hidden' name='DetailBq[<?= $no;?>][sub_delivery]' value='<?= $valx['sub_delivery'];?>'>
							...
							<td><input type='text' name='DetailBq[<?= $no;?>][length]' id='length_<?= $no;?>' class='form-control numberOnly width_long' value='<?= floatval($valx['length']);?>' maxlength='7'></td>
							...
							<td><button type='button' id='del' class='btn btn-danger btn-sm' data-toggle='tooltip' data-placement='bottom' onClick='delRow(<?= $no;?>)'>Del</button></td>
						</tr>
						<?php
					}
				?>
	...
	<div class="box box-success">
		<div class="box-header">
			<h3 class="box-title"><b>NEW BQ</b></h3>
		</div>
	...
	<div class='box-footer'>
		<?php
		echo form_button(array('type'=>'button','class'=>'btn btn-md btn-success','value'=>'savedraf','content'=>'Save to Draf','id'=>'simpan-draf')).' ';
		echo form_button(array('type'=>'button','class'=>'btn btn-md btn-primary','value'=>'save','content'=>'Save BQ','id'=>'simpan-bro')).' ';
		?>
	</div>
<style>
	td{
...
<script>
	...
	for(a=1; a <= numRows; a++){
		...
		else if(KS_id_category == 'flange mould' || KS_id_category == 'flange slongsong' || KS_id_category == 'colar' || KS_id_category == 'colar slongsong'){
			$(KS_diameter_1).show();
			$(KS_diameter_2).hide();
			$(KS_length).hide();
			$(KS_thickness).show();
		...
		else if(KS_id_category == 'concentric reducer' || KS_id_category == 'eccentric reducer' || KS_id_category == 'joint puddle flange' || KS_id_category == 'figure 8' || KS_id_category == 'spacer ring'){
			$(KS_diameter_1).show();
			$(KS_diameter_2).show();
			$(KS_thickness).show();
			$(KS_length).hide();
		...
		else if(KS_id_category == 'reducer tee mould' || KS_id_category == 'reducer tee slongsong' || KS_id_category == 'rib' || KS_id_category == 'joint rib' || KS_id_category == 'plate' || KS_id_category == 'loose flange' || KS_id_category == 'spacer' || KS_id_category == 'blind spacer' ){
			$(KS_diameter_1).show();
			$(KS_diameter_2).show();
			$(KS_thickness).show();
			$(KS_length).hide();
		...
		else if(KS_id_category == 'shop joint' || KS_id_category == 'field joint' || KS_id_category == 'end cap'|| KS_id_category == 'blind flange with hole' || KS_id_category == 'spectacle blind' || KS_id_category == 'blank and spacer'){
			$(KS_diameter_1).show();
			$(KS_length).hide();
			$(KS_diameter_2).hide();
			$(KS_thickness).show();
	...
	$(document).on('change','.productXC', function(e){
		// e.preventDefault();
		var KS_id_category 	= $(this).val();
		var urut 			= $(this).data('urut');
		var KS_diameter_1 	= "#diameter_1_"+urut;
		var KS_diameter_2 	= "#diameter_2_"+urut;
		var KS_length 		= "#length_"+urut;
		var KS_thickness 	= "#thickness_"+urut;
		var KS_sudut 		= "#sudut_"+urut;
		var KS_id_standard 	= "#id_standard_"+urut;
		var KS_type 		= "#type_"+urut;
		var KS_qty 			= "#qty_"+urut;
		
		// alert(urut);
		// alert(KS_diameter_1);
		
		if(KS_id_category == 'pipe' || KS_id_category == 'saddle'){
			$(KS_diameter_1).show();
			$(KS_diameter_2).hide();
			$(KS_length).show();
			$(KS_thickness).show();
		...
		else if(KS_id_category == 'branch point'){
			$(KS_diameter_1).show();
			$(KS_length).hide();
			$(KS_diameter_2).show();
			$(KS_thickness).show();
		...
		else if(KS_id_category == 'shop join' || KS_id_category == 'field joint' || KS_id_category == 'end cap'|| KS_id_category == 'blind flange with hole' || KS_id_category == 'spectacle blind' || KS_id_category == 'blank and spacer'){
			$(KS_diameter_1).show();
			$(KS_length).hide();
			$(KS_diameter_2).hide();
			$(KS_thickness).show();
	...
	$('#simpan-draf').click(function(e){
		...
		if(intError > 0){
			...
			$('#simpan-draf').prop('disabled',false);
			return false;
		}
		...
		swal({
			...
			function(isConfirm) {
			  if (isConfirm) {
					...
					var baseurl		= base_url + active_controller +'/SavedDraf'; 
					$.ajax({
						url			: baseurl,
						type		: "POST",
						data		: formData,
						...
						success		: function(data){
							if(data.status == 1){
								...
								window.location.href = base_url + active_controller +'/draf';
							}
		...
		swal({
			...
			function(isConfirm) {
			  if (isConfirm) {
					loading_spinner();
					var formData 	= new FormData($('#form_proses_bro')[0]);
					var baseurl		= base_url + active_controller +'/SavedBQ';
					$.ajax({
						url			: baseurl,
						type		: "POST",
						data		: formData,
	...
	function AppendBaris(intd){
		...
		$(document).on('change', series, function(){
			$.ajax({
				// url: base_url +'index.php/'+active_controller+'/getDelivery',
				url: base_url +'index.php/'+active_controller+'/getDeliveryX',
				cache: false,
				type: "POST",
		...
		$(document).on('change', id_delivery, function(){
			$.ajax({
				// url: base_url +'index.php/'+active_controller+'/getSubDelivery',
				url: base_url +'index.php/'+active_controller+'/getSubDeliveryX',
				cache: false,
				type: "POST",
		...
		$.ajax({
			// url: base_url +'index.php/'+active_controller+'/getSeriesL',
		...
		$(document).on('change', id_category, function(){
			...
			else if(KS_id_category == 'flange mould'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).hide();
				$(KS_length).hide();
				$(KS_thickness).show();
			...
			else if(KS_id_category == 'flange slongsong'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).hide();
				$(KS_length).hide();
				$(KS_thickness).show();
			...
			else if(KS_id_category == 'concentric reducer' || KS_id_category == 'eccentric reducer' || KS_id_category == 'joint puddle flange'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).show();
				$(KS_thickness).show();
				$(KS_length).hide();
			...
			else if(KS_id_category == 'reducer tee mould' || KS_id_category == 'plate' || KS_id_category == 'rib' || KS_id_category == 'joint rib' || KS_id_category == 'loose flange' || KS_id_category == 'spacer' || KS_id_category == 'blind spacer'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).show();
				$(KS_thickness).show();
				$(KS_length).hide();
			...
			else if(KS_id_category == 'reducer tee slongsong'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).show();
				$(KS_thickness).show();
				$(KS_length).hide();
			...
			else if(KS_id_category == 'equal tee mould'){
				$(KS_diameter_1).show();
				$(KS_length).hide();
				$(KS_diameter_2).hide();
				$(KS_thickness).show();
			...
			else if(KS_id_category == 'equal tee slongsong'){
				$(KS_diameter_1).show();
				$(KS_length).hide();
				$(KS_diameter_2).hide();
				$(KS_thickness).show();
			...
			else if(KS_id_category == 'elbow mould'){
				$(KS_diameter_1).show();
				$(KS_length).hide();
				$(KS_diameter_2).hide();
				$(KS_thickness).show();
			...
			else if(KS_id_category == 'elbow mitter'){
				$(KS_diameter_1).show();
				$(KS_length).hide();
				$(KS_diameter_2).hide();
				$(KS_thickness).show();
