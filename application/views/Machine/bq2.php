...
<form action="#" method="POST" id="form_proses_bro">
	<div class="box box-primary">
		<div class="box-header">
			...
			<div class='note' style='margin-top:5px;'>
				<p>
					...
					Untuk Product Kosong gunakan <b>IPN06-0.5</b>
				</p>
			</div>
		</div>
		<!-- /.box-header -->
				...
				<div class='col-sm-4'>
					...
					<?php
						foreach($ListIPP AS $val => $valx){
							$selected = ($no_ipp == $valx['no_ipp'])?'selected':'';
							echo "<option value='".$valx['no_ipp']."' ".$selected.">".$valx['no_ipp']." - ".$valx['nm_customer']." | ".strtoupper($valx['project'])."</option>";
						}
					?>
					</select>						
				</div>
		...
		<div class='box-footer'>
			<?php
			echo form_button(array('type'=>'button','class'=>'btn btn-md btn-success','value'=>'save','content'=>'Save to Draf','id'=>'simpan-draf')).' ';
...
<script>
	$(document).ready(function(){
		...
		$('#simpan-bro').click(function(e){
			...
			$('#detail_body').find('tr').each(function(){
				...
				//blind flange
				if(spool == 'blind flange' || spool == 'end cap' || spool == 'equal tee mould' || spool == 'equal tee slongsong' || spool == 'flange mould' || spool == 'flange slongsong' || spool == 'shop joint'){
					if(thickness == '' || thickness == 0 || thickness == null){
						intError++;
						pesan = "Thickness has not empty ...";
					}
				...
				//branch point
				if(spool == 'branch joint' || spool == 'concentric reducer' || spool == 'eccentric reducer' || spool == 'reducer tee mould' || spool == 'reducer tee slongsong'){
					if(thickness == '' || thickness == 0 || thickness == null){
						intError++;
						pesan = "Thickness has not empty ...";
					}
			...
			swal({
				  ...
				  text: "Data yang dibuat sudah tidak bisa di edit kembali !",
				  type: "warning",
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl=base_url + active_controller +'/bq2';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
							...
							error: function() {
								...
								swal({
								 title				: "Error Message !",
								  text				: 'Terjadi kesalahan saat proses penyimpanan. Silahkan ulangi kembali. Pastikan QTY tidak KOSONG !!!',
								  type				: "warning",
								  timer				: 7000,
		...
		$('#simpan-draf').click(function(e){
			...
			swal({
				  title: "Apakah kamu yakin ???",
				  text: "Data yang dibuat masih bisa di edit kembali !",
				  type: "warning",
				  showCancelButton: true,
				  confirmButtonClass: "btn-danger",
				  confirmButtonText: "Yes, Process it!",
				...
				function(isConfirm) {
				  if (isConfirm) {
						loading_spinner();
						var formData 	=new FormData($('#form_proses_bro')[0]);
						var baseurl=base_url + active_controller +'/saveDraf';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
	...
	function AppendComponent(nil1, nil2, nil3, NilSpool, hasilUrut2, Series){
		...
		var Rows	 = 	"<tr id='tr3_"+nil1+"_"+nil2+"_"+nil3+"' class='delAll'>";
			...
			Rows	+=			"<input type='text' class='form-control numberOnly qty_"+hasilUrut2+"' style='text-align: center;' name='ListDetailKomp["+hasilUrut2+"][qty]' id='qty_"+nil1+"_"+nil2+"_"+nil3+"' maxlength='5' required autocomplete='off' placeholder='Qty'>";
			Rows	+= 		"</td>";
		...
		var KS_qty 			= "#qty_"+nil1+"_"+nil2+"_"+nil3;
		...
		$(K_id_category).on('change', function(e){
			...
			e.preventDefault();
			if($(this).val() == 'pipe' || $(this).val() == 'saddle'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).hide();
				$(KS_length).show();
				$(KS_thickness).show();
			...
			else if($(this).val() == 'pipe slongsong'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).hide();
				$(KS_length).show();
				$(KS_thickness).show();
			...
			else if($(this).val() == 'end cap' || $(this).val() == 'blind flange with hole'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).hide();
				$(KS_length).hide();
				$(KS_sudut).hide();
			...
			else if($(this).val() == 'blind flange' || $(this).val() == 'field joint'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).hide();
				$(KS_length).hide();
				$(KS_thickness).show();
			...
			else if($(this).val() == 'flange mould' || $(this).val() == 'colar'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).hide();
				$(KS_length).hide();
				$(KS_thickness).show();
			...
			else if($(this).val() == 'flange slongsong' || $(this).val() == 'colar slongsong'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).hide();
				$(KS_length).hide();
				$(KS_thickness).show();
			...
			else if($(this).val() == 'concentric reducer' || $(this).val() == 'joint puddle flange' || $(this).val() == 'blind spacer' ){
				$(KS_diameter_1).show();
				$(KS_diameter_2).show();
				$(KS_thickness).show();
				$(KS_length).hide();
			...
			else if($(this).val() == 'reducer tee mould' || $(this).val() == 'plate' || $(this).val() == 'rib' || $(this).val() == 'joint rib' || $(this).val() == 'loose flange' || $(this).val() == 'spacer' || $(this).val() == 'spacer ring' || $(this).val() == 'figure 8'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).show();
				$(KS_thickness).show();
				$(KS_length).hide();
			...
			else if($(this).val() == 'shop joint' || $(this).val() == 'spectacle blind' || $(this).val() == 'blank and spacer'){
				$(KS_diameter_1).show();
				$(KS_length).hide();
				$(KS_diameter_2).hide();
				$(KS_thickness).show();
	...
	function AppendSlongsong(intd, intd2, intd3, intd4, NilSpool, hasilUrut2, Series)
	{
		...
		var Rows	 = 	"<tr id='tr4_"+intd+"_"+intd2+"_"+intd3+"_"+intd4+"' class='delAll'>";
			...
			Rows	+= 		"</td>";
			Rows	+= 		"<td style='width: 7%;'>";
			Rows	+=			"<input type='text' class='form-control numberOnly qty_"+hasilUrut2+"' style='text-align: center;' name='ListDetailKompSub["+hasilUrut2+"][qty]' id='qty_"+intd+"_"+intd2+"_"+intd3+"_"+intd4+"' maxlength='5' required autocomplete='off' placeholder='Qty'>";
			Rows	+= 		"</td>";
	...
	function AppendBarisKom2(intd, num, nilComp, hasilUrut1, SeriesComp){
		...
		var Rows	 = 	"<tr id='trX2_"+intd+"_"+num+"' class='delAll'>"; 
			...
			Rows	+= 		"</td>";
			Rows	+= 		"<td style='width: 9%;' align='left'>";
			Rows	+=			"<select name='ListDetailKompSingle["+hasilUrut1+"][type]' id='type_"+intd+"_"+num+"' class='chosen_select form-control inline-block type_"+hasilUrut1+"' required><option value='0'>Type</option><option value='SR'>Short Rad</option><option value='LR'>Long Rad</option></select>";
			Rows	+= 		"</td>";
			Rows	+= 		"<td style='width: 7%;'>"; 
			Rows	+=			"<input type='text' class='form-control numberOnly qty_"+hasilUrut1+"' style='text-align: center;' name='ListDetailKompSingle["+hasilUrut1+"][qty]' id='qty_"+intd+"_"+num+"' maxlength='5' required autocomplete='off' placeholder='Qty'>";
			Rows	+= 		"</td>";
			Rows	+= 		"<td style='width: 7%;' align=\"center\">";
			Rows 	+=			"<div style='text-align: center;'>";
		...
		$(K_id_category).on('change', function(e){
			...
			else if($(this).val() == 'concentric reducer' || $(this).val() == 'blind spacer'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).show();
				$(KS_thickness).show();
				$(KS_length).hide();
			...
			else if($(this).val() == 'eccentric reducer' || $(this).val() == 'joint puddle flange'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).show();
				$(KS_thickness).show();
				$(KS_length).hide();
			...
			else if($(this).val() == 'reducer tee mould' || $(this).val() == 'plate' || $(this).val() == 'rib' || $(this).val() == 'joint rib' || $(this).val() == 'loose flange' || $(this).val() == 'spacer' || $(this).val() == 'figure 8' || $(this).val() == 'spacer ring'){
				$(KS_diameter_1).show();
				$(KS_diameter_2).show();
				$(KS_thickness).show();
				$(KS_length).hide();
	...
	function AppendSlongsongComp(intd, intd2, intd3, NilSpool, hasilUrut2, SeriesComp)
	{
		...
		var Rows	 = 	"<tr id='trX3_"+intd+"_"+intd2+"_"+intd3+"' class='delAll'>";
			...
			Rows	+= 		"</td>";
			Rows	+= 		"<td style='width: 7%;'>";
			Rows	+=			"<input type='text' class='form-control numberOnly qty' style='text-align: center;' name='ListDetailKompSub2["+hasilUrut2+"][qty]' id='qty_"+intd+"_"+intd2+"_"+intd3+"' maxlength='5' required autocomplete='off' placeholder='Qty'>";
			Rows	+= 		"</td>";
			Rows	+= 		"<td style='width: 7%;' align=\"center\" class='delAll'>";
			Rows 	+=			"<div style='text-align: center;'>";
