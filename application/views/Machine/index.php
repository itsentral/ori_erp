...
?>
<form action="#" method="POST" id="form_proses_bro" enctype="multipart/form-data" autocomplate='off'> 
<div class="box box-primary">
	<div class="box-header">
		...
		?>
		<!--
		  <a href="<?php echo site_url('machine/bq2') ?>" class="btn btn-sm btn-success" id='btn-add'>
			<i class="fa fa-plus"></i> Add Structure BQ
		  </a> -->
		  <?php
			// }
		  ?>
...
<script>
	...
	$(document).on('click', '#edit_series', function(e){
		...
		$.ajax({
			type:'POST',
			url: base_url+active_controller+'/modalSeries/'+$(this).data('id_bq'),
	...
	$(document).on('click', '#editBQ', function(e){
		...
		$.ajax({
			type:'POST',
			url: base_url+active_controller+'/modalEditBQ/'+$(this).data('id_bq'),
			success:function(data){
				$("#ModalView2").modal();
				$("#view2").html(data);
	...
	$(document).on('click', '#series_change', function(){
		...
		function(isConfirm) {
			if (isConfirm) {
				...
				var formData  	= new FormData($('#form_proses_bro')[0]);
				$.ajax({
					url			: base_url+'index.php/'+active_controller+'/EditSeries/'+bq+'/'+series+'/'+id,
					type		: "POST",
					data		: formData,
					cache		: false,
					dataType	: 'json',
