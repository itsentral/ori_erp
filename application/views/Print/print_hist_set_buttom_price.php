<?php
	...
	$result2BTS		= $this->db->get_where('laporan_revised_etc',array('id_bq'=>$id_bq,'category'=>'engine','revised_no'=>$rev))->result_array();
	$result3BTS		= $this->db->get_where('laporan_revised_etc',array('id_bq'=>$id_bq,'category'=>'packing','revised_no'=>$rev))->result_array();
	$result4BTS		= $this->db->get_where('laporan_revised_etc',array('id_bq'=>$id_bq,'category'=>'export','revised_no'=>$rev))->result_array();
	$result5BTS		= $this->db->get_where('laporan_revised_etc',array('id_bq'=>$id_bq,'category'=>'lokal','revised_no'=>$rev))->result_array();
	$rest_mat		= $this->db->get_where('laporan_revised_etc',array('id_bq'=>$id_bq,'category'=>'aksesoris','revised_no'=>$rev))->result_array();
	$rest_baut		= $this->db->get_where('laporan_revised_etc',array('id_bq'=>$id_bq,'category'=>'baut','revised_no'=>$rev))->result_array();
	$rest_plate		= $this->db->get_where('laporan_revised_etc',array('id_bq'=>$id_bq,'category'=>'plate','revised_no'=>$rev))->result_array();
	$rest_gasket	= $this->db->get_where('laporan_revised_etc',array('id_bq'=>$id_bq,'category'=>'gasket','revised_no'=>$rev))->result_array();
	$rest_lainnya	= $this->db->get_where('laporan_revised_etc',array('id_bq'=>$id_bq,'category'=>'lainnya','revised_no'=>$rev))->result_array();
	...
	<table class='header_style' border='0' width='100%' cellpadding='2'>
		<thead>
		<tr>
			<td><b><h2>Selling Price <span style='color:#0e5ca9;'><?=str_replace('BQ-', '', $id_bq);?></span> for Internal (Revision <?=$rev;?>)</h2></b></td>
		</tr>
		</thead>
	</table>
	<br>
	...
	<table class="gridtable" width='100%' border='0' cellpadding='2'>
		...
		<tbody class='body_x'>
			...
			foreach($result1BTS AS $val => $valx){
				...
				echo "<tr>";
					...
					echo "<td align='right'>".number_format($bottom_price,2)."</td>";
					echo "<td align='right'>".number_format($extra,2)."</td>";
					echo "<td align='right'>".number_format($allow,2)."</td>";
			...
			<tr class='FootColor'>
				...
				<td align='center'><b><?= number_format(($SUMx - $HPP_Tot)/$SUMx,2) * 100;?> %</b></td>
				<td align='right'><b></b></td>
		...
		if(!empty($rest_baut)){
			...
			foreach($rest_baut AS $val => $valx){
				...
				echo "<tr>";
					echo "<td colspan='9'>".strtoupper(get_name_acc($valx['caregory_sub']))."</td>";
					echo "<td align='center'>".number_format($valx['qty'])."</td>";
					...
					echo "<td align='right'>".number_format($valx['profit'],2)."</td>";
					echo "<td align='right'>".number_format($valx['price'],2)."</td>";
					echo "<td align='right'>".number_format($valx['allow'],2)."</td>";
		...
		if(!empty($rest_mat)){
			...
			foreach($rest_mat AS $val => $valx){
				...
				$unit_price = 0;
				if($valx['weight'] > 0){
					$unit_price = $valx['fumigasi']/$valx['weight'];
				}
				echo "<tr>";
					echo "<td colspan='9'>".strtoupper(get_name('raw_materials', 'nm_material', 'id_material', $valx['caregory_sub']))."</td>";
					echo "<td align='right'>".number_format($valx['weight'],2)."</td>";
					echo "<td align='center'>".strtoupper(get_name('raw_pieces', 'kode_satuan', 'id_satuan', $valx['option_type']))."</td>";
		...
		<tbody class='body_x'>
			...
			foreach($result2BTS AS $val => $valx){
				...
				$no1++;
				echo "<tr>";
					echo "<td colspan='11'>".strtoupper($valx['caregory_sub'])."</td>";
		...
		<tbody class='body_x'>
			...
			foreach($result3BTS AS $val => $valx){
				...
				echo "<tr>";
					echo "<td colspan='14'>".strtoupper($valx['caregory_sub']);
					echo "</td>";
		...
		<tbody class='body_x'>
			...
			foreach($result4BTS AS $val => $valx){
				...
				$no3++;
				echo "<tr>";
					echo "<td colspan='8'>".strtoupper(get_name('country_all','name','iso3',$country_name))."</td>";
	...
	$mpdf->SetTitle('Selling Price '.str_replace('BQ-', '', $id_bq).' Rev.'.$rev);
	// $mpdf->AddPage('L');
	// $mpdf->SetDisplayMode('fullpage');
	$mpdf->SetFooter($footer);
	$mpdf->WriteHTML($html);
	$mpdf->Output("SELLING PRICE REV.".$rev." ".str_replace('BQ-', '', $id_bq)."_".date('d/m/Y/H/i/s').".pdf" ,'I');