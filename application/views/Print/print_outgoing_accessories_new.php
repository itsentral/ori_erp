...
$rest_d		= $this->db
                    ->select('c.*, d.satuan AS id_satuan')
                    ->join('warehouse_adjustment_detail c','a.kode_trans=c.kode_trans','left')
                    ->join('con_nonmat_new d','c.id_material=d.code_group','left')
                    ->get_where('warehouse_adjustment a', array('a.kode_trans'=>$kode_trans,'a.deleted_date'=>NULL))->result_array();
...
<table class="gridtable2" width="100%" border='0' style='border-bottom:none;'>
	...
	<tr>
		<td colspan='3' style='border-bottom:none;' align='center'><u><b>SPK PENGIRIMAN BARANG</b></u></td>
	...
	<tr>
		<td style='border-bottom:none; border-top: none; border-right: none;'>Kode</td>
		<td style='border:none;'>:</td>
		<td style='border-bottom:none; border-top: none; border-left:none;'><?=$rest_data[0]->kode_trans;?></td>
...
<table class="gridtable2" width='100%' border='1' cellpadding='2'>
	...
	foreach($rest_d AS $val => $valx){
		echo "<tr>";
			echo "<td align='right'>".number_format($valx['check_qty_oke'],2)."</td>";
			echo "<td align='center'>".strtoupper(get_name('raw_pieces','kode_satuan','id_satuan',$valx['id_satuan']))."</td>";
			echo "<td align='left'>".strtolower($valx['nm_material'])."</td>";
			echo "<td align='left'></td>";
		echo "</tr>";
	}
	echo "<tr>";
		echo "<td colspan='2'></td>";
		echo "<td colspan='2'><b>Note: Barang dikirim dari dalam keadaan baik</b></td>";
	// echo "<tr>";
...
$mpdf->WriteHTML($html);
ob_clean();
$mpdf->Output('spk-pengiriman-barang-'.$kode_trans.'.pdf' ,'I');