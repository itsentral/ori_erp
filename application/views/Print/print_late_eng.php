...
<table class="gridtable2" border='1' width='100%' cellpadding='2'>
	...
	<tr>
		<td align='center'><b><h2>LIST LATE ENGINNERING</h2></b></td>
	</tr>
</table>
<br>
	...
	<tbody>
		...
		if(!empty($data)){
			foreach($data AS $val => $valx){ 
                $release_plus2 = date('Y-m-d', strtotime('+2 days', strtotime($valx['release_ipp'])));
                if(date('Y-m-d') > $release_plus2){
                    ...
                    echo "<tr>";
                        ...
                        echo "<td style='vertical-align:top;' align='center'>".date('d-M-Y', strtotime($valx['release_ipp']))."</td>";
...
$mpdf->SetTitle('Late Enginnering'); 
$mpdf->AddPageByArray([
	'margin-left' => 5,
	'margin-right' => 5,
	'margin-top' => 5,
...
$mpdf->WriteHTML($html);
$mpdf->Output('late-est-enginnering.pdf' ,'I');