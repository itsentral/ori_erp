						...
						<table class="table table-bordered table-striped" id="my-grid" width='100%'>
							<thead class='thead'>
								<tr class='bg-blue'>
									...
									<th class="text-center th no-sort">Surat Jalan</th>
...
<script src="//cdn.datatables.net/plug-ins/1.13.1/pagination/select.js"></script>
<script>
		...
		$(document).on('click', '#approvedQ', function(){
			...
			function(isConfirm) {
				if (isConfirm) {
					...
					$.ajax({
						url			: base_url+active_controller+'/closing_jurnal',
						type		: "POST",
						data		: formData,
						cache		: false,
						dataType	: 'json',
						...
						success		: function(data){								
							if(data.status == 1){											
								...
								window.location.href = base_url + active_controller+'/material/'+data.gudang;
							}
	...
	function DataTables(id=null){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/server_side_material',
				type: "post",
				data: function(d){
					d.id = id
				},
	...
	function DataTables2(id=null){
		var dataTable = $('#my-grid2').DataTable({
			...
			"aLengthMenu": [[10, 20, 50, 100, 150, 500, 750, 1000], [10, 20, 50, 100, 150, 500, 750, 1000]],
			"ajax":{
				url : base_url + active_controller+'/server_side_material_close',
				type: "post",
				data: function(d){
					d.id = id
				},
	...
	$(document).on('click', '.view2', function(){
		...
		$.ajax({
			type:'POST',
			url:base_url+'jurnal_nomor/view_jurnal_jv/'+id+'/'+kd+'/'+ket+'/'+akses+'/'+total+'/'+id_vendor+'/'+nm_vendor,
			data:{'id':id},
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
	...
	$(document).on('click', '.view3', function(){
		...
		var akses = 'approval_jurnal_po_nonstok';
		var kd ='JV003'
		var ket ='SUB GUDANG - GD PRODUKSI'
		// alert(id);
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Detail Jurnal</b>");
		$.ajax({
			type:'POST',
	...
	$(document).on('click', '.view5', function(){
		var id = $(this).data('id_material');
		var total = $(this).data('id_total');
		var id_vendor = $(this).data('id_vendor');
		var nm_vendor = $(this).data('nm_vendor');
		...
		var akses = 'approval_jurnal_po_nonstok';
		var kd ='JV003'
		var ket ='RETUR MATERIAL'
		// alert(id);
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Detail Jurnal</b>");
		$.ajax({
			type:'POST',
			url:base_url+'jurnal_nomor/view_jurnal_retur_gudang/'+id+'/'+kd+'/'+ket+'/'+akses+'/'+total+'/'+id_vendor+'/'+nm_vendor,
			data:{'id':id},
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
