						...
						<table class="table table-bordered table-striped" id="my-grid" width='100%'>
							<thead class='thead'>
								<tr class='bg-blue'>
									...
									<th class="text-center th no-sort">Asset</th>
...
<script>
		...
		$(document).on('click', '#approvedQ', function(){
			...
			function(isConfirm) {
				if (isConfirm) {
					...
					$.ajax({
						url			: base_url+active_controller+'/closing_jurnal_assets',
						type		: "POST",
						data		: formData,
						cache		: false,
						dataType	: 'json',
						...
						success		: function(data){
							if(data.status == 1){
								...
								window.location.href = base_url + active_controller+'/assets';
							}
	...
	function DataTables(id=null){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/server_side_assets',
				type: "post",
				// data: function(d){
				// 	d.id = id
				// },
	...
	function DataTables2(id=null){
		var dataTable = $('#my-grid2').DataTable({
			...
			"aLengthMenu": [[10, 20, 50, 100, 150, 500, 750, 1000], [10, 20, 50, 100, 150, 500, 750, 1000]],
			"ajax":{
				url : base_url + active_controller+'/server_side_assets_close',
				type: "post",
				// data: function(d){
				// 	d.id = id
				// },
	...
	$(document).on('click', '.view2', function(){
		...
		var kd ='JV037'
		var akses = 'assets';
		var ket ='ASSETS DEPRECIATION'
		// alert(id);
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Detail Jurnal</b>");
		$.ajax({
			type:'POST',
			url:base_url+'jurnal_nomor/view_jurnal_aset_depreciation/'+id+'/'+kd+'/'+ket+'/'+akses+'/'+total+'/'+id_vendor+'/'+nm_vendor,
			data:{'id':id},
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
