						...
						<table class="table table-bordered table-striped" id="my-grid" width='100%'>
							<thead class='thead'>
								<tr class='bg-blue'>
									...
									<th class="text-center th no-sort">Nomor PO</th>
...
<script>
		...
		$(document).on('click', '#approvedQ', function(){
			...
			function(isConfirm) {
				if (isConfirm) {
					...
					$.ajax({
						url			: base_url+active_controller+'/closing_jurnal_department_incoming',
						type		: "POST",
						data		: formData,
						cache		: false,
						dataType	: 'json',
						...
						success		: function(data){								
							if(data.status == 1){											
								...
								window.location.href = base_url + active_controller+'/department_incoming';
							}
	...
	function DataTables(id=null){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/server_side_department_incoming',
				type: "post",
				// data: function(d){
				// 	d.id = id
				// },
	...
	function DataTables2(id=null){
		var dataTable = $('#my-grid2').DataTable({
			...
			"aLengthMenu": [[10, 20, 50, 100, 150, 500, 750, 1000], [10, 20, 50, 100, 150, 500, 750, 1000]],
			"ajax":{
				url : base_url + active_controller+'/server_side_department_incoming_close',
				type: "post",
				// data: function(d){
				// 	d.id = id
				// },
	...
	$(document).on('click', '.view2', function(){
		...
		var pp = 'incoming department';
		var kd ='JV036'
		var akses = 'department_incoming';
		var ket ='Incoming Department Jurnal'
		// alert(id);
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Detail Jurnal</b>");
		$.ajax({
			type:'POST',
			url:base_url+'jurnal_nomor/view_jurnal_incoming_department/'+id+'/'+kd+'/'+ket+'/'+akses+'/'+total+'/'+id_vendor+'/'+nm_vendor+'/'+no_po,
			data:{'id':id},
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
