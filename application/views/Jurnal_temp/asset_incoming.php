...
<script>
		...
		$(document).on('click', '#approvedQ', function(){
			...
			function(isConfirm) {
				if (isConfirm) {
					...
					$.ajax({
						url			: base_url+active_controller+'/closing_jurnal_asset_incoming',
						type		: "POST",
						data		: formData,
						cache		: false,
						dataType	: 'json',
	...
	function DataTables(id=null){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/server_side_asset_incoming',
				type: "post",
				// data: function(d){
				// 	d.id = id
				// },
	...
	function DataTables2(id=null){
		var dataTable = $('#my-grid2').DataTable({
			...
			"aLengthMenu": [[10, 20, 50, 100, 150, 500, 750, 1000], [10, 20, 50, 100, 150, 500, 750, 1000]],
			"ajax":{
				url : base_url + active_controller+'/server_side_asset_incoming_close',
				type: "post",
				// data: function(d){
				// 	d.id = id
				// },
	...
	$(document).on('click', '.view2', function(){
		...
		var pp = 'incoming jurnal assets';
		var kd ='JV038'
		var akses = 'asset_incoming';
		var ket ='JURNAL INCOMING ASSETS'
		// alert(id);
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Detail Jurnal</b>");
		$.ajax({
			type:'POST',
			url:base_url+'jurnal_nomor/view_jurnal_incoming_asset/'+id+'/'+kd+'/'+ket+'/'+akses+'/'+total+'/'+id_vendor+'/'+nm_vendor,
			data:{'id':id},
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
