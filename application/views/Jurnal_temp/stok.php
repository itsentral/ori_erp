						...
						<table class="table table-bordered table-striped" id="my-grid2" width='100%'>
							<thead class='thead'>
								<tr class='bg-blue'>
									...
									<th class="text-center th no-sort">View Jurnal <input type='checkbox' name='chk_all_2' id='chk_all_2'></th>
								</tr>
							</thead>
							<tbody></tbody>
						</table>
...
<script>
	...
	function doLoop() {
		...
		autoj="autoj";
		var idx=$(elements[i]).val();
		var id = $(elements[i]).data('id_material');
		var total = $(elements[i]).data('id_total');
		var id_vendor = $(elements[i]).data('id_vendor');
		var nm_vendor = $(elements[i]).data('nm_vendor');
		var pp = 'stok';
		var kd ='JV039'
		var akses = 'jurnal_temp/stok';
		var ket ='OUTGOING STOK'
		$('#spinner').show();
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Detail Jurnal</b>");
		$.ajax({
			type:'POST',
			url:base_url+'jurnal_nomor/view_jurnal_outgoing_stock/'+id+'/'+kd+'/'+ket+'/'+akses+'/'+total+'/'+id_vendor+'/'+nm_vendor+'/autoj',
			data:{'id':id},
		...
		$(document).on('click', '#approvedQ', function(){
			...
			function(isConfirm) {
				if (isConfirm) {
					...
					$.ajax({
						url			: base_url+active_controller+'/closing_jurnal_stok',
						type		: "POST",
						data		: formData,
						cache		: false,
						dataType	: 'json',
	...
	function DataTables(id=null){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/server_side_stok',
				type: "post",
				// data: function(d){
				// 	d.id = id
				// },
	...
	function DataTables2(id=null){
		var dataTable = $('#my-grid2').DataTable({
			...
			"aLengthMenu": [[10, 20, 50, 100, 150, 500, 750, 1000], [10, 20, 50, 100, 150, 500, 750, 1000]],
			"ajax":{
				url : base_url + active_controller+'/server_side_stok_close',
				type: "post",
				// data: function(d){
				// 	d.id = id
				// },
	...
	$(document).on('click', '.view2', function(){
		...
		$.ajax({
			type:'POST',
			url:base_url+'jurnal_nomor/view_jurnal_outgoing_stock/'+id+'/'+kd+'/'+ket+'/'+akses+'/'+total+'/'+id_vendor+'/'+nm_vendor,
			data:{'id':id},
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
	...
	$(document).on('click', '.viewonly', function(){
		var id = $(this).data('id');
		var tipe = 'JV';
		var jenisjurnal ='outgoing stok';
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Detail Jurnal</b>");
		$.ajax({
			type:'POST',
			url:base_url+'jurnal_nomor/view_jurnal_tras_finish/'+id+'/'+tipe+'/'+jenisjurnal,
			data:{'id':id},
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
