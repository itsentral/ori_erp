						...
						<table class="table table-bordered table-striped" id="my-grid" width='100%'>
							<thead class='thead'>
								<tr class='bg-blue'>
									...
									<th class="text-center th no-sort">Nilai WIP</th>
...
<script>
	...
	function doLoop() {
		...
		var id_milik = $(elements[i]).data('id_milik');
		var total_sum = $(elements[i]).data('total_sum');
		var id_spk = $(elements[i]).data('id_spk');
		var tanggal = $(elements[i]).data('tanggal');
		var pp = 'PRODUKSI - WIP';
		var kd ='JV004'
		var akses = 'material_wip';
		$('#spinner').show();
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Detail Jurnal</b>");
		$.ajax({
			type:'POST',
			url:base_url+'jurnal_nomor/view_jurnal_wip/autoj',
	...
	$(document).ready(function(){
		...
		$(document).on('click', '#approvedQ', function(){
			...
			function(isConfirm) {
				if (isConfirm) {
					...
					$.ajax({
						url			: base_url+active_controller+'/closing_jurnal_wip',
						type		: "POST",
						data		: formData,
						cache		: false,
						dataType	: 'json',
						...
						success		: function(data){								
							if(data.status == 1){											
								...
								window.location.href = base_url + active_controller+'/material_wip';
							}
		...
		$(document).on('click', '.detail_material', function(){
			...
			$.ajax({
				type:'POST',
				url:base_url+active_controller+'/modal_detail_wip/'+id_milik+'/'+id_spk,
				success:function(data){
					$("#ModalView2").modal();
					$("#view2").html(data);
				},
	...
	function DataTables(){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/server_side_material_wip',
				type: "post",
				// data: function(d){
				// 	d.id = id
				// },
	...
	function DataTables2(){
		var dataTable2 = $('#my-grid2').DataTable({
			...
			"aLengthMenu": [[10, 20, 50, 100, 150, 500, 750, 1000], [10, 20, 50, 100, 150, 500, 750, 1000]],
			"ajax":{
				url : base_url + active_controller+'/server_side_material_close_wip',
				type: "post",
				// data: function(d){
				// 	d.id = id
				// },
	...
	$(document).on('click', '.view2', function(){
		...
		$.ajax({
			type:'POST',
			url:base_url+'jurnal_nomor/view_jurnal_wip/',
			data:{'id_milik':id_milik,'total_sum':total_sum,'id_spk':id_spk,'tanggal':tanggal,'pp':pp,'kd':kd,'akses':akses},
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
		...
		$.ajax({
			type:'POST',
			url:base_url+'jurnal_nomor/view_jurnal_wip_history/',
			data:{'id_milik':id_milik,'total_sum':total_sum,'id_spk':id_spk,'tanggal':tanggal,'pp':pp,'kd':kd,'akses':akses},
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
