		...
		<div class="box box-primary">
			<div class="box-body">
				...
				<div class="form-group ">
					...
					<div class="col-sm-4">
						<input type="text" class="form-control tanggal" id="date1" name="date1" value="<?php echo (isset($data->date1) ? $data->date1: date("Y-m-d")); ?>" placeholder="Tanggal Awal" required>
					</div>
				...
				<div class="form-group ">
					...
					<div class='col-sm-4 col-md-4'>
					 <?php
						$deptid=(isset($data->department)?$data->department:$dept);
						echo form_dropdown('departement',$combodept,$deptid,array('id'=>'departement','class'=>'form-control','required'=>'required'));
					 ?>
					</div>
					<label class='col-sm-2 col-md-2 control-label'><b>COA</b> <b class="text-red">*</b></label>
					<div class='col-sm-4 col-md-4'>
					 <?php
						$combocoa[]='Select an option';
						$dtmsgen=$this->db->query("select kode_text from ms_generate where tipe='coa_transportasi'")->row();
						$coatrans=str_ireplace(";","','",$dtmsgen->kode_text);
						$data_coa = $this->db->query("select * from ".DBACC.".coa_master where no_perkiraan in ('".$coatrans."')")->result();
						foreach($data_coa as $keys){
							$combocoa[$keys->no_perkiraan] = $keys->no_perkiraan.' - '.$keys->nama;
						}
						$coa=(isset($data->coa)?$data->coa:"");
						echo form_dropdown('coa',$combocoa,$coa,array('id'=>'coa','class'=>'form-control','required'=>'required'));
					 ?>
					<?php
			...
			<div class="box-footer">
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-10">
						...
						<a class="btn btn-warning btn-sm" onclick="window.location.reload();return false;"><i class="fa fa-reply"></i> Batal</a>
					</div>
				</div>
				<div class="row">
				<?=$gambar?>
...
<script type="text/javascript">
	var url_save = base_url+'expense/transport_req_save/';
	var url_approve = base_url+'expense/transport_req_approve/';
	var url_reject = base_url+'expense/transport_req_reject/';
	var nomor=<?=$idd?>;
	$('.divide').divide();
	$('#coa').select2();	
...
//			data_save();
	...
	function add_detail(){
		...
		var date2 = $("#date2").val();
		$.ajax({
			url: base_url +'expense/get_list_req_transport/'+nama+'/'+date1+'/'+date2,
			cache: false,
			type: "POST",
			dataType: "json",
			success: function(data){
				...
				for(i=0; i<data.length; i++){
					var Rows	 = 	"<tr id='tr1_"+nomor+"' class='delAll kasbonrow'>";
						...
						Rows	+= 		"</td>";
						Rows	+= 		"<td>";
						Rows	+=			"<input type='text' class='form-control divide ftol input-sm' name='bensin[]' value='"+data[i].tol+"' id='tol_"+nomor+"' tabindex='-1' readonly />";
						Rows	+= 		"</td>";
						Rows	+= 		"<td>";
