            ...
            <tbody>
				...
				if(!empty($detail)){
					foreach($detail AS $val => $valx){ $nomor++;
						$qty_rev = (!empty($valx['qty_rev']))?$valx['qty_rev']:$valx['qty_purchase'];
						...
						echo "<tr class='header_".$nomor."'>";
                            ...
                            echo "<td align='left'>
									<select name='detail[".$nomor."][id_material]' class='form-control' ".$disabled2.">";
									echo "<option value='0'>Pilih</option>";
									foreach ($raw_material as $key => $value) {
										$selected = ($value['id_material'] == $valx['id_material'])?'selected':'';
										echo "<option value='".$value['id_material']."' ".$selected.">".$value['nm_material']."</option>";
									}
							echo "</select></td>";
							echo "<td align='left'><input type='text' ".$disabled2." id='qty_".$nomor."' name='detail[".$nomor."][qty]' class='form-control input-md text-center autoNumeric2 sum_tot' value='".$qty_rev."'></td>";
							echo "<td align='center'>KG</td>";
							echo "<td align='left'><input type='text' ".$disabled2." id='harga_".$nomor."' name='detail[".$nomor."][price_unit]' class='form-control input-md text-right autoNumeric2 sum_tot' value='".$nil_rev."'></td>";
...
<script>
	...
	$(document).on('click', '#back', function(e){
		...
		window.location.href = base_url + active_controller +'/pengajuan'+tanda;
	});
				...
				$.ajax({
					...
					success		: function(data){
						if(data.status == 1){
							...
							window.location.href = base_url + active_controller+'/pengajuan/'+data.tanda;
						}
