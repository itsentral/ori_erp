...
$standart_code 		= $this->uri->segment(3);
$parent_product 	= str_replace('-', ' ', $this->uri->segment(4)); 
$diameter 			= $this->uri->segment(5);
$id_product 		= $this->uri->segment(6); 

$sqlHead	= "SELECT * FROM component_header WHERE id_product='".$id_product."' LIMIT 1 ";
$restHead 	= $this->db->query($sqlHead)->result();
...
if($parent_product == 'concentric reducer' OR $parent_product == 'eccentric reducer' OR $parent_product == 'equal tee mould' OR $parent_product == 'equal tee slongsong' OR $parent_product == 'reducer tee mould' OR $parent_product == 'reducer tee slongsong'){
	$sqlPlus = " AND diameter2 = '".$restHead[0]->diameter2."' ";
}

$sqlDefaultChk	= "SELECT * FROM component_default WHERE id_product='".$id_product."' LIMIT 1";
$NumDft	= $this->db->query($sqlDefaultChk)->num_rows();
if($NumDft > 0){
	$sqlDefault	= "SELECT * FROM component_default WHERE id_product='".$id_product."' LIMIT 1";
}
else{
	$sqlDefault	= "SELECT * FROM help_default WHERE product_parent='".$parent_product."' AND standart_code='".$standart_code."' AND diameter='".$diameter."' ".$sqlPlus." LIMIT 1";
}
// echo $sqlDefault; exit;
$getDefault	= $this->db->query($sqlDefault)->result_array();

if(!empty($getDefault)){
	
?>
<form action="#" method="POST" id="form_proses_bro2">  
<div class="box box-success">
	<div class="box-body" style="">
		<table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
			<tbody>
				...
				<tr>
					...
					<td class="text-left">
						<?php
							echo form_input(array('type'=>'hidden','id'=>'id_product','name'=>'id_product','class'=>'form-control input-sm inSpL'), $id_product );	
							...
							echo form_input(array('type'=>'hidden','id'=>'standart_code','name'=>'standart_code','class'=>'form-control input-sm inSpL'), $standart_code );	
							echo form_input(array('type'=>'hidden','id'=>'product_parent','name'=>'product_parent','class'=>'form-control input-sm inSpL'), $parent_product );	
							echo form_input(array('type'=>'hidden','id'=>'diameter','name'=>'diameter','class'=>'form-control input-sm inSpL'), $diameter );
							echo form_input(array('type'=>'hidden','id'=>'diameter2','name'=>'diameter2','class'=>'form-control input-sm inSpL'), $getDefault[0]['diameter2'] );	
							echo form_input(array('type'=>'hidden','id'=>'pn','name'=>'pn','class'=>'form-control input-sm inSpL'), $getDefault[0]['pn'] );	
							echo form_input(array('type'=>'hidden','id'=>'liner','name'=>'liner','class'=>'form-control input-sm inSpL'), $getDefault[0]['liner'] );	
		...
		<br>
		<?php if($getDefault[0]['product_parent'] == 'flange mould' OR $getDefault[0]['product_parent'] == 'flange slongsong' OR $getDefault[0]['product_parent'] == 'colar' OR $getDefault[0]['product_parent'] == 'colar slongsong'){ ?>
		<table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
			<tbody>
				<tr style='background-color: #175477; color: white; font-size: 15px;'>
...
<?php
	...
	echo "<span style='font-size:large; color:red;'><b>Opps... <u>Data Default Tidak Ditemukan</u>, Cek di Menu <span style='color:blue;'><u>'Master Default'</u></span> </b><span>";
}
?>
