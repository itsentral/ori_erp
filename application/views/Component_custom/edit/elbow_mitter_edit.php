	...
	<div class="box box-primary">
		...
		<div class="box-body">
			...
			<div class='form-group row'>		 	 
				...
				<div class='col-sm-4'>              
					<?php
						...
						echo form_input(array('id'=>'diameter','name'=>'diameter','class'=>'HideCost'),$header[0]->diameter);
						...
						echo form_input(array('id'=>'overlap','name'=>'overlap','class'=>'HideCost'));
						// echo form_input(array('id'=>'radius','name'=>'radius','class'=>'form-control input-sm HideCost','autocomplete'=>'off'));	
					?>	
					<select name='top_typeList' id='top_typeList' class='form-control input-sm' disabled>
						...
						<?php
							foreach($product AS $val => $valx){
								$selx	= ($header[0]->diameter == $valx['value_d'])?'selected':'';
			...
			<div class='form-group row'>		 	 
				...
				<div class='col-sm-3'>
					<!--
					<select name='standart_codex' id='standart_codex' class='form-control input-sm'>
			...
			<div class='form-group row'>
				...
				<div class='col-sm-2'>              
					<select name='type_elbow' id='type_elbow' class='form-control input-sm'>
						<option value='SR' <?= ($header[0]->type_elbow == 'SR')?'selected':''; ?>>Short Radius</option>
						<option value='LR' <?= ($header[0]->type_elbow == 'LR')?'selected':''; ?>>Long Radius</option>
					</select>
				</div>
				<div class='col-sm-1'>              
					<?php
						if(!empty($header[0]->high)){
							$nil_type = floatval($header[0]->high);
						}
						else{
							if($header[0]->type_elbow == 'SR'){
								$nil_type = 1;
							}
							else{
								$nil_type = 1.5;
				...
				<div class='col-sm-1'>              
					<?php
						echo form_input(array('type'=>'text','id'=>'radius','name'=>'radius','class'=>'form-control input-sm ','autocomplete'=>'off','readonly'=>'readonly','placeholder'=>'Radius'), $header[0]->radius);											
					?>	
				</div>
				
				<label class='label-control col-sm-2'><b>Angle | Length Mitter</b></label>
				<div class='col-sm-2'>              
					<?php
						echo form_input(array('id'=>'angle','name'=>'angle','class'=>'form-control input-sm numberOnly','autocomplete'=>'off','placeholder'=>'Angle', 'maxlength'=>'4'), floatval($header[0]->angle));											
					?>	
				</div>
				<div class='col-sm-2'>              
					<?php
						echo form_input(array('id'=>'panjang','name'=>'panjang','class'=>'form-control input-sm numberOnly','autocomplete'=>'off','placeholder'=>'Length Mitter','readonly'=>'readonly'), $header[0]->panjang);											
					?>	
				</div>
			</div>
			<!-- ====================================================================================================== -->
							...
							<?php
								...
								foreach($detStructure AS $val => $valx){
									...
									if($ListdetStructure[0]['category'] == 'RESIN'){
										...
										$layY	= "<input type='text' name='ListDetail2[".$no."][layer]' id='layerStr_".$no."' data-nomor='".$no."' data-type='".$valx['id_ori']."' class='form-control input-sm numberOnly layerStr' style='text-align:right; width:80px;' value='".floatval($valx['layer'])."'>";
...
<script src="<?php echo base_url('application/views/Component/edit/javascript/elbow_mitter.js'); ?>"></script>
...
<script>	
	$(document).ready(function(){
		...
		var nil = parseFloat($('#nil_type').val());
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl		= base_url +'edit_custom/elbow_mitter_edit';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
