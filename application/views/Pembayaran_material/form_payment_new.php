...
<?=form_open('pembayaran_material/save_payment_new',array('id'=>'frm_data','name'=>'frm_data'));?>
		  ...
		  <div class="box-body">
			...
			<tr>
				...
				if($nopo!=$record->no_po){
					$datapo  = $this->db->query("select terima_barang_idr,sisa_hutang_idr from tran_material_po_header where no_po='".$record->no_po."'")->row();
					$terima_barang_idr=($terima_barang_idr+$datapo->terima_barang_idr);
					$sisahutangidr = $datapo->sisa_hutang_idr;
				}
				$nopo=$record->no_po;
			...
			<tfoot>
				...
				<tr>
					...
					if($total>0){
						echo number_format($sisahutangidr,2);
					}
					?>
					</td>
					<td><input type="text"  class="form-control"name="curs_header" id="curs_header" value="<?=(isset($data)?$data->curs_header:$curs_header); ?>" readonly />
			</td>
				</tr>
			</tfoot>
			</table>
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {					  
					  ...
					  $.ajax({
							url         : base_url + active_controller+"/save_payment_new",
							type		: "POST",
							data		: formData,
							cache		: false,
							dataType	: 'json',
