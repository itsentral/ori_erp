...
<script>
	$(document).ready(function(){
		...
		$(document).on('click', '#viewM', function(e){
			...
			$("#view").load(base_url+ active_controller+'/modalDetail1/'+id+'/foh_tanki');
			$("#ModalView").modal();
		});
		...
		$(document).on('click', '#editM', function(e){
			...
			e.preventDefault();
			$("#head_title").html("<b>DETAIL COST FOH</b>");
			$("#view").load(base_url + active_controller+'/modalEdit1/'+id+'/foh_tanki');
			$("#ModalView").modal();
		});
					...
					$.ajax({
						...
						success		: function(data){								
							if(data.status == 1){											
								...
								window.location.href = base_url + active_controller + '/foh_tanki';
							}
