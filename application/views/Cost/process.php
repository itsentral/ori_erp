...
<script>
	$(document).ready(function(){
		...
		$(document).on('click', '#viewM', function(e){
			...
			$("#view").load(base_url +'index.php/'+ active_controller+'/modalDetail1/'+id+'/process');
			$("#ModalView").modal();
		});
		...
		$(document).on('click', '#editM', function(e){
			...
			e.preventDefault();
			$("#head_title").html("<b>DETAIL PROCESS COST</b>");
			$("#view").load(base_url +'index.php/'+ active_controller+'/modalEdit1/'+id+'/process');
			$("#ModalView").modal();
		});
		...
		$(document).on('click', '#del_satuan', function(){
			...
			function(isConfirm) {
				if (isConfirm) {
					loading_spinner();
					$.ajax({
						url			: base_url+'index.php/'+active_controller+'/hapus/'+bF+'/process',
						type		: "POST",
						data		: "id="+bF,
						cache		: false,
						dataType	: 'json',
