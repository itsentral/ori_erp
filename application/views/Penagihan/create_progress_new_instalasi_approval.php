	...
	<div class="box box-primary">
		...
		<div class="box-body">
			...
			<div class='form-group row'>
				...
				<label class='label-control col-sm-2 hidden'><b>No Surat Jalan</b></label>
				<div class='col-sm-4 hidden'>
					<input type="text" name="delivery_no" id="delivery_no" class="form-control input-md" value="<?=$penagihan[0]->delivery_no;?>" readonly tabindex="-1">
				</div>
...
<?php
			if ($base_cur=='USD'){
				$this->load->view('Penagihan/progress_usd_instalasi');
			}else{
				$this->load->view('Penagihan/progress_idr_instalasi');
			}
?>
			<?php
				if(isset($approval)){
...
?>
	function approve(id){		
		...
		function(isConfirm) {
			if (isConfirm) {
			  var baseurl=base_url + active_controller +'/create_invoice/'+id;
			  $.ajax({
				url			: baseurl,
				type		: "POST",
				cache		: false,
	...
	$(document).ready(function(){
			...
			if ($('#tgl_inv').val() == "") {
				...
				function(isConfirm) {
					if (isConfirm) {
					  ...
					  var baseurl=base_url + active_controller +'/create_progress_new_instalasi';
					  $.ajax({
						url			: baseurl,
						type		: "POST",
						data		: formData,
		...
		$(document).on('blur', '#down_payment', function(){
			harga_total = ($('#grand_total').val());
			dp = ($('#down_payment').val());			
			umpersen = ((parseFloat(dp)/parseFloat(harga_total))*100);
			$('#persen').val(umpersen);
			grandtotal()
		});
...
*/
		...
		$(document).on('keyup change', '.qty_oth', function(){
			let dataNomor = $(this).data('nomor');
			let hargaSat  = $('#harga_sat9'+dataNomor).val();
			let dataIni	  = $(this).val();
			let datasisa=$("#qty9_belum_"+dataNomor).val();
			if(parseFloat(datasisa)<parseFloat(dataIni)) {
				$(this).val(datasisa);
				dataIni=datasisa;
			}
			let total     = getNum(hargaSat*dataIni).toFixed(2);
			$('#harga_tot9'+dataNomor).val((total));
			fnAlltotal9();
		});
		...
		$(document).on('blur', '#persen_retensi', function(){
		...
		$(document).on('blur', '#potongan_retensi', function(){
			...
			var datRetensi         	= $('#potongan_retensi').val();
			persenret=(parseFloat(datRetensi)/parseFloat(grand_total)*100).toFixed(2);
			$('#persen_retensi').val(num2(persenret));
			ppn();
		});
		$(document).on('blur', '#persen_retensi2', function(){
			var grand_total_hidden	= $("#grand_total").val();
		...
		$(document).on('blur', '#potongan_retensi2', function(){
			var grand_total	= $("#grand_total").val();
			var datRetensi         	= $('#potongan_retensi2').val();
			persenret=(parseFloat(datRetensi)/parseFloat(grand_total)*100).toFixed(2);
			$('#persen_retensi2').val(num2(persenret));
			ppn();
		});
