		...
		foreach($getDetail AS $val => $valx){ $numb++;
			...
			<tr id='tr_<?= $numb;?>' >
				...
				<td>
					...
					<input type="text" class="form-control input-sm text-right" id="harga_sat_<?= $numb;?>" name="data1[<?=$numb ?>][harga_sat]" value="<?=set_value('harga_sat', isset($harga_sat) ? $harga_sat : '0'); ?>" readonly  tabindex="-1">
				</td>
				...
				<td><input type="text" class="form-control input-sm qty_product text-center" id="qty_<?= $numb;?>" data-nomor='<?=$numb ?>' name="data1[<?=$numb ?>][qty]" value="<?=$valx['qty_delivery']; ?>" readonly tabindex="-1"></td>
				...
				<td>
					<input type="text" class="form-control text-right input-sm divide" id="harga_tot_<?=$numb ?>" name="data1[<?=$numb ?>][harga_tot]" value="<?=$harga_tot; ?>" readonly tabindex="-1">
				</td>
			</tr>
		<?php
		}
		...
		<tbody>
			...
			foreach($getEngCost AS $val => $valx){
				...
				<tr id='tr3_<?= $numb4;?>' >
					...
					<td colspan='11'>
						<input type="text" class="form-control input-sm" id="material_name4_<?=$numb4 ?>" name="data4[<?=$numb4 ?>][material_name4]" value="<?=set_value('material_name4', isset($material_name4) ? $material_name4 : ''); ?>" readonly tabindex="-1" >
					</td>
		...
		<tbody>
			...
			foreach($getPackCost AS $val => $valx){
				...
				<tr id='tr4_<?= $numb5;?>' >
					...
					<td colspan='11'>
						<input type="text" class="form-control input-sm" id="material_name5_<?=$numb5 ?>" name="data5[<?=$numb5 ?>][material_name5]" value="<?=set_value('material_name5', isset($material_name5) ? $material_name5 : ''); ?>" readonly tabindex="-1" >
					</td>
		...
		<tbody>
			...
			foreach($getTruck AS $val => $valx){
				...
				<tr id='tr5_<?= $nomor;?>' >
					...
					<td colspan='11'>
						<input type="text" class="form-control input-sm" id="material_name6_<?=$nomor ?>" name="data6[<?=$nomor ?>][material_name6]" value="<?=set_value('material_name6', isset($material_name6) ? $material_name6 : ''); ?>" readonly tabindex="-1" >
					</td>
	...
	if(!empty($other)){
		...
		echo "<tbody class='body_x'>";
			...
			foreach($other AS $val => $valx){
				...
				$checked="";$readonly=" disabled";
				if(isset($valx['total_deal_idr'])){
					$harga_sat9= round($valx['total_deal_idr']/$valx['qty_total'],2);
				}else{
					$harga_sat9= round($valx['harga_satuan_idr'],2);
				}
				if(isset($valx['checked'])){
					if($valx['checked']!="") { 
						$checked="checked";$readonly="";
	...
	<tfoot>
		...
		<tr class='HeaderHr'>
			...
			<td align='right' style='text-align:center;' colspan='2'>
				<?php
					$grand_total = round($SUM + $SUM2 + $SUM3 + $SUM1 + $SUM_MAT + $SUM_NONFRP + $SUM_OTHER, 2);
		...
		<tr class='HeaderHr hidden'>
			...
			<td align='center' colspan='2'></td>
			<td align='right' colspan='2'>
				<input type="text" class="form-control down_payment2 text-right input-sm divide" id="down_payment2" name="down_payment2" value="<?=$down_payment2; ?>" tabindex="-1" readonly >
			</td>
		</tr>
		<tr class='HeaderHr hidden'>
		...
		$ret       =  $this->db->select('SUM(retensi_um_idr) AS retensi_um_idr')->where_in('no_ipp',$arr_in_ipp)->get('billing_so')->row();
		...
		<tr class='HeaderHr'>
			...
			<td align='right' colspan='2'>
				...
				if(isset($penagihan[0]->total_invoice)){
					...
					$grand_total = round($SUM + $SUM2 + $SUM3 + $SUM1 + $SUM_MAT + $SUM_NONFRP + $SUM_OTHER - ($down_payment + $down_payment2), 2);
				}
				?>
