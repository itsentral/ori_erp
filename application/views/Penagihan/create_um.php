...
<form action="#" method="POST" id="form_proses" enctype="multipart/form-data" autocomplete='off'> 
	<div class="box box-primary">
		...
		<div class="box-body">
			...
			<?php 
			$um1 = $uang_muka_persen;

			if($um1 < 1){?>
				<div class='form-group row'>
					  <label for="jenis_invoice" class="col-sm-2 control-label">Persentase UM I <span class='text-red'>*</span></font></label>
					  <div class="col-sm-4">
						 <input type="text" name="um_persen" id="um_persen" class="form-control input-md autoNumeric" value="">
					  </div>
				</div>
			<?php 
			}
			...
			<table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
				<thead>
					<tr class='bg-blue'>
						<td class="text-left" colspan='13'><b>PRODUCT</b></td>
					</tr>
					<tr class='bg-blue'>
				...
				<tbody>
					...
					foreach($getDetail AS $val => $valx){ $numb++;
						...
						<tr id='tr_<?= $numb;?>' >
							...
							<td><input type="text" class="form-control input-md qty_product text-center maskMoney" id="qty_<?= $numb;?>" data-nomor='<?=$numb ?>' name="data1[<?=$numb ?>][qty]" readonly data-decimal='.' data-thousand='' data-precision='0' data-allow-zero=''  value="<?=$sisa_inv; ?>" ></td>
					...
					echo "<tbody class='body_x'>";
						...
						foreach($non_frp AS $val => $valx){
							...
							<tr id='tr1_<?= $numb2;?>' >
								...
								<td>
								   <input type="text" class="form-control qty_bq text-right maskMoney" data-nomor='<?=$numb2 ?>' readonly id="qty2" name="data2[<?=$numb2 ?>][qty2]" value="<?=set_value('qty2', isset($valx['qty']) ? $valx['qty'] : ''); ?>" >
								</td>
								<td>
									<?php
								...
								<td>
									...
									?>
									<input type="text" class="form-control text-right" readonly id="harga_sat2<?=$numb2 ?>" name="data2[<?=$numb2 ?>][harga_sat2]" value="<?=set_value('harga_sat2', isset($harga_sat2) ? $harga_sat2 : ''); ?>" readonly >
						...
						?>
						<tr class='FootColor'>
							<td colspan='11'><b>TOTAL BILL OF QUANTITY NON FRP</b></td>
							<td align='center'></td> 
							<td align="right">
								<?php
					...
					echo "<tbody class='body_x'>";
						...
						foreach($material AS $val => $valx){
							...
							<tr id='tr2_<?= $numb3;?>' >
								...
								<td>
								   <input type="text" class="form-control qty_material text-right maskMoney" readonly id="qty3_<?= $numb3;?>" data-nomor='<?=$numb3 ?>'  name="data3[<?=$numb3 ?>][qty3]" value="<?=set_value('qty3', isset($valx['qty']) ? $valx['qty'] : ''); ?>" >
								</td>
								<td>
									<?php
						...
						?>
						<tr class='FootColor'>
							<td colspan='12'><b>TOTAL MATERIAL</b></td>
							<td align="right">
								<?php
						...
						foreach($getEngCost AS $val => $valx){
							...
							<tr id='tr3_<?= $numb4;?>' >
								...
								<td>
									...
									<input type="text" class="form-control text-right harga_tot4 changeAll maskMoney" readonly id="harga_tot4<?=$numb4 ?>" data-nomor='<?=$numb4 ?>' name="data4[<?=$numb4 ?>][harga_tot4]" value="<?=set_value('harga_tot4', isset($harga_tot4) ? $harga_tot4 : ''); ?>" >
						...
						foreach($getPackCost AS $val => $valx){
							...
							<tr id='tr4_<?= $numb5;?>' >
								...
								<td>
									...
									<input type="text" class="form-control text-right  harga_tot5 maskMoney"  readonly id="harga_tot5<?=$numb5 ?>" data-nomor='<?=$numb5 ?>' name="data5[<?=$numb5 ?>][harga_tot5]" value="<?=set_value('harga_tot5', isset($harga_tot5) ? $harga_tot5 : ''); ?>" >
				...
				if(!empty($getTruck)){
					...
					<tbody>
						...
						<tr id='tr5X' class='FootColor'>
							<td colspan='12'><b>TOTAL TRUCKING</b></td>
							<td align="right">
								<?php
				...
				<tfoot>
					<tr class='HeaderHr'>
						<td align='right' colspan='13' height='20px;'></td>
					</tr>
					<tr class='HeaderHr'>
					...
					?>
					<tr class='HeaderHr'>
						<td align='right' colspan='11'><b>DOWN PAYMENT I</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
						<td align='center'></td>
						<td align='right'>
					...
					<tr class='HeaderHr'>
						...
						<td align='right'>
							<?php 
							$grand_total = number_format($SUM + $SUM2 + $SUM3 + $SUM1 + $SUM_MAT - $down_payment_hidden, 2);
							$grand_total_hidden = round($SUM + $SUM2 + $SUM3 + $SUM1 + $SUM_MAT - $down_payment_hidden, 2);
							?>
