<table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
	...
	<tbody>
		...
		$numb=0;
		$SUM = 0;$total_cogs=0;
		foreach($getDetail AS $val => $valx){ $numb++;$checked="";$readonly=" disabled";
			$pr		= 'pr';
			$numb1	= $pr.$numb;
			...
			$harga_sat=0;
			if($valx['qty']>0) $harga_sat	= round($dataSum / $valx['qty'],2);
...
//			$SUM += ($harga_sat * $sisa_inv);
		...
		foreach($getTankiproduct AS $val => $valx){ $numb++;$checked="";$readonly=" disabled";
			$pr		= 'pr';
			$numb1	= $pr.$numb;
...
//			$SUM += ($harga_sat * $sisa_inv);
			...
			$spesifikasi='';
			$get_bqHead = $this->db->get_where('tanki.bq_detail_header', array('no_ipp'=>$valx['no_ipp'],'category'=>'frp'))->result_array();
			if(!empty($get_bqHead))  {
				$spesifikasi=number_format($get_bqHead[0]['length']).' X '
				.number_format($get_bqHead[0]['wide']).' X '
				.number_format($get_bqHead[0]['high']);
			}
			?>
			<tr id='tr_<?= $numb;?>' >
				<td align='center'>
