	...
	if(!empty($non_frp)){
		echo "<tbody>";
			echo "<tr class='bg-blue'>";
				echo "<td class='text-left headX HeaderHr' colspan='15'><b>BILL OF QUANTITY NON FRP IDR</b></td>";
			echo "</tr>";
			echo "<tr class='bg-blue'>";
	...
	if(!empty($material)){
		...
		echo "<tbody class='body_x'>";
			...
			foreach($material AS $val => $valx){
				...
				$harga_tot3=$valx['harga_total_idr'];
				$SUM_MAT += ($harga_tot3);
				...
				<tr id='tr2_<?= $numb3;?>' >
					...
					<td>
						<input type="text" class="form-control text-right input-sm divide amount3" id="harga_tot3<?=$numb3 ?>" name="data3[<?=$numb3 ?>][harga_tot3]" value="<?=$harga_tot3?>" readonly tabindex="-1" >
					</td>
				</tr>
			<?php
			}
	...
	if(!empty($getPackCost)){
		...
		<tbody>
			...
			foreach($getPackCost AS $val => $valx){
				...
				$harga_tot5=round($harga_sisa_so,2);
				$SUM2 += ($harga_tot5);
