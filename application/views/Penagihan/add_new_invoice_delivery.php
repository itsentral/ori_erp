		...
		<div class="box-body">
			...
			<div class='form-group row'>
				...
				<div class='col-sm-2'>
					...
					<input type="hidden" id='wilayah' name="wilayah" class="form-control input-sm" value="<?= if(!empty(get_name('so_number','wilayah','id_bq', "BQ-".$getHeader[0]->no_ipp))){get_name('so_number','wilayah','id_bq', "BQ-".$getHeader[0]->no_ipp)}else{'-'};?>">
				</div>
				<div class='col-sm-2'>
...
<?php
			if ($base_cur=='USD'){
				$this->load->view('Penagihan/add_progress_usd_delivery');
			}else{
				$this->load->view('Penagihan/add_progress_idr_delivery');
			}
?>
			<?php
				if(isset($approval)){
			...
			if ($('#tgl_inv').val() == "") {
				...
				function(isConfirm) {
					if (isConfirm) {
					  ...
					  var baseurl=base_url + active_controller +'/create_progress_new_delivery';
					  $.ajax({
						url			: baseurl,
						type		: "POST",
						data		: formData,
