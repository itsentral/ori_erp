<table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
	...
	<tbody>
		...
		foreach($getDetail AS $val => $valx){ $numb++;
			...
			$pr		= 'pr';
			$numb1	= $pr.$numb;
			if(isset($valx['harga_total_idr'])){
				$HrgTot	= $valx['harga_total_idr'];
				$dataSum = $HrgTot;
				$unitT = $valx['unit'];
				$sisa_inv = $valx['qty_delivery'];
				$harga_sat	= round($valx['harga_satuan_idr'],2);
				$harga_tot	= round($valx['harga_total_idr'],2);
//				$SUM += ($harga_tot);
			}else{
				if($valx['qty'] <> 0){
					$HrgTot  	= $valx['total_deal_idr'];
					$dataSum	= $HrgTot;
				}
...
//				$SUM += ($harga_tot);
			...
			if(isset($valx['checked'])){
				if($valx['checked']!="") { 
					...
					$harga_tot=round($harga_sat*$valx['qty_delivery'],2);
					$SUM += ($harga_tot);
				}
			}
			?>
			<tr id='tr_<?= $numb;?>' >
				<td align='center'>
					<input type="checkbox" <?=$checked?> id="ck_<?= $numb;?>" name="tr1_<?= $numb;?>" value="<?= $numb;?>" onclick="showdata('',<?= $numb;?>)">
				...
				<td><input type="text" class="form-control input-sm qty_product text-center" id="qty_<?= $numb;?>" data-nomor='<?=$numb ?>' name="data1[<?=$numb ?>][qty]" value="<?=$valx['qty_delivery']; ?>" <?=$readonly?>></td>
		...
		//tanki
		foreach($getTankiproduct AS $val => $valx){ $numb++;
			$checked="";$readonly=" disabled";
			$dataSum=0;
			$pr		= 'pr';
			$numb1	= $pr.$numb;
...
//				$SUM += ($harga_tot);
			...
			$spesifikasi='';
			$get_bqHead = $this->db->get_where('bq_detail_header', array('id_bq'=>$valx['no_ipp'],'id_category'=>'frp'))->result();
			if(!empty($get_bqHead))  {
			...
			<tr id='tr_<?= $numb;?>' >
				<td align='center'>
					...
					<input type="hidden" id="ck_<?= $numb;?>" name="tr1_<?= $numb;?>" value="<?= $numb;?>">
					<input type="hidden" id="data_ipp_<?=$numb?>" name="data1[<?=$numb?>][no_ipp]" value="<?=$valx['no_ipp']?>">
					<input type="hidden" id="data_so_<?=$numb?>" name="data1[<?=$numb?>][no_so]" value="<?=get_nomor_so_tanki($valx['no_ipp'])?>">
					<input type="hidden" id="data_idmilik_<?=$numb?>" name="data1[<?=$numb?>][id_milik]" value="<?=$valx['id_milik']?>">
					<input type="hidden" id="data_cogs_<?=$numb?>" name="data1[<?=$numb?>][cogs]" value="<?=$valx['cogs']?>">
				</td>
				<td><input type="text" class="form-control input-sm" id="material_name1_<?= $numb;?>" name="data1[<?=$numb ?>][material_name1]" value="<?=strtoupper(str_replace('"','',$valx['product'])); ?>" readonly tabindex="-1"></td>
				<td><input type="text" class="form-control input-sm" id="product_cust<?= $numb;?>" name="data1[<?=$numb ?>][product_cust]" value="<?=strtoupper(str_replace('"','',$valx['customer_item'])); ?>" readonly tabindex="-1"></td>
				<td><input type="text" class="form-control input-sm" id="product_desc<?= $numb;?>" name="data1[<?=$numb ?>][product_desc]" value="<?=strtoupper(str_replace('"','',$valx['desc'])); ?>" readonly tabindex="-1"></td>
				<td><input type="text" class="form-control input-sm text-right" id="diameter_1_<?= $numb;?>" name="data1[<?=$numb ?>][diameter_1]" value="" readonly  tabindex="-1"></td>
				<td><input type="text" class="form-control input-sm text-right" id="diameter_2_<?= $numb;?>" name="data1[<?=$numb ?>][diameter_2]" value="" readonly  tabindex="-1"></td>
				<td><input type="text" class="form-control input-sm text-right" id="liner_<?= $numb;?>" name="data1[<?=$numb ?>][liner]" value="" readonly  tabindex="-1"></td>
				<td><input type="text" class="form-control input-sm text-center" id="pressure_<?= $numb;?>" name="data1[<?=$numb ?>][pressure]" value="" readonly  tabindex="-1"></td>
				<td><input type="text" class="form-control input-sm" id="id_milik_<?= $numb;?>" name="data1[<?=$numb ?>][spesifikasi]" value="<?=$spesifikasi; ?>" readonly  tabindex="-1"></td>
				<td>
					<input type='hidden' name="data1[<?=$numb ?>][id]" value='<?=$valx['id'];?>'>
					<input type="text" class="form-control input-sm text-right divide" id="harga_sat_<?= $numb;?>" name="data1[<?=$numb ?>][harga_sat]" value="<?=set_value('harga_sat', isset($harga_sat) ? $harga_sat : '0'); ?>" readonly  tabindex="-1">
				</td>
	...
	if(!empty($non_frp)){
		...
		echo "<tbody class='body_x'>";
			...
			foreach($non_frp AS $val => $valx){
				$numb2++;$harga_tot2=0;
				$checked="";$readonly=" disabled";
				if(isset($valx['total_deal_idr'])){
					$harga_sat2= round($valx['total_deal_idr']/$valx['qty_total'],2);
				}else{
	...
	if(!empty($material)){
		...
		echo "<tbody class='body_x'>";
			...
			foreach($material AS $val => $valx){
				...
				$checked="";$readonly=" disabled";
				if(isset($valx['total_deal_idr'])){
					$harga_sat3= round($valx['total_deal_idr']/$valx['qty_total'],2);
				}else{
					$harga_sat3= round($valx['harga_satuan_idr'],2);
				}
				if(isset($valx['checked'])){
					if($valx['checked']!="") { 
						$checked="checked";$readonly="";
				...
				<tr id='tr2_<?= $numb3;?>' >
					...
					<td>
						<input type="text" class="form-control text-right input-sm divide" id="harga_sat3_<?= $numb3;?>" name="data3[<?=$numb3 ?>][harga_sat3]" value="<?=set_value('harga_sat3', isset($harga_sat3) ? $harga_sat3 : '0'); ?>" readonly tabindex="-1" >
					</td>
					<td>
	...
	?>
		...
		<tbody>
			...
			foreach($getEngCost AS $val => $valx){
				...
				$harga_total_so=(isset($valx['total_deal_idr'])?$valx['total_deal_idr']:$valx['harga_total_so']);
				$harga_sisa_so=(isset($valx['total_deal_idr'])?($valx['total_deal_idr']-$valx['total_deal_inv_idr']):$valx['harga_sisa_so']);
 				if(isset($valx['checked'])){
					if($valx['checked']!="") { 
						$checked="checked";$readonly="";
						$harga_tot4= round($valx['harga_total_idr'],2);
						$SUM1 += ($harga_tot4);
					}
				}
	...
	$SUM2=0;
	if(!empty($getPackCost) || !empty($getTankipacking)){
		?>
		<thead>
			<tr class='bg-blue'>
		...
		<tbody>
			...
			foreach($getPackCost AS $val => $valx){
				...
				$readonly=" disabled";
				$harga_total_so=(isset($valx['total_deal_idr'])?$valx['total_deal_idr']:$valx['harga_total_so']);
				$harga_sisa_so=(isset($valx['total_deal_idr'])?($valx['total_deal_idr']-$valx['total_deal_inv']):$valx['harga_sisa_so']);

 				if(isset($valx['checked'])){
					if($valx['checked']!="") { 
						$checked="checked";$readonly="";
						$harga_tot5= round($valx['harga_total_idr'],2);
						$SUM2 += $harga_tot5;
					}
				}
			...
			foreach($getTankipacking AS $val => $valx){
				$numb5++;$checked='';$harga_tot5=0;
				$readonly=" disabled";
				$harga_total_so=(isset($valx['total_deal_idr'])?$valx['total_deal_idr']:$valx['harga_total_so']);
				$harga_sisa_so=(isset($valx['total_deal_idr'])?($valx['total_deal_idr']-$valx['total_deal_inv']):$valx['harga_sisa_so']);
				...
				$material_name5= strtoupper($valx['product']);
				$unit5= strtoupper('-');
				?>
				<tr id='tr4_<?= $numb5;?>' >
					<td align='center'>
						<input type="checkbox" <?=$checked ?> id="ck4_<?= $numb5;?>" name="tr5_<?= $numb5;?>" value="<?= $numb5;?>" onclick="showdata('4',<?= $numb5;?>)">
						<input type="hidden" id="data4_ipp_<?=$numb5?>" name="data5[<?=$numb5?>][no_ipp]" value="<?=$valx['no_ipp']?>">
						<input type="hidden" id="data4_so_<?=$numb5?>" name="data5[<?=$numb5?>][no_so]" value="<?=get_nomor_so_tanki($valx['no_ipp'])?>">
						<input type="hidden" id="data4_idmilik_<?=$numb5?>" name="data5[<?=$numb5?>][id_milik]" value="<?=$valx['id_milik']?>">
					</td>
					<td colspan='11'>
						<input type="text" class="form-control input-sm" id="material_name5" name="data5[<?=$numb5 ?>][material_name5]" value="<?=set_value('material_name5', isset($material_name5) ? $material_name5 : ''); ?>" readonly tabindex="-1" >
	...
	//TRUCKING
	$SUM3=0;
	if(!empty($getTruck) || !empty($getTankishipping)){
		?>
		<tbody>
			<tr class='bg-blue'>
		...
		<tbody>
			...
			foreach($getTruck AS $val => $valx){
				...
				$harga_sisa_so=(isset($valx['total_deal_idr'])?($valx['total_deal_idr']-$valx['total_deal_inv']):$valx['harga_sisa_so']);;
 				...
 				if(isset($valx['checked'])){
					if($valx['checked']!="") { 
						$checked="checked";$readonly="";
						$harga_tot6	= round($valx['harga_total_idr'],2);
						$SUM3 += $harga_tot6;
					}
				}
				?>
			...
			foreach($getTankishipping AS $val => $valx){
				$nomor++;$harga_tot6=0;
				$material_name6 = strtoupper($valx['product']);
				$unit6			= strtoupper('-');
				$harga_total_so=(isset($valx['total_deal_idr'])?$valx['total_deal_idr']:$valx['harga_total_so']);
				$harga_sisa_so=(isset($valx['total_deal_idr'])?($valx['total_deal_idr']-$valx['total_deal_inv']):$valx['harga_sisa_so']);;
				...
				<tr id='tr5_<?= $nomor;?>' >
					<td align='center'>
						<input type="checkbox" <?=$checked ?> id="ck5_<?= $nomor;?>" name="tr6_<?= $nomor;?>" value="<?= $nomor;?>" onclick="showdata('5',<?= $nomor;?>)">
						<input type="hidden" id="data5_ipp_<?=$nomor?>" name="data6[<?=$nomor?>][no_ipp]" value="<?=$valx['no_ipp']?>">
						<input type="hidden" id="data5_so_<?=$nomor?>" name="data6[<?=$nomor?>][no_so]" value="<?=get_nomor_so_tanki($valx['no_ipp'])?>">
						<input type="hidden" id="data5_idmilik_<?=$nomor?>" name="data6[<?=$nomor?>][id_milik]" value="<?=$valx['id_milik']?>">
					</td>
					<td colspan='11'>
						<input type="text" class="form-control input-sm" id="material_name6" name="data6[<?=$nomor ?>][material_name6]" value="<?=set_value('material_name6', isset($material_name6) ? $material_name6 : ''); ?>" readonly tabindex="-1" >
	...
	<tfoot>
		...
		if(isset($penagihan[0]->total_retensi)){
			$total_retensi=$penagihan[0]->total_retensi_idr;
			$persen_retensi=$penagihan[0]->persen_retensi;
		}else{
			$total_retensi=0;
			$persen_retensi=0;
		}
		$ret       =  $this->db->select('SUM(retensi_um_idr) AS retensi_um_idr')->where_in('no_ipp',$arr_in_ipp)->get('billing_so_gabung')->row();
		$retum 		= $ret->retensi_um_idr;
		?>
		<tr class='HeaderHr'>
		...
		if(isset($penagihan[0]->total_retensi2)){
			$retum=$penagihan[0]->total_retensi2_idr;
			$persen_retensi2=$penagihan[0]->persen_retensi2;
		}else{
			$ret       =  $this->db->select('SUM(retensi_um) AS retensi_um')->where_in('no_ipp',$arr_in_ipp)->get('billing_so_gabung')->row();
