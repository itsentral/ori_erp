...
<script>
	...
	$(document).on('click', '.request', function(e){
		e.preventDefault();
		loading_spinner();
		$("#head_title").html("<b>REQUEST LIST ITEM PROJECT</b>");
		$.ajax({
			type:'POST',
			url: base_url+active_controller+'/add/'+$(this).data('kode'),
	...
	$(document).on('click', '#btnRequestSub', function(e){
		e.preventDefault();
		let no_surat_jalan_req = $('#no_surat_jalan_req').val()


		if(no_surat_jalan_req == ''){
			swal({
			  title	: "Error Message!",
			  text	: 'No surat jalan masih kosong !',
			  type	: "warning"
			});
			return false;
		}
		...
		function(isConfirm) {
			if (isConfirm) {
				...
				var formData  	= new FormData($('#form_proses')[0]);
				$.ajax({
					url			: base_url + active_controller+'/addSubgudang',
					type		: "POST",
					data		: formData,
					cache		: false,
					dataType	: 'json',
