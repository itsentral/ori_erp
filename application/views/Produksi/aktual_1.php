	...
	<div class="box-body">
        <div class="form-group row">
            ...
            <div class='col-sm-7 '>
                ...
                foreach ($hist_produksi as $key => $value) {
					...
					$input_produksi 	= ($value['created_date'] == $value['updated_date'])?"":' (DONE)';
        ...
        <div class="form-group row">
            ...
            <div class='col-sm-3 '>
				...
				<input type="hidden" name='id_milik2' id='id_milik2'>
            </div>
            <div class='col-sm-7 text-right'>
                <button type='button' id='back' class='btn btn-md btn-danger' style='float:right; margin-left:5px;'>Back</button>
				<button type='button' id='update_real_spk1_closing' class='btn btn-md btn-primary' style='float:right; margin-left:5px;'>Closing</button>
                <button type='button' id='update_real_spk1' class='btn btn-md btn-success' style='float:right;'>Update</button>
...
<script>
                    ...
                    $.ajax({
                        ...
                        success		: function(data){
                            if(data.status == 1){
                                ...
                                window.location.href = base_url + active_controller+"/aktual_1/"+data.kode_spk;
                            }
    ...
    $(document).on('click', '#show_material', function(e){
		...
		loading_spinner();
		var formData 	=new FormData($('#form_proses_bro')[0]);
		var baseurl=base_url + active_controller +'/show_material_input';
		$.ajax({
			url			: baseurl,
			type		: "POST",
			data		: formData,
            ...
            $.ajax({
                ...
                success		: function(data){
                    if(data.status == 1){
                        ...
                        let linkDownload = '';
                        if(data.upload_eng_change1 != ''){
                            linkDownload = `<a href='${data.upload_eng_change1}' target='_blank'>Download</a> `;
                        }
	...
	$(document).on('click', '#update_real_spk1_closing', function(e){
		e.preventDefault();
		$(this).prop('disabled',true);
		...
		swal({
			...
			function(isConfirm) {
				if (isConfirm) {
					loading_spinner();
					var formData 	=new FormData($('#form_proses_bro')[0]);
					var baseurl=base_url + active_controller +'/<?=$link_submit_close;?>';
					$.ajax({
						url			: baseurl,
						type		: "POST",
						data		: formData,
    ...
    $(document).on('change', '.changeOB', function(e){
        let parentHTML = $(this).parent().parent()
