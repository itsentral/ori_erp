        ...
        <tbody>
            <?php
                ...
                foreach ($get_liner_utama as $key => $value) { $nomor++;
                    ...
                    $get_edit = $this->db->group_by('id_material')->limit(1)->select('id_material, SUM(aktual_total) AS aktual_total')->get_where('production_spk_input',array('id_material'=>$value['id_material'],'layer'=>'LINER THIKNESS / CB','spk'=>'1','kode_spk'=>$kode_spk,'status_date'=>$hist_produksi))->result();
                    $terpakai = (!empty($get_edit))?$get_edit[0]->aktual_total:'';
                    $material   = (!empty($get_edit))?$get_edit[0]->id_material:$value['id_material'];

                    $get_edit2 = $this->db->group_by('id_material')->limit(1)->select('id_material, SUM(aktual_total) AS aktual_total')->where_in('id_spk',$id_spk)->get_where('production_spk_input',array('id_material'=>$value['id_material'],'layer'=>'LINER THIKNESS / CB','spk'=>'1','kode_spk'=>$kode_spk))->result();
        ...
        <tbody>
            <?php
                ...
                foreach ($get_str_n1_utama as $key => $value) { $nomor++;
                    $list_material = $this->db->get_where('raw_materials', array('id_category'=>$value['id_category'],'delete'=>'N'))->result_array();
                    $get_edit = $this->db->group_by('id_material')->limit(1)->select('id_material, SUM(aktual_total) AS aktual_total')->get_where('production_spk_input',array('id_material'=>$value['id_material'],'layer'=>'STRUKTUR NECK 1','spk'=>'1','kode_spk'=>$kode_spk,'status_date'=>$hist_produksi))->result();
                    $terpakai = (!empty($get_edit))?$get_edit[0]->aktual_total:'';
                    $material   = (!empty($get_edit))?$get_edit[0]->id_material:$value['id_material'];

                    $get_edit2 = $this->db->group_by('id_material')->limit(1)->select('id_material, SUM(aktual_total) AS aktual_total')->where_in('id_spk',$id_spk)->get_where('production_spk_input',array('id_material'=>$value['id_material'],'layer'=>'STRUKTUR NECK 1','spk'=>'1','kode_spk'=>$kode_spk))->result();
                    $terpakai2 = (!empty($get_edit2))?$get_edit2[0]->aktual_total:0;
                    ?>
                    <tr>
                        <td class='text-center'><?=$nomor;?></td>
        ...
        <tbody>
            <?php
                ...
                foreach ($get_str_n2_utama as $key => $value) { $nomor++;
                    $list_material = $this->db->get_where('raw_materials', array('id_category'=>$value['id_category'],'delete'=>'N'))->result_array();
                    $get_edit = $this->db->group_by('id_material')->limit(1)->select('id_material, SUM(aktual_total) AS aktual_total')->get_where('production_spk_input',array('id_material'=>$value['id_material'],'layer'=>'STRUKTUR NECK 2','spk'=>'1','kode_spk'=>$kode_spk,'status_date'=>$hist_produksi))->result();
                    $terpakai = (!empty($get_edit))?$get_edit[0]->aktual_total:'';
                    $material   = (!empty($get_edit))?$get_edit[0]->id_material:$value['id_material'];

                    $get_edit2 = $this->db->group_by('id_material')->limit(1)->select('id_material, SUM(aktual_total) AS aktual_total')->where_in('id_spk',$id_spk)->get_where('production_spk_input',array('id_material'=>$value['id_material'],'layer'=>'STRUKTUR NECK 2','spk'=>'1','kode_spk'=>$kode_spk))->result();
                    $terpakai2 = (!empty($get_edit2))?$get_edit2[0]->aktual_total:0;
                    ?>
                    <tr>
                        <td class='text-center'><?=$nomor;?></td>
        ...
        <tbody>
            <?php
                ...
                foreach ($get_structure_utama as $key => $value) { $nomor++;
                    $list_material = $this->db->get_where('raw_materials', array('id_category'=>$value['id_category'],'delete'=>'N'))->result_array();
                    $get_edit = $this->db->group_by('id_material')->limit(1)->select('id_material, SUM(aktual_total) AS aktual_total')->get_where('production_spk_input',array('id_material'=>$value['id_material'],'layer'=>'STRUKTUR THICKNESS','spk'=>'1','kode_spk'=>$kode_spk,'status_date'=>$hist_produksi))->result();
                    $terpakai = (!empty($get_edit))?$get_edit[0]->aktual_total:'';
                    $material   = (!empty($get_edit))?$get_edit[0]->id_material:$value['id_material'];

                    $get_edit2 = $this->db->group_by('id_material')->limit(1)->select('id_material, SUM(aktual_total) AS aktual_total')->where_in('id_spk',$id_spk)->get_where('production_spk_input',array('id_material'=>$value['id_material'],'layer'=>'STRUKTUR THICKNESS','spk'=>'1','kode_spk'=>$kode_spk))->result();
                    $terpakai2 = (!empty($get_edit2))?$get_edit2[0]->aktual_total:0;
                    ?>
                    <tr>
                        <td class='text-center'><?=$nomor;?></td>
        ...
        <tbody>
            <?php
                ...
                foreach ($get_external_utama as $key => $value) { $nomor++;
                    $list_material = $this->db->get_where('raw_materials', array('id_category'=>$value['id_category'],'delete'=>'N'))->result_array();
                    $get_edit = $this->db->group_by('id_material')->limit(1)->select('id_material, SUM(aktual_total) AS aktual_total')->get_where('production_spk_input',array('id_material'=>$value['id_material'],'layer'=>'EXTERNAL LAYER THICKNESS','spk'=>'1','kode_spk'=>$kode_spk,'status_date'=>$hist_produksi))->result();
                    $terpakai = (!empty($get_edit))?$get_edit[0]->aktual_total:'';
                    $material   = (!empty($get_edit))?$get_edit[0]->id_material:$value['id_material'];

                    $get_edit2 = $this->db->group_by('id_material')->limit(1)->select('id_material, SUM(aktual_total) AS aktual_total')->where_in('id_spk',$id_spk)->get_where('production_spk_input',array('id_material'=>$value['id_material'],'layer'=>'EXTERNAL LAYER THICKNESS','spk'=>'1','kode_spk'=>$kode_spk))->result();
                    $terpakai2 = (!empty($get_edit2))?$get_edit2[0]->aktual_total:0;
                    ?>
                    <tr>
                        <td class='text-center'><?=$nomor;?></td>
        ...
        <tbody>
            <?php
                ...
                foreach ($get_topcoat_utama as $key => $value) { $nomor++;
                    $list_material = $this->db->get_where('raw_materials', array('id_category'=>$value['id_category'],'delete'=>'N'))->result_array();
                    $get_edit = $this->db->group_by('id_material')->limit(1)->select('id_material, SUM(aktual_total) AS aktual_total')->get_where('production_spk_input',array('id_material'=>$value['id_material'],'layer'=>'TOPCOAT','spk'=>'1','kode_spk'=>$kode_spk,'status_date'=>$hist_produksi))->result();
                    $terpakai = (!empty($get_edit))?$get_edit[0]->aktual_total:'';
                    $material   = (!empty($get_edit))?$get_edit[0]->id_material:$value['id_material'];

                    $get_edit2 = $this->db->group_by('id_material')->limit(1)->select('id_material, SUM(aktual_total) AS aktual_total')->where_in('id_spk',$id_spk)->get_where('production_spk_input',array('id_material'=>$value['id_material'],'layer'=>'TOPCOAT','spk'=>'1','kode_spk'=>$kode_spk))->result();
                    $terpakai2 = (!empty($get_edit2))?$get_edit2[0]->aktual_total:0;
                    ?>
                    <tr>
                        <td class='text-center'><?=$nomor;?></td>
            ...
            <tbody>
                <?php
                    ...
                    foreach ($get_liner_utama as $key => $value) { $nomor++; $id_key++;
                        ...
                        $SQL1       = "SELECT id_material, id_material_aktual, SUM(aktual_total) AS aktual_total, persen FROM production_spk_input WHERE id_material='".$value['id_material']."' AND kode_spk='".$kode_spk."' AND status_date='".$hist_produksi."' AND spk = '1' AND (layer = 'LINER THIKNESS / CB' OR layer = 'RESIN AND ADD') GROUP BY id_material ";
                        ...
                        <tr>
                            ...
                            <td>
                                <select name='detail_liner[<?=$nomor;?>][actual_type]' class='form-control chosen_select id_material mat_<?=$material;?>' data-mat='<?=$material;?>'>
                                    ...
                                    <?php
                                        foreach($list_material AS $valMat => $valxMat){
                                            // if($valxMat['id_material'] == $material){
                                                $sel = ($valxMat['id_material'] == $material)?'selected':'';
                                                echo "<option value='".$valxMat['id_material']."' ".$sel.">".$valxMat['nm_material']."</option>";
                                            // }
                                        }
