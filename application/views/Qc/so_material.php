...
<script>
	$(document).ready(function(){
        ...
        $(document).on('click', '.check_real', function(e){
            ...
            $("#view2").load(base_url + active_controller + '/modal_qc_so_material/'+$(this).data('kode_trans'));
            $("#ModalView2").modal();
        });
            ...
            swal({
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        ...
                        var baseurl=base_url + active_controller +'/process_qc_so_material';
                        $.ajax({
                            url			: baseurl,
                            type		: "POST",
                            data		: formData,
                            ...
                            success		: function(data){
                                if(data.status == 1){
                                    ...
                                    window.location.href = base_url + active_controller+"/so_material";
                                }
            ...
            swal({
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        loading_spinner();
                        var formData 	=new FormData($('#form_proses_bro')[0]);
                        var baseurl=base_url + active_controller +'/process_reject_qc_so_material';
                        $.ajax({
                            url			: baseurl,
                            type		: "POST",
                            data		: formData,
