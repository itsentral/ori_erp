...
<div class="box box-primary">
	<div class="box-header">
		...
		<div class="box-tool pull-right">
            ...
            <a href="<?php echo site_url('qc/report') ?>" class="btn btn-md btn-warning">
                <i class="fa fa-replay"></i> TO - QC PIPE FITTING
            </a>
		</div><br><br>
        ...
        <table class="table table-sm table-bordered table-striped" id="my-grid2" width='100%'>
            <thead>
                <tr class='bg-blue'>
                    ...
                    <th class="text-center no-sort">Daycode</th>
...
<script>
	$(document).ready(function(){
		...
		$(document).on('click', '.edit', function(e){
            e.preventDefault();
            loading_spinner();
            $("#head_title2").html("<b>Edit Report QC</b>");
            $("#view2").load(base_url + active_controller + '/modalEditReportSpool/'+$(this).data('id'));
            $("#ModalView2").modal();
        });

		$(document).on('click', '.detail_spool', function(e){
            e.preventDefault();
            loading_spinner();
            $("#head_title3").html("<b>Detail Spool</b>");
            $("#view3").load(base_url + active_controller + '/modal_detail_spool/'+$(this).data('id'));
            $("#ModalView3").modal();
        });
	...
	$(document).on('click', '#loading_excel', function(e){
		...
		var Link	= base_url + active_controller +'/download_excel_qc_spool_report/'+tgl_awal+'/'+tgl_akhir;
		window.open(Link);
	});
		...
		swal({
			...
			function(isConfirm) {
			  if (isConfirm) {
					...
					var baseurl=base_url + active_controller +'/modalEditReportSpool';
					$.ajax({
						url			: baseurl,
						type		: "POST",
						data		: formData,
						...
						success		: function(data){
							if(data.status == 1){
								...
								window.location.href = base_url + active_controller + '/report_spool';
							}
							else{
								swal({
								  title	: "Save Failed!",
	...
	function DataTables2(status=null,no_ipp=null,no_spk=null,product=null,tgl_awal=null,tgl_akhir=null){
		var dataTable = $('#my-grid2').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/server_side_qc_report_spool',
				type: "post",
				data: function(d){
					d.status = status,
					d.no_ipp = no_ipp,
