	...
	<table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
		<thead id='head_table'>
			<tr class='bg-blue'>
                ...
                <th class="text-center" style='vertical-align:middle;'>Qty In</th>
                <th class="text-center" style='vertical-align:middle;'>Qty Out</th>
		...
		<tbody>
			...
			foreach($result AS $val => $valx){
				...
				echo "<tr>";
					echo "<td>".$No."</td>";
					echo "<td>".strtoupper($valx['kode_trans'])."</td>";
                    ...
                    echo "<td class='text-right ".$color."'>".number_format($valx['qty_in'],4)."</td>";
                    echo "<td class='text-right ".$color."'>".number_format($valx['qty_out'],4)."</td>";					
					echo "<td class='text-right ".$color."'>".number_format($valx['qty_stock_akhir'],4)."</td>";
					echo "<td class='".$color."'>".strtoupper($valx['ket'])."</td>";
					echo "<td class='text-right'>".date('d-M-Y H:i:s', strtotime($valx['tgl_trans']))."</td>";
					if($this->uri->segment(5) =='1'){
					echo "<td class='text-right ".$color."'>".number_format($valx['harga'],2)."</td>";
                    echo "<td class='text-right ".$color."'>".number_format($valx['nilai_awal_rp'],2)."</td>";
					echo "<td class='text-right ".$color."'>".number_format($valx['nilai_trans_rp'],2)."</td>";
					echo "<td class='text-right ".$color."'>".number_format($valx['nilai_akhir_rp'],2)."</td>";
					}
				echo "</tr>";
			}
            if(empty($result)){
