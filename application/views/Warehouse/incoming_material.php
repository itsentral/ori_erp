			...
			<div class="box-body">
				...
				<div class='in_ipp'>
					<div class='form-group row'>
						<label class='label-control col-sm-2'><b>Nomor PO/NON-PO</b></label>
						<div class='col-sm-4'>
							<select id='no_ipp' name='no_ipp' class='form-control input-sm' style='min-width:200px;'>
								...
								<?php
									foreach($no_po AS $val => $valx){
										...
										echo "<option value='".$valx['no_po']."' ".$disabled.">".strtoupper($valx['ket_'])." - ".strtoupper($valx['no_po'].$label_add)." - ".$valx['nm_supplier']."</option>";
									}
								?>
							</select>
						</div>
				...
				$.ajax({
					...
					success		: function(data){
						if(data.status == 1){
							...
							window.location.href = base_url + active_controller+'/incoming_material';
						}
						else if(data.status == 0){
							swal({
								title	: "Save Failed!",
