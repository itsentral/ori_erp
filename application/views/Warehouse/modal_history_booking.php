	...
	<table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
		...
		<tbody>
			...
			foreach($result AS $val => $valx){
				...
				$dari_gudang 	= (!empty($gudang_dari))?$gudang_dari:$valx['kd_gudang_dari'];
				$ke_gudang 		= $valx['kd_gudang_ke'];
				...
				echo "<tr>";
					...
					echo "<td class='text-center'>".strtoupper($valx['no_ipp'])."</td>";
					echo "<td class='text-center'>".strtoupper(get_name('so_number','so_number','id_bq','BQ-'.$valx['no_ipp']))."</td>";
					...
					<select name="sales_order" id="sales_order" class='from-control chosen-select'>
						...
						<?php
							foreach ($listSO as $key => $value) {
								$NO_SO = (!empty($GET_SO[$value['no_ipp']]['so_number']))?$GET_SO[$value['no_ipp']]['so_number']:'Not Found';
								echo "<option value='".$value['no_ipp']."'>".$NO_SO."</option>";
							}
						?>
					</select>
