	...
	<div class="box box-primary">
		<div class="box-header">
			...
			<div class="box-tool pull-left">
				...
				<?php }else{echo "<div style='color:red;'><b>Server failed update !!!</b></div>";} ?>
				<div id="spinnerx">
			...
			<table class="table table-bordered table-striped" id="my-grid" width='100%'>
				<thead>
					<tr class='bg-blue'>
						...
						<th class="text-center" width='5%'>IPP</th>
						<th class="text-center" width='7%'>No PO</th>
						<th class="text-center" width='13%'>Customer</th>
						<th class="text-center" width='14%'>Project</th>
						<th class="text-center no-sort" width='7%'>Series</th>
						<th class="text-center no-sort" width='3%'>Rev</th>
						<th class="text-center no-sort" width='7%'>Nilai SO ($)</th>
						<th class="text-center no-sort" width='7%'>Berat SO (Kg)</th>
...
<script>
	...
	$(document).on('click', '.closeso', function(e){
		e.preventDefault();
		var no_ipp	= $(this).data('no_ipp');
		loading_spinner();
		$("#head_title2").html("<b>CLOSE SO ["+no_ipp+"]</b>");
		$.ajax({
			type:'POST',
			url: base_url + active_controller+'/closeso/'+no_ipp,
			success:function(data){
				$("#ModalView2").modal();
				$("#view2").html(data);
	...
	function close_this_so (no_ipp){
		...
		function(isConfirm) {
			if (isConfirm) {
				loading_spinner();
				$.ajax({
					url			: base_url + active_controller+'/close_sales_order',
					type		: "POST",
					data		: {
						'no_ipp' 		: no_ipp
					},
