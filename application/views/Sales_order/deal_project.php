	...
	<div class="box box-primary">
		...
		<div class="box-body">
			...
			<div class='form-group row'>
				...
				<div class='col-sm-4'>
					<select name='no_po' id='no_po' class='form-control input-md chosen-select' required>
					 <option value=''>Select A PO Customer</option>
					 <?php
						$nomorpo = $this->db->query("SELECT * FROM tr_kartu_po_customer where status='1' and id_customer='".$getHeader[0]->id_customer."' order by nomor_po ")->result();
						$idpo=((!empty($restSO[0]->no_po))?$restSO[0]->no_po:'');
						foreach($nomorpo AS $valx){
							$selected='';
							if($valx->nomor_po==$idpo) $selected=' selected';
							echo "<option value='".$valx->nomor_po."'". $selected.">".($valx->nomor_po)."</option>";
						}
					 ?>
					 </select>
				</div>
			...
			<table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
				<tbody>
					...
					<tr class='bg-bluexyz'>
						...
						<th class="text-center" width='9%'>Item No</th>
						<th class="text-center" width='8%'>PO Description</th>
					...
					<?php
						...
						foreach($product AS $val => $valx){
							if($valx['qty'] <> '0'){
								...
								$get_so 	= $this->db->query("SELECT * FROM billing_so_product WHERE id_milik='".$valx['id_milik']."' LIMIT 1")->result();
								$cust_item 	= (!empty($get_so))?$get_so[0]->customer_item:'';
								$desc 		= (!empty($get_so))?$get_so[0]->desc:$valx['id_category'];
								$dataSumupd	= (!empty($get_so))?$get_so[0]->total_deal_usd:$dataSum;

								$SUM += $dataSum;
								$SUMupd += $dataSumupd;
								$dataSumIdr=(!empty($get_so))?$get_so[0]->total_deal_idr:($dataSumupd*$kurs_usd_dipakai);
								$SUM_IDR += $dataSumIdr;
								...
								echo "<tr class='product_".$no."'>";
									...
									echo "<td align='right'>
									<input type='text' name='detail_product[".$no."][total_deal_usd]' data-no='".$no."' class='form-control input-sm text-right productSUMusd base_usd divide' id='total_deal_usd_".$no."' onblur='calcusd(".$no.")' value='".($dataSumupd)."'></td>";
									echo "<td align='right'>
						...
						echo "<tr>";
							...
							echo "<td class='sumAwal' align='right'><b><div id='sum_product_usd' class='divide'>".($SUMupd)."</div></b></td>";
						...
						<tr class='bg-bluexyz'>
							<th class="text-left" colspan='3'>Material Name</th>
							<th class="text-center" colspan=2>Item No</th>
							<th class="text-center" colspan=3>PO Description</th>
						...
						if(!empty($data_material)){
							foreach($data_material AS $val => $valx){
								...
								$get_deal_usd = $this->db->select('*')->get_where('billing_so_add', array('id_milik'=>$valx['id_milik']))->result();
								$cust_item 	= (!empty($get_deal_usd))?$get_deal_usd[0]->customer_item:'';
								...
								if(!empty($get_deal_usd)){
									...
									$deal_idr=$get_deal_usd[0]->total_deal_idr;
									$SUM_MAT_IDR += ($deal_idr);
								}else{
									$deal_idr=($deal_usd*$kurs_usd_dipakai);
									$SUM_MAT_IDR += ($deal_usd*$kurs_usd_dipakai);
								}
								$SUM_MATupd+=$deal_usd;

								echo "<tr class='material_".$no."'>";
									echo "<td colspan='3'>".strtoupper(get_name('raw_materials','nm_material','id_material',$valx['caregory_sub']))."
									...
									echo "<td align='left' colspan=2><input type='text' name='detail_material[".$no."][customer_item]' class='form-control input-sm text-left' value='".$cust_item."'></td>";
									echo "<td align='left' colspan=3><input type='text' name='detail_material[".$no."][desc]' class='form-control input-sm text-left' value='".$desc."'></td>";
						...
						echo "<tr>";
							...
							echo "<td class='sumAwal' align='right'><b><div id='sum_material_usd' class='divide'>".($SUM_MATupd)."</div></b></td>";
						...
						if(!empty($data_non_frp)){
							foreach($data_non_frp AS $val => $valx){
								...
								$price_satuan = 0;
                                if($valx['price_total'] > 0 AND $qty > 0){
								...
								$get_deal_usd = $this->db->select('*')->get_where('billing_so_add', array('id_milik'=>$valx['id_milik2']))->result();
						...
						echo "<tr>";
							...
							echo "<td class='sumAwal' align='right'><b><div id='sum_acc_usd' class='divide'>".($SUM_ACCupd)."</div></b></td>";
						...
						if(!empty($data_eng)){
							foreach($data_eng AS $val => $valx){
								// if($valx['qty'] > 0){
									...
									$get_deal_usd = $this->db->select('*')->get_where('billing_so_add', array('id_milik'=>$valx['id']))->result();
									$deal_usd = round($valx['price_total'],2);
									if(!empty($get_deal_usd)){
										$deal_usd = round($get_deal_usd[0]->total_deal_usd,2);
										$deal_idr = ($get_deal_usd[0]->total_deal_idr);
										$SUM_ENG_IDR += $deal_idr;
									}else{
										$deal_idr = ($deal_usd*$kurs_usd_dipakai);
										$SUM_ENG_IDR += $deal_idr;
									}
						...
						echo "<tr>";
							...
							echo "<td class='sumAwal' align='right'><b><div id='sum_eng_usd' class='divide'>".($SUM_ENGupd)."</div></b></td>";
						...
						<tr class='bg-bluexyz'>
							<th class="text-left" colspan='4'>Item Product</th>
							<th class="text-left" colspan='6'>Desc</th>
						...
						if(!empty($data_pack)){
							foreach($data_pack AS $val => $valx){
								...
								$SUM_PACK += round($deal_usd,2);
								if(!empty($get_deal_usd)){
									$deal_usd = $get_deal_usd[0]->total_deal_usd;
									$deal_idr = $get_deal_usd[0]->total_deal_idr;
									$SUM_PACK_IDR += ($deal_idr);
								}else{
									$deal_idr = ($deal_usd*$kurs_usd_dipakai);
									$SUM_PACK_IDR += ($deal_idr);
								}
								$SUM_PACKupd+=$deal_usd;
								$desc = (!empty($get_deal_usd[0]->desc))?$get_deal_usd[0]->desc:'';

								echo "<tr class='packing_".$no."'>";
									echo "<td colspan='4'>".strtoupper($valx['caregory_sub'])."
									...
									echo "<td colspan='6'><input type='text' name='detail_packing[".$no."][desc]' class='form-control input-sm' placeholder='Description' value='".$desc."'></td>";
						...
						echo "<tr>";
							...
							echo "<td class='sumAwal' align='right'><b><div id='sum_pack_usd' class='divide'>".($SUM_PACKupd)."</div></b></td>";
						...
						<tr class='bg-bluexyz'>
							<th class="text-left" colspan='4'>Item Product</th>
							<th class="text-left" colspan='5'>Desc</th>
							<th class="text-center">Qty</th>
							<th class="text-center">Unit Price</th>
							<th class="text-center">Total Price (USD)</th>
							<th class="text-center">Nilai Deal (USD)</th>
						...
						if(!empty($data_ship)){
							foreach($data_ship AS $val => $valx){
								...
								if(!empty($get_deal_usd)){
									...
									$deal_idr=$get_deal_usd[0]->total_deal_idr;
									$SUM_SHIP_IDR += $deal_idr;
								}else{
									$deal_idr=($deal_usd*$kurs_usd_dipakai);
									$SUM_SHIP_IDR += $deal_idr;
								}
								$SUM_SHIPupd+=$deal_usd;

								$desc = (!empty($get_deal_usd[0]->desc))?$get_deal_usd[0]->desc:'';

								echo "<tr class='shipping_".$no."'>";
									echo "<td colspan='4'>".strtoupper($valx['caregory_sub'].$Add)."
									...
									echo "<td colspan='5'><input type='text' name='detail_shipping[".$no."][desc]' class='form-control input-sm' placeholder='Description' value='".$desc."'></td>";
						...
						echo "<tr>";
							...
							echo "<td class='sumAwal' align='right'><b><div id='sum_ship_usd' class='divide'>".($SUM_SHIPupd)."</div></b></td>";
							echo "<td class='sumAwal' align='right'><b><div id='sum_ship_idr' class='divide'>".($SUM_SHIP_IDR)."</div></b></td>";
						echo "</tr>";
						//=========================================OTHER=============================================
						?>
						<tr class='bg-bluexyz'>
							<th class="text-left" colspan='14'>OTHER</th>
						</tr>
						<tr class='bg-bluexyz'>
							<th class="text-left" colspan='9'>Description</th>
							<th class="text-center">Qty</th>
							<th class="text-center">Unit Price</th>
							<th class="text-center">Total Price (USD)</th>
							<th class="text-center">Nilai Deal (USD)</th>
						...
						$SUM_OTHER_IDR = 0;
						if(!empty($data_other)){
							foreach($data_other AS $val => $valx){
								$no++;
								...
								if(!empty($get_deal_usd)){
									$deal_usd = $get_deal_usd[0]->total_deal_usd;
									$deal_idr=$get_deal_usd[0]->total_deal_idr;
									$SUM_OTHER_IDR += $deal_idr;
								}else{
									$deal_idr=($deal_usd*$kurs_usd_dipakai);
									$SUM_OTHER_IDR += $deal_idr;
								}
								$SUM_OTHERupd+=$deal_usd;

								$desc = (!empty($get_deal_usd[0]->desc))?$get_deal_usd[0]->desc:'';

								echo "<tr class='other_".$no."'>";
									echo "<td colspan='9'>".strtoupper($valx['caregory_sub'])."
											<input type='hidden' name='detail_other[".$no."][desc]' value='".$valx['caregory_sub']."'>
											<input type='hidden' name='detail_other[".$no."][id]' value='".$valx['id']."'>
											<input type='hidden' name='detail_other[".$no."][qty]' value='".$valx['qty']."'>
											<input type='hidden' name='detail_other[".$no."][price_satuan]' value='".$valx['price_total']/$valx['qty']."'>
											<input type='hidden' name='detail_other[".$no."][total_price]' value='".$valx['price_total']."'>
											</td>";
									echo "<td align='center'>".number_format($valx['qty'])."</td>";
									echo "<td align='right'>".number_format($valx['price_total']/$valx['qty'],2)."</td>";
									echo "<td align='right'><div id='other_usdori_".$no."'>".number_format($valx['price_total'],2)."</div></td>";
									echo "<td align='right'>
									<input type='text' name='detail_other[".$no."][total_deal_usd]' data-no='".$no."' class='form-control input-sm text-right otherSUMusd base_usd divide' id='total_deal_usd_".$no."' onblur='calcusd(".$no.")' value='".number_format($deal_usd,2)."'></td>";
									echo "<td align='right'>
									<input type='text' name='detail_other[".$no."][total_deal_idr]' data-no='".$no."' class='form-control input-sm text-right otherSUMidr base_idr divide' id='total_deal_idr_".$no."' onblur='calcidr(".$no.")' value='".number_format($deal_idr,2)."'></td>";
								echo "</tr>";
							}
						}
						else{
						...
						echo "<tr>";
							echo "<td class='sumAwal' colspan='11'><b>TOTAL OTHER</b></td>";
							echo "<td class='sumAwal' align='right'><b><div id='sum_other_usd_awal'>".number_format($SUM_OTHER,2)."</div></b></td>";
							echo "<td class='sumAwal' align='right'><b><div id='sum_other_usd' class='divide'>".($SUM_OTHERupd)."</div></b></td>";
							echo "<td class='sumAwal' align='right'><b><div id='sum_other_idr' class='divide'>".($SUM_OTHER_IDR)."</div></b></td>";
						echo "</tr>";
						echo "<tr>";
						...
						echo "<tr>";
							...
							echo "<td class='sumTotal' align='right'><b><div id='sum_all_usd_awal' class='divide'>".($SUM + $SUM_ENG + $SUM_PACK + $SUM_SHIP + $SUM_MAT + $SUM_ACC + $SUM_OTHER)."</div></b></td>";
						...
						$total_deal_usd = (!empty($restSO))?($restSO[0]->total_deal_usd):($SUMupd + $SUM_ENGupd + $SUM_PACKupd +$SUM_SHIPupd + $SUM_MATupd + $SUM_ACCupd + $SUM_OTHERupd);
						$total_deal_idr = (!empty($restSO))?($restSO[0]->total_deal_idr):($SUM_IDR + $SUM_ENG_IDR + $SUM_PACK_IDR + $SUM_SHIP_IDR + $SUM_MAT_IDR  + $SUM_ACC_IDR  + $SUM_OTHER_IDR);
						echo "<tr>";
						...
						echo "<tr>";
							echo "<td colspan='14'>
									...
									<input type='hidden' name='detail_total[1][product_usd]' id='product_usd' value='".$SUMupd."'>
									<input type='hidden' name='detail_total[1][product_idr]' id='product_idr' value='".$SUM_IDR."'>
									<input type='hidden' name='detail_total[1][material_usd_awal]' id='material_usd_awal' value='".$SUM_MAT."'>
									<input type='hidden' name='detail_total[1][material_usd]' id='material_usd' value='".$SUM_MATupd."'>
									<input type='hidden' name='detail_total[1][material_idr]' id='material_idr' value='".$SUM_MAT_IDR."'>
									<input type='hidden' name='detail_total[1][acc_usd_awal]' id='acc_usd_awal' value='".$SUM_ACC."'>
									<input type='hidden' name='detail_total[1][acc_usd]' id='acc_usd' value='".$SUM_ACCupd."'>
									<input type='hidden' name='detail_total[1][acc_idr]' id='acc_idr' value='".$SUM_ACC_IDR."'>
									<input type='hidden' name='detail_total[1][eng_usd_awal]' id='eng_usd_awal' value='".$SUM_ENG."'>
									<input type='hidden' name='detail_total[1][eng_usd]' id='eng_usd' value='".$SUM_ENGupd."'>
									<input type='hidden' name='detail_total[1][eng_idr]' id='eng_idr' value='".$SUM_ENG_IDR."'>
									<input type='hidden' name='detail_total[1][pack_usd_awal]' id='pack_usd_awal' value='".$SUM_PACK."'>
									<input type='hidden' name='detail_total[1][pack_usd]' id='pack_usd' value='".$SUM_PACKupd."'>
									<input type='hidden' name='detail_total[1][pack_idr]' id='pack_idr' value='".$SUM_PACK_IDR."'>
									<input type='hidden' name='detail_total[1][ship_usd_awal]' id='ship_usd_awal' value='".$SUM_SHIP."'>
									<input type='hidden' name='detail_total[1][ship_usd]' id='ship_usd' value='".$SUM_SHIPupd."'>
									<input type='hidden' name='detail_total[1][ship_idr]' id='ship_idr' value='".$SUM_SHIP_IDR."'>
									<input type='hidden' name='detail_total[1][other_usd_awal]' id='other_usd_awal' value='".$SUM_OTHER."'>
									<input type='hidden' name='detail_total[1][other_usd]' id='other_usd' value='".$SUM_OTHERupd."'>
									<input type='hidden' name='detail_total[1][other_idr]' id='other_idr' value='".$SUM_OTHER_IDR."'>
...
<script>
	...
	function cek_other(){
		...
		// OTHER
		otherSUMusd=0
		$('.otherSUMusd').each(function(){ otherSUMusd += parseFloat($(this).val()); });
		$("#sum_other_usd").html(otherSUMusd);
		$('#other_usd').val(otherSUMusd);
		otherSUMidr=0
		$('.otherSUMidr').each(function(){ otherSUMidr += parseFloat($(this).val()); });
		$("#sum_other_idr").html(otherSUMidr);
		$('#other_idr').val(otherSUMidr);
