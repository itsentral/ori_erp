...
<table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
	...
	if(!empty($non_frp)){
		...
		foreach($non_frp AS $val => $valx){
			$QTY_AWAL = $valx['qty_costing'];
			if($valx['category'] == 'plate'){
				$QTY_AWAL = $valx['weight_costing'];
			}

			$UNIT_PRICE = $valx['price_total'] / $QTY_AWAL;
	...
	<?php 
		...
		<tbody>
			<tr class='bg-blue'>
				<td class="text-left headX HeaderHr" colspan='11'><b>OTHER</b></td>
			</tr>
			<tr class='bg-bluexyz'>
		...
		<tbody>
			...
			foreach ($otherArray as $key => $value) { $nomor++;
				...
				echo "<tr>";
					echo "<td align='left' colspan='8'>".$value['caregory_sub']."</td>";
