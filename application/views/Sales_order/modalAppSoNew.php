...
<div class="box-body">
	...
	$data_berat_pipa = $this->db
		->select('
		SUM((a.est_material / a.qty) * c.qty) AS berat_pipa, 
		SUM((a.est_harga / a.qty) * c.qty) AS biaya_pipa,
		SUM((a.direct_labour + a.indirect_labour + a.machine + a.mould_mandrill + a.consumable) / a.qty * c.qty) AS biaya_pipa_mp,
		SUM((a.foh_consumable + a.foh_depresiasi) / a.qty * c.qty) AS biaya_pipa_foh,
		SUM((a.biaya_gaji_non_produksi + a.biaya_non_produksi + a.biaya_rutin_bulanan) / a.qty * c.qty) AS biaya_pipa_ga,
		SUM(a.unit_price * c.qty) AS biaya_pipa_dasar,
		SUM((a.total_price / a.qty * c.qty) - (a.unit_price * c.qty)) AS biaya_pipa_profit,
		SUM(a.total_price / a.qty * c.qty) AS biaya_pipa_bp,
		SUM((a.total_price_last / a.qty * c.qty) - (a.total_price / a.qty * c.qty)) AS biaya_pipa_allow,
		SUM(a.total_price_last / a.qty * c.qty) AS biaya_pipa_sp,
		SUM(a.total_price_last) AS biaya_pipa_sp_so
		')
	...
	$data_berat_flange = $this->db
		->select('
		SUM((a.est_material / a.qty) * c.qty) AS berat_flange, 
		SUM((a.est_harga / a.qty) * c.qty) AS biaya_flange,
		SUM((a.direct_labour + a.indirect_labour + a.machine + a.mould_mandrill + a.consumable) / a.qty * c.qty) AS biaya_flange_mp,
		SUM((a.foh_consumable + a.foh_depresiasi) / a.qty * c.qty) AS biaya_flange_foh,
		SUM((a.biaya_gaji_non_produksi + a.biaya_non_produksi + a.biaya_rutin_bulanan) / a.qty * c.qty) AS biaya_flange_ga,
		SUM(a.unit_price * c.qty) AS biaya_flange_dasar,
		SUM((a.total_price / a.qty * c.qty) - (a.unit_price * c.qty)) AS biaya_flange_profit,
		SUM(a.total_price / a.qty * c.qty) AS biaya_flange_bp,
		SUM((a.total_price_last / a.qty * c.qty) - (a.total_price / a.qty * c.qty)) AS biaya_flange_allow,
		SUM(a.total_price_last / a.qty * c.qty) AS biaya_flange_sp,
		SUM(a.total_price_last) AS biaya_flange_sp_so
		')
	->from('laporan_revised_detail a')
	->join('product_parent b','a.product_parent=b.product_parent','left')
	->join('so_bf_detail_header c','a.id_milik=c.id_milik','left')
	...
	$data_berat_fitting = $this->db
		->select('
		SUM((a.est_material / a.qty) * c.qty) AS berat_fitting, 
		SUM((a.est_harga / a.qty) * c.qty) AS biaya_fitting,
		SUM((a.direct_labour + a.indirect_labour + a.machine + a.mould_mandrill + a.consumable) / a.qty * c.qty) AS biaya_fitting_mp,
		SUM((a.foh_consumable + a.foh_depresiasi) / a.qty * c.qty) AS biaya_fitting_foh,
		SUM((a.biaya_gaji_non_produksi + a.biaya_non_produksi + a.biaya_rutin_bulanan) / a.qty * c.qty) AS biaya_fitting_ga,
		SUM(a.unit_price * c.qty) AS biaya_fitting_dasar,
		SUM((a.total_price / a.qty * c.qty) - (a.unit_price * c.qty)) AS biaya_fitting_profit,
		SUM(a.total_price / a.qty * c.qty) AS biaya_fitting_bp,
		SUM((a.total_price_last / a.qty * c.qty) - (a.total_price / a.qty * c.qty)) AS biaya_fitting_allow,
		SUM(a.total_price_last / a.qty * c.qty) AS biaya_fitting_sp,
		SUM(a.total_price_last) AS biaya_fitting_sp_so
		')
	->from('laporan_revised_detail a')
	->join('product_parent b','a.product_parent=b.product_parent','left')
	->join('so_bf_detail_header c','a.id_milik=c.id_milik','left')
	...
	$data_berat_bnw = $this->db
		->select('
		SUM((a.est_material / a.qty) * c.qty) AS berat_bnw, 
		SUM((a.est_harga / a.qty) * c.qty) AS biaya_bnw,
		SUM((a.direct_labour + a.indirect_labour + a.machine + a.mould_mandrill + a.consumable) / a.qty * c.qty) AS biaya_bnw_mp,
		SUM((a.foh_consumable + a.foh_depresiasi) / a.qty * c.qty) AS biaya_bnw_foh,
		SUM((a.biaya_gaji_non_produksi + a.biaya_non_produksi + a.biaya_rutin_bulanan) / a.qty * c.qty) AS biaya_bnw_ga,
		SUM(a.unit_price * c.qty) AS biaya_bnw_dasar,
		SUM((a.total_price / a.qty * c.qty) - (a.unit_price * c.qty)) AS biaya_bnw_profit,
		SUM(a.total_price / a.qty * c.qty) AS biaya_bnw_bp,
		SUM((a.total_price_last / a.qty * c.qty) - (a.total_price / a.qty * c.qty)) AS biaya_bnw_allow,
		SUM(a.total_price_last / a.qty * c.qty) AS biaya_bnw_sp,
		SUM(a.total_price_last) AS biaya_bnw_sp_so
		')
	->from('laporan_revised_detail a')
	->join('product_parent b','a.product_parent=b.product_parent','left')
	->join('so_bf_detail_header c','a.id_milik=c.id_milik','left')
	...
	$data_berat_field = $this->db
		->select('
		SUM((a.est_material / a.qty) * c.qty) AS berat_field, 
		SUM((a.est_harga / a.qty) * c.qty) AS biaya_field,
		SUM((a.direct_labour + a.indirect_labour + a.machine + a.mould_mandrill + a.consumable) / a.qty * c.qty) AS biaya_field_mp,
		SUM((a.foh_consumable + a.foh_depresiasi) / a.qty * c.qty) AS biaya_field_foh,
		SUM((a.biaya_gaji_non_produksi + a.biaya_non_produksi + a.biaya_rutin_bulanan) / a.qty * c.qty) AS biaya_field_ga,
		SUM(a.unit_price * c.qty) AS biaya_field_dasar,
		SUM((a.total_price / a.qty * c.qty) - (a.unit_price * c.qty)) AS biaya_field_profit,
		SUM(a.total_price / a.qty * c.qty) AS biaya_field_bp,
		SUM((a.total_price_last / a.qty * c.qty) - (a.total_price / a.qty * c.qty)) AS biaya_field_allow,
		SUM(a.total_price_last / a.qty * c.qty) AS biaya_field_sp,
		SUM(a.total_price_last) AS biaya_field_sp_so
		')
	->from('laporan_revised_detail a')
	->join('product_parent b','a.product_parent=b.product_parent','left')
	->join('so_bf_detail_header c','a.id_milik=c.id_milik','left')
	...
	<!-- <div class="table-responsive"> -->
		<table id="my-grid" class="table table-striped table-bordered table-hover table-condensed" width="100%">
			...
			$data_berat_baut = $this->db
				->select('
				SUM(a.fumigasi  / a.qty * c.qty) AS harga_baut, 
				SUM((a.price  / a.qty * c.qty) - (a.fumigasi  / a.qty * c.qty)) AS profit_baut,
				SUM(a.price  / a.qty * c.qty) AS bp_baut,
				SUM((a.price_total  / a.qty * c.qty) - (a.price  / a.qty * c.qty)) AS allow_baut,
				SUM(a.price_total  / a.qty * c.qty) AS sp_baut,
				SUM(a.price_total) AS sp_baut_so
				')
			...
			$data_berat_gasket = $this->db
								->select('
								SUM(a.fumigasi  / a.qty * c.qty) AS harga_gasket, 
								SUM((a.price  / a.qty * c.qty) - (a.fumigasi  / a.qty * c.qty)) AS profit_gasket,
								SUM(a.price  / a.qty * c.qty) AS bp_gasket,
								SUM((a.price_total  / a.qty * c.qty) - (a.price  / a.qty * c.qty)) AS allow_gasket,
								SUM(a.price_total  / a.qty * c.qty) AS sp_gasket,
								SUM(a.price_total) AS sp_gasket_so
								')
								->from('cost_project_detail a')
								->join('so_bf_acc_and_mat c','a.id_milik=c.id_milik','left')
								->where('a.category','gasket')
			...
			$data_berat_plate = $this->db
				->select('
				SUM(a.fumigasi  / a.qty * c.qty) AS harga_plate, 
				SUM((a.price  / a.qty * c.qty) - (a.fumigasi  / a.qty * c.qty)) AS profit_plate,
				SUM(a.price  / a.qty * c.qty) AS bp_plate,
				SUM((a.price_total  / a.qty * c.qty) - (a.price  / a.qty * c.qty)) AS allow_plate,
				SUM(a.price_total  / a.qty * c.qty) AS sp_plate,
				SUM(a.price_total) AS sp_plate_so
				')
			->from('cost_project_detail a')
			->join('so_bf_acc_and_mat c','a.id_milik=c.id_milik','left')
			->where('a.category','plate')
			...
			$data_berat_lainnya = $this->db
				->select('
				SUM(a.fumigasi  / a.qty * c.qty) AS harga_lainnya, 
				SUM((a.price  / a.qty * c.qty) - (a.fumigasi  / a.qty * c.qty)) AS profit_lainnya,
				SUM(a.price  / a.qty * c.qty) AS bp_lainnya,
				SUM((a.price_total  / a.qty * c.qty) - (a.price  / a.qty * c.qty)) AS allow_lainnya,
				SUM(a.price_total  / a.qty * c.qty) AS sp_lainnya,
				SUM(a.price_total) AS sp_lainnya_so
				')
			->from('cost_project_detail a')
			->join('so_bf_acc_and_mat c','a.id_milik=c.id_milik','left')
			->where('a.category','lainnya')
			...
			$data_berat_material = $this->db
				->select('
				SUM(a.fumigasi  / a.weight * c.qty) AS harga_material, 
				SUM((a.price  / a.weight * c.qty) - (a.fumigasi  / a.weight * c.qty)) AS profit_material,
				SUM(a.price  / a.weight * c.qty) AS bp_material,
				SUM((a.price_total  / a.weight * c.qty) - (a.price  / a.weight * c.qty)) AS allow_material,
				SUM(a.price_total  / a.weight * c.qty) AS sp_material,
				SUM(a.price_total) AS sp_material_so
				')
			->from('cost_project_detail a')
			->join('so_bf_acc_and_mat c','a.id_milik=c.id_milik','left')
			->where('a.category','aksesoris')
			...
			$get_packing_deal = $this->db->select('SUM(total_deal_usd) AS harga_packing ')->get_where('billing_so_add',array('no_ipp'=>$no_ipp,'category'=>'pack'))->result();
			if(!empty($get_packing_deal)){
				$harga_packing = (!empty($get_packing_deal))?$get_packing_deal[0]->harga_packing:0;
			}
			...
			$get_transport_deal = $this->db->select('SUM(total_deal_usd) AS harga_transport ')->get_where('billing_so_add',array('no_ipp'=>$no_ipp,'category'=>'ship'))->result();
			if(!empty($get_transport_deal)){
				$harga_transport = (!empty($get_transport_deal))?$get_transport_deal[0]->harga_transport:0;
			}
			...
			$get_engine_deal = $this->db->select('SUM(total_deal_usd) AS harga_engine ')->get_where('billing_so_add',array('no_ipp'=>$no_ipp,'category'=>'eng'))->result();
			if(!empty($get_engine_deal)){
				$harga_engine = (!empty($get_engine_deal))?$get_engine_deal[0]->harga_engine:0;
			}
			?>
			<thead id='head_table'>
				<tr class='bg-blue'>
			...
			<?php
			$harga_penawaran 	= $biaya_total_sp_so + $sp_total_so + $harga_packing + $harga_transport + $harga_engine;
			$harga_penawaran_so = $biaya_total_sp + $sp_total + $harga_packing + $harga_transport + $harga_engine;
			$harga_cost			= $biaya_total_dasar + $harga_total + $harga_packing + $harga_transport + $harga_engine;
			$net_profit			= $harga_penawaran - $harga_cost;
			$net_persent = 0;
			if($harga_penawaran_so <> 0 AND $net_profit <> 0 ){
            ...
            $est_net_persent = 0;
			if($harga_penawaran_so <> 0 AND $est_net_profit <> 0 ){
		...
		<?php
			...
			if(!empty($qBQdetailRest)){ ?>
				<thead id='head_table'>
					...
					<tr class='bg-blue'>
						...
						<th class="text-center" colspan='3' style='vertical-align:middle;'>Customer Item</th>
						<th class="text-center" style='vertical-align:middle;' width='7%'>Unit Price</th>
					...
					<?php
						...
						foreach($qBQdetailRest AS $val => $valx){ $no++;
							...
							echo "<tr>";
								...
								echo "<td align='left' colspan='3' class='so_style_list'>".strtoupper($valx['customer_item'])."</span></td>";
								echo "<td align='right'>".number_format($valx['cost']/$valx['qty'], 2)."</td>";
			...
			if(!empty($non_frp)){
				echo "<tbody>";
					...
					echo "<tr  class='bg-blue'>";
						...
						echo "<th class='text-center' colspan='4'>Satuan</th>";
				...
				foreach($non_frp AS $val => $valx){
					...
					$qty 	= $valx['qty_so'];
					$satuan = $valx['satuan_so'];
					
					$total_unit 	= $valx['total_deal_usd'] / $qty;
					$price_total 	= $valx['total_deal_usd'];
					
					$SUM_NONFRP += $price_total;

					echo "<tr>";
						echo "<td>".get_name_acc($valx['caregory_sub'])."</td>";
						echo "<td align='right'>".number_format($qty,2)."</td>";
						echo "<td align='left'  colspan='4'>".strtoupper($satuan)."</td>";
						echo "<td align='right'>".number_format($total_unit,2)."</td>";
			...
			if(!empty($material)){
				echo "<tbody>";
					...
					echo "</tr>";
					echo "<tr  class='bg-blue'>";
						echo "<th class='text-center' colspan='2'>Material Name</th>";
						echo "<th class='text-center'>Weight</th>";
						echo "<th class='text-center' colspan='3'>Satuan</th>";
						echo "<th class='text-center'>Unit Price</th>";
						echo "<th class='text-center'>Total Price</th>";
					echo "</tr>";
				echo "</tbody>";
				...
				foreach($material AS $val => $valx){
					...
					echo "<tr>";
						echo "<td colspan='2'>".strtoupper(get_name('raw_materials', 'nm_material', 'id_material', $valx['caregory_sub']))."</td>";
						echo "<td align='right'>".number_format($qty,2)."</td>";
						echo "<td align='left' colspan='3'>".strtoupper($satuan)."</td>";
						echo "<td align='right'>".number_format($total_unit,2)."</td>";
						echo "<td align='right'>".number_format($price_total,2)."</td>";
					echo "</tr>";
				}
