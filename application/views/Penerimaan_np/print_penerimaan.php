...
<body>
<?php
$header	= $this->db->query("SELECT * FROM tr_invoice_np_payment WHERE kd_pembayaran ='$kodebayar'")->row();
...
<tr><td rowspan=2 valign=top>Say</td><td rowspan=2 valign=top>: <?php if($header->kurs_bayar>0){ 
	...
	<td valign=top>Mata Uang</td><td valign=top>: <?php echo($header->kurs_bayar>0?'USD':'IDR') ?></td></tr>
    ...
    <br>
		...
		<?php
			...
			foreach($detail as $val=>$det){
				...
				$invdtl= $this->db->query("SELECT * FROM tr_invoice_np_header WHERE no_invoice='$kodeinv'")->row();
		?>
		<tbody>
			<tr>
