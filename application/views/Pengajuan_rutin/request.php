		...
		<table class="table table-bordered table-striped">
			...
			if(!empty($data_detail)){
				...
				<tr id='tr1_<?=$idd?>' class='delAll'>
					...
					<td><input type="text" class="form-control divide text-right" name="nilai_<?=$record->id;?>" value="<?=($record->nilai);?>"></td>
					<td><input type="text" class="form-control" name="keterangan_<?=$record->id;?>" value="<?=$record->keterangan;?>"></td>
					<td>
                        <?php if($tanda == 'request'){?><input type="file" name="doc_file_<?=$record->id?>" id="doc_file<?=$idd?>"><?php } ?>
                        <?=($record->doc_file!=''?'<a href="'.base_url('assets/bayar_rutin/'.$record->doc_file).'" target="_blank">Download</a>':'')?>
                    </td>
					<?php if($tanda == 'request'){?>
					<td>
						<input type="text" class="form-control text-center" name="tanggalbayar_<?=$record->id;?>" id="tanggalbayar_<?=$idd;?>" value="<?=$record->tgl_bayar;?>" readonly data-role="datepicker_lost" style="cursor: pointer;">
					</td>
					<?php } ?>
					<?php if($tanda == 'edit'){?>
					<td><?=$record->reason;?></td>
