...
<div class="box box-primary">
    ...
    <?=form_open($this->uri->uri_string(),array('id'=>'frm_data','name'=>'frm_data','role'=>'form','class'=>'form-horizontal'));?>
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        if(!empty($new_data_detail)){
                            ...
                            <tr id='tr1_<?=$idd?>' class='delAll'>
                                ...
                                <td><select class="form-control select2" name="coa[]" id="coa_<?=$idd;?>" style="width:250px" required>
                                <?php 
                                if($record->coa!='') {
                                ...
                                <td><input type="text" class="form-control input-sm" name="nama[]" id="nama_<?=$idd;?>" value=""></td>
                                ...
                                <td><input type="text" class="form-control divide input-sm" name="nilai[]" id="nilai_<?=$idd;?>" value=""></td>
                                <td><input type="text" class="form-control input-sm" name="keterangan[]" id="keterangan_<?=$idd;?>" value=""></td>
                        ...
                        if(!empty($data_detail)){
                            ...
                            <tr id='tr1_<?=$idd?>' class='delAll'>
                                ...
                                <td><select class="form-control select2" style="width:100%" name="coa[]" id="coa_<?=$idd;?>" style="width:250px" required>
                                <?php
                                if($record->coa!='') {
                                    echo str_ireplace($record->coa."'",$record->coa."' selected ", $combocoa);
                                }else{
                                ...
                                <td><input type="text" class="form-control input-sm" name="nama[]" id="nama_<?=$idd;?>" value="<?=$record->nama;?>"></td>
                                <td><select class="form-control" name="tipe[]" id="tipe_<?=$idd;?>" onchange="cektipe(<?=$idd;?>)">
                                <?php 
                                if($record->coa!='') {
                                ...
                                <td><input type="text" class="form-control divide input-sm" name="nilai[]" id="nilai_<?=$idd;?>" value="<?=($record->nilai);?>"></td>
                                <td><input type="text" class="form-control input-sm" name="keterangan[]" id="keterangan_<?=$idd;?>" value="<?=$record->keterangan;?>"></td>
                                <td align='center'>
        ...
        //								echo '<button type="button" name="Approve" class="btn btn-primary btn-sm" id="approve" onclick="data_approve()"><i class="fa fa-save">&nbsp;</i>Approve</button>';
                                ...
                                <?php } ?>
                                <a href='<?=base_url('budget_coa/budget_periodik');?>' class="btn btn-danger btn-md">Back</a>
...
<script type="text/javascript">
	var url_save = base_url+'budget_coa/save_data_periodik/';
	var url_approve = base_url+'budget_coa/approve_periodik/';
	var nomor=<?=$idd?>;
	...
	function add_alokasi(id){
		...
		var Rows	 = 	"<tr id='tr1_dtl"+nomor+"_"+nomor_detail+"' class='delAll rows"+kode_id+"'>";
			...
			Rows	+= 		"</td>";
			Rows	+= 		"<td colspan=3>";
			Rows	+=			"COA :<br /><select class='form-control select2' style='width:100%' name='coa_alokasi[]' id='coa_alokasi_"+nomor_detail+"'>";
            Rows	+=      "<?= $all_coa?>";
            Rows	+=          "</select>";
			Rows	+= 		"</td>";
			Rows	+= 		"<td>";
	...
	function add_detail(id){
		var Rows	 = 	"<tr id='tr1_"+nomor+"' class='delAll'>";
			...
			Rows	+= 		"</td>";
			Rows	+= 		"<td>";
			Rows	+=			"<input type='text' class='form-control input-sm' name='nama[]' id='nama_"+nomor+"' />";
			Rows	+= 		"</td>";
			Rows	+= 		"<td>";
			...
			Rows	+= 		"</td>";
			Rows	+= 		"<td>";
			Rows	+=			"<input type='text' class='form-control divide input-sm' name='nilai[]' value='0' id='nilai_"+nomor+"' />";
			Rows	+= 		"</td>";
			Rows	+= 		"<td>";
