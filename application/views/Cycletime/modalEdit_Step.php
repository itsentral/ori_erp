...
<div class="box box-success">
	<div class="box-body" style="">
			...
			<tbody>
				...
				<tr>
					...
					<td class="text-left">
						...
						<select id='komponenx' name='komponenx' class='chosen_select form-control inline-block' style='min-width:200px;' disabled>
						...
						<?php
							echo form_input(array('type'=>'hidden','id'=>'komponen','name'=>'komponen','class'=>'form-control input-sm','placeholder'=>'Standard Length','autocomplete'=>'off'), $getDefault->product_parent);
						?>
					</td>
				</tr>
        <tr>
        ...
        <tr>
				...
				-->
				<?php
				if($getDefault->product_parent == 'pipe' OR $getDefault->product_parent == 'pipe slongsong'){
				?>
				<tr>
						...
						foreach($getDetail AS $val => $valz){$detNum++;?>
				            ...
				            <td style="background-color:#E0E0E0;text-align:center">
										<select name="step[]" id="step[]" class="chosen_select form-control inline-block select_step">
											...
											<?php foreach ($getStep AS $val => $valx): $selected = ($valz->step==$valx["step_name"])?'selected':''?>
												<option value="<?=$valx["step_name"]?>" <?=$selected?> ><?=strtoupper($valx["step_name"])?></option>
											<?php endforeach; ?>
										</select>
		...
		<br>
		<a class="btn btn-sm btn-success" id="addS">Add Step</a>
		<br>
		<?php
