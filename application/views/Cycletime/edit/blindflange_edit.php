...
<form action="#" method="POST" id="form_proses_bro">   
	<div class="box box-primary">
		...
		<div class="box-body">
			...
			<div class='form-group row'>
				<label class='label-control col-sm-2'><b>Thickness | Flange OD <span class='text-red'>*</span></b></label>
				<div class='col-sm-2'>              
					<?php
				...
				<div class='col-sm-2'>   
					<?php
						echo form_input(array('id'=>'flange_od','name'=>'flange_od','class'=>'form-control input-sm numberOnly','autocomplete'=>'off','placeholder'=>'Flange OD'),floatval($header[0]->flange_od));
					?>
				</div>
						...
						<tbody>
							<?php
								...
								foreach($detLiner AS $val => $valx){
									$no++;
									$OriType	= $this->db->query("SELECT id_ori, id_ori2 FROM help_type WHERE parent_product='blind flange' AND jenis='LIN' AND nomor='".$no."' ")->result_array();
						...
						<tbody>
							<?php
								...
								foreach($detStructure AS $val => $valx){
									$no++;
									$OriType	= $this->db->query("SELECT id_ori, id_ori2 FROM help_type WHERE parent_product='blind flange' AND jenis='STR' AND nomor='".$no."' ")->result_array();
						...
						<tbody>
							<?php
								...
								foreach($detEksternal AS $val => $valx){
									$no++;
									$OriType	= $this->db->query("SELECT id_ori, id_ori2 FROM help_type WHERE parent_product='blind flange' AND jenis='EKS' AND nomor='".$no."' ")->result_array();
...
<script src="<?php echo base_url('application/views/Component/edit/blindflange_edit/change_all.js'); ?>"></script>
<script src="<?php echo base_url('application/views/Component/edit/blindflange_edit/change_material.js'); ?>"></script>
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl		= base_url + active_controller +'/blindflange_edit';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
