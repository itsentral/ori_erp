...
function LuasArea(diameter, waste, length, estimasi){
	
	var LuasAreaOne		= 3.14 * (diameter + estimasi)/1000 * (length / 1000) * (1+(waste/100));
	
	if(isNaN(LuasAreaOne)){var LuasAreaOne = 0;}
	if(isNaN(length)){var length = 0;}
	
	$("#area").val(LuasAreaOne.toFixed(4));
	$("#length").val(Math.round(length / 10) * 10);

	return LuasAreaOne;
}

function LuasArea2(diameter2, waste, high, estimasi){
	
	var LuasAreaTwo		= 3.14 * (diameter2 + estimasi)/1000 * (high / 1000) * (1+(waste/100));
...
function Length(){
	...
	var length		= wrap_length + (diameter2 + 2 * estimasi) + 50;
...
function High(){
	...
	var high			= 0.5 * (wrap_length2 + diameter + 2 * design) + 25;
	// console.log(high);
	if(isNaN(high)){
		var high = 0;
	}
...
function ChangeLuasArea(){
	...
	var LuasArea1x = LuasArea(diameter, waste, length, estimasi);
	var LuasArea2x = LuasArea2(diameter2, waste, high, estimasi);
	
	var Luas_Area_Rumus		= LuasArea1x + LuasArea2x;
	
	var LastCoat	= Luas_Area_Rumus * 0.25 * 1.2;
...
function LastWeight(){
	...
	var area2		= parseFloat($('#area2').val());
