        ...
        <table class="table table-sm table-bordered table-striped" id="my-grid" width='100%'>
            <thead>
                <tr class='bg-blue'>
                    ...
                    <th class="text-center" width='20%'>Kode Spool</th>
...
<script>
		...
		$(document).on('click', '#make_spool', function(){
			...
			function(isConfirm) {
				if (isConfirm) {
					...
					$.ajax({
						url			: base_url + active_controller+'/create_delivery_spool',  
						type		: "POST",
						data		: formData,
						cache		: false,
						dataType	: 'json',
	...
	function DataTables(no_ipp=null){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/server_side_request_spool',
				type: "post",
				data: function(d){
					d.no_ipp = no_ipp
				},
