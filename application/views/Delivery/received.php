...
<script>
	$(document).ready(function(){
		...
		$(document).on('click', '.back_to_delivery', function(e){
			e.preventDefault();
			...
			swal({
				...
				function(isConfirm) {
					if (isConfirm) {
						loading_spinner();
						var baseurl=base_url + active_controller +'/reject_delivery';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: {
	...
	function DataTables2(status=null){
		var dataTable = $('#my-grid2').DataTable({
			...
			"ajax":{
				url : base_url + active_controller+'/server_side_received',
				type: "post",
				data: function(d){
					d.status = status
				},
