...
class Salary_model extends CI_Model{
	...
	public function getDataJSON(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$nestedData[]	= "<div align='left'>".strtoupper(strtolower($row['kode']))."</div>";
			$nestedData[]	= "<div align='left'>".strtoupper(strtolower($row['periode']))."</div>";
			...
			$nestedData[]	= "<div align='center'>
									<a class='btn btn-default btn-sm view' href='".base_url('salary/view/'.$row['id'])."' title='View'><i class='fa fa-eye'></i></a> 
									".$delX."
									".$updX."
									</div>";
			$data[] = $nestedData;
	...
	public function queryDataJSON($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		$sql = "
			...
			WHERE 1=1
				...
				or a.periode LIKE '%".$this->db->escape_like_str($like_value)."%'
	        )
		";
