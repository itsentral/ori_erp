<?php
class Budget_rutin_model extends CI_Model {
	...
	public function add_rutin(){
		if($this->input->post()){
			...
			$code = $this->uri->segment(3);
			$header 			= $this->db->query("SELECT * FROM budget_rutin_header WHERE code_budget='".$code."' ")->result();
			$warehouse_project 	= $this->db->query("SELECT a.id, b.nm_customer, a.nm_gudang FROM warehouse a LEFT JOIN customer b ON a.nm_gudang=b.id_customer WHERE a.category='customer' OR a.category='project' ORDER BY a.id ASC ")->result_array();
	...
	public function get_data_json_rutin(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$nm_gudang = (!empty($row['nm_customer']))?'Gudang Project '.$row['nm_customer']:$row['nm_gudang'];
	...
	public function query_data_json_rutin($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		...
		$sql = "
			...
			FROM
				...
				LEFT JOIN warehouse d ON a.id_gudang = d.id
				LEFT JOIN customer e ON d.nm_gudang = e.id_customer,
				(SELECT @row:=0) r
		    WHERE  (
