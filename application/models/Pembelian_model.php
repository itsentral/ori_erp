...
class Pembelian_model extends CI_Model {
	...
	public function index_pr(){
		...
		$this->load->view('Pembelian/pr_new',$data);
	}
	...
	public function get_data_json_progress_pr_new(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$nestedData[]	= "<div align='center'>
								<button type='button' class='btn btn-sm btn-primary detail_pr' title='Detail' data-no_pr_group='".$row['no_pr_group']."'><i class='fa fa-eye'></i></button>
								</div>";
			$data[] = $nestedData;
	...
	public function add_rfq(){
		...
		$this->load->view('Pembelian/add_rfq', $data);
	}
	...
	public function get_data_json_list_pr(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$nestedData 	= array();
			$nestedData[]	= "<div align='center'><center><input type='checkbox' name='check[$nomor]' class='chk_personal changeCheckList' data-nomor='".$nomor."' value='".$row['id']."' ".$CHECK."></center></div>";
			$nestedData[]	= "<div align='center'>".$row['no_pr_group']."</div>";
			$nestedData[]	= "<div align='center'>".date('d-M-Y', strtotime($row['tgl_pr']))."</div>";
			$nestedData[]	= "<div align='left'>".strtoupper($row['nm_barang'])."</div>";
			if($row['category'] == 'asset'){
			...
			$tgl_dibutuhkan = ($row['tgl_dibutuhkan'] <> '0000-00-00')?date('d-M-Y', strtotime($row['tgl_dibutuhkan'])):'NOT SET';
			$USERNAME 		= (!empty($GET_USERNAME[$row['created_by']]['nm_lengkap']))?$GET_USERNAME[$row['created_by']]['nm_lengkap']:$row['created_by'];
	...
	public function query_data_json_purchase_order($category, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		...
		$sql = "
			...
			(@row:=@row+1) AS nomor,
				...
				GROUP_CONCAT(CONCAT(b.nm_barang,', <b>(',b.qty_purchase,')</b>') ORDER BY b.id ASC SEPARATOR '<br>') AS nm_barang_group
			FROM
				tran_po_detail b
	...
	public function delete_sebagian_po(){
		...
		$SUM_MAT = 0;
		if(!empty($detail)){
			foreach($detail AS $val => $valx){ $urutan2PR++;
