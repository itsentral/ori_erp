...
class Revenue_model extends CI_Model {
	...
	public function modal_detail_so(){
		...
		$this->load->view('Revenue/modalViewSo', $data);
	}
	...
	public function modal_deal_so(){
		...
		$this->load->view('Revenue/modalAppCost', $data);
	}
	...
	public function get_data_json_so(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$nestedData[]	= "<div align='left'><span class='badge' style='background-color:".$warna."'>".$row['status']."</span></div>";
					$deal	= "<a href='".base_url('Revenue/deal_project/'.$row['id_bq'])."' class='btn btn-sm btn-primary'  title='Deal Project' ><i class='fa fa-edit'></i></a>";
					$top	= "";
					$approve= "";
					$print	= "";
					$print2	= "";
					...
					$print	= "<a href='".base_url('Revenue/print_sales_order_usd/'.$row['no_ipp'])."' target='_blank' class='btn btn-sm btn-info'  title='Print Sales Order USD' ><i class='fa fa-print'></i></a>";
					...
					if($num_so > 0){
						$print2	= "<a href='".base_url('Revenue/print_Revenue/'.$row['no_ipp'])."' target='_blank' class='btn btn-sm btn-success'  title='Print Sales Order IDR' ><i class='fa fa-print'></i></a>";
						$print3	= "<a href='".base_url('Revenue/print_sales_order_ex_price/'.$row['no_ipp'])."' target='_blank' class='btn btn-sm btn-primary'  title='Print Sales Order Deal Tanpa Harga' ><i class='fa fa-print'></i></a>";
	...
	public function approve_so(){
		...
		$this->load->view('Revenue/approve_so',$data);
	}
	...
	public function modal_approve_so(){
		...
		$this->load->view('Revenue/modalAppSo', $data);
	}
	...
	public function modal_approve_so_new(){
		...
		$this->load->view('Revenue/modalAppSoNew', $data);
	}
	...
	public function get_data_json_so_app(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$nestedData[]	= "<div align='left'><span class='badge' style='background-color:".$warna."'>".$row['status']."</span></div>";
					...
					$num_so = $this->db->query("SELECT * FROM billing_so WHERE no_ipp = '".$row['no_ipp']."' ")->num_rows();
					if($num_so > 0){
						$Print	= "&nbsp;<a href='".base_url('Revenue/print_sales_order_usd/'.$row['no_ipp'])."' target='_blank' class='btn btn-sm btn-warning'  title='Print SO DEAL USD' ><i class='fa fa-print'></i></a>";
					}
	...
	public function deal_project(){
		if($this->input->post()){
			...
			$this->load->view('Revenue/deal_project',$data);
		}
	...
	public function print_sales_order(){
		...
		$sql_product	= "SELECT a.* FROM billing_so_product a WHERE no_ipp='".$no_ipp."' AND product <> 'product kosong' AND total_deal_usd > 0";
