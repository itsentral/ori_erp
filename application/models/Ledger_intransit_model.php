...
class Ledger_intransit_model extends CI_Model {
	...
	public function get_ledger_data($bulan, $tahun){
		...
		$cek = $this->db->query("SELECT id FROM begining_stock WHERE no_perkiraan = 'intransit' AND bln = '".$this->db->escape_str($bulan)."' AND thn = '".$this->db->escape_str($tahun)."'")->row();
		if(!empty($cek)){
		...
		$cek_next = $this->db->query("SELECT id FROM begining_stock WHERE no_perkiraan = 'intransit' AND bln = '".$bln_next_str."' AND thn = '".$thn_next."'")->row();
		if(!empty($cek_next)){
			$this->db->where('id', $cek_next->id);
			$this->db->update('begining_stock', array('saldoawal' => $saldo_akhir));
		} else {
			$this->db->insert('begining_stock', array(
				'no_perkiraan'	=> 'intransit',
				'nama'			=> 'intransit',
