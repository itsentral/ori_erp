<?php
    $ENABLE_ADD     = has_permission('Master_process.Add');
    $ENABLE_MANAGE  = has_permission('Master_process.Manage');
    $ENABLE_VIEW    = has_permission('Master_process.View');
    $ENABLE_DELETE  = has_permission('Master_process.Delete');
?>
<style type="text/css">
thead input {
	width: 100%;
...
<div class="box">
	...
	<div class="box-body">
		...
		<tr>
		    <td><?= $numb; ?></td>
			<td><?= strtoupper($record->nm_process);?></td>
			<td><?= ucfirst(strtolower($record->keterangan));?></td>
...
<script type="text/javascript">
	...
	$(document).on('click', '.edit', function(e){
		...
		$.ajax({
			type:'POST',
			url:siteurl+'master_process/add/'+id,
	...
	$(document).on('click', '.add', function(){
		$("#head_title").html("<i class='fa fa-list-alt'></i><b>Master Process</b>");
		$.ajax({
			type:'POST',
			url:siteurl+'master_process/add',
			success:function(data){
				$("#dialog-popup").modal();
				$("#ModalView").html(data);
	...
	$(document).on('click', '.delete', function(e){
		...
		function(){
		  $.ajax({
			  type:'POST',
			  url:siteurl+'master_process/hapus',
			  dataType : "json",
			  data:{'id':id},
