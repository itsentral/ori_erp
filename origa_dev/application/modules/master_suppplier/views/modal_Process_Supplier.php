...
if (!empty($this->uri->segment(3))) {
  ...
  $getCP            = $this->db->get_where('child_supplier_pic', array('id_supplier' => $id))->result();
  $getCB            = $this->db->get_where('child_supplier_branch', array('id_supplier' => $id))->result();
  ...
  $name_category    = $this->db->get_where('child_supplier_category', array('id_supplier_customer' => $getC->id_category_customer))->row();
  $name_sales       = $this->db->get_where('ms_karyawan', array('id_karyawan' => $getC->id_karyawan))->row();
  $religion         = $this->db->get_where('child_supplier_pic', array('id_supplier' => $id))->row();
  $name_reference   = $this->db->get_where('child_supplier_reference', array('id_reference' => $getC->id_reference))->row();
}
$getI   = $this->db->query("SELECT MAX(id_supplier) AS max_id FROM master_supplier ORDER BY id_supplier DESC LIMIT 1")->row();
//echo "$first_letter";
//exit;
$num = substr($getI->max_id, 1, 5) + 1;
$id_supplier = 'S' . str_pad($num, 5, "0", STR_PAD_LEFT);
