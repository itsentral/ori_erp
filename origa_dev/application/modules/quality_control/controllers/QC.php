   ...
   public function __construct()
    ...
    public function good_bad_action(){
      ...
      if($tanda == 'oke'){
        ...
        //update sebelumnya
        $query	  = $this->db->query("SELECT * FROM cycletime_fast WHERE id_product='".$data_header[0]->id_product."' AND costcenter='".$data_header[0]->id_costcenter."' LIMIT 1 ")->result();
        $antrialx  = (!empty($query[0]->urut) AND $query[0]->costcenter != 'CC2000012')?intval($query[0]->urut):0;
        $urut     = sprintf('%02s',$antrialx - 1);

        $query	  = $this->db->query("SELECT * FROM cycletime_fast WHERE id_product='".$data_header[0]->id_product."' AND urut='".$urut."' LIMIT 1 ")->result();
        $antrial  = (!empty($query[0]->costcenter))?$query[0]->costcenter:0;
        ...
        $qty_stock3 = $qty_stock2;
        if($antrial != 'CC2000012'){
        ...
        if($data_header[0]->id_costcenter == 'CC2000001'){
    ...
    public function good_bad_action_check(){
      ...
      if($ket == 'good'){
        $data_header    = $this->db->query("SELECT a.id, a.id_product, b.id_costcenter, a.id_produksi FROM report_produksi_daily_detail a LEFT JOIN report_produksi_daily_header b ON a.id_produksi=b.id_produksi WHERE a.id IN ".$dtImplode." ")->result_array();
        $ArrInsertStock	= array();
        $ArrHistProduct = array();
        $ArrInsertStock23 = array();
        $ArrHistProduct23 = array();
        ...
        foreach($data_header AS $val => $valx){
          ...
          $ArrInsertStock[$val]['qty_stock']   = $qty_stock + 1;
          ...
          $ArrHistProduct[$val]['qty_stock_akhir']  = $qty_stock + 1;
          ...
          //update sebelumnya
          $query	  = $this->db->query("SELECT * FROM cycletime_fast WHERE id_product='".$valx['id_product']."' AND costcenter='".$valx['id_costcenter']."' LIMIT 1 ")->result();
          $antrialx  = (!empty($query[0]->urut) AND $query[0]->costcenter != 'CC2000012')?intval($query[0]->urut):0;
          $urut     = sprintf('%02s',$antrialx - 1);

          $query2	  = $this->db->query("SELECT * FROM cycletime_fast WHERE id_product='".$valx['id_product']."' AND urut='".$urut."' LIMIT 1 ")->result();
          ...
          if($valx['id_costcenter'] == 'CC2000001'){
              ...
              $ArrInsertStock2[$val]['qty_stock']       = $qty_stock2 + 1;
              ...
              $ArrHistProduct2[$val]['qty_stock_akhir'] = $qty_stock2 + 1;
              $ArrHistProduct2[$val]['no_trans']        = $valx['id'].", ".$valx['id_produksi'];
