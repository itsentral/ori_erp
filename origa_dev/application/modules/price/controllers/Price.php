   ...
   public function __construct()
    ...
    public function excel_report_material_price(){
        ...
        $sheet->setCellValue('D'.$NewRow, 'Price');
  		...
  		header('Content-Disposition: attachment;filename="MATERIAL_PRICE_'.date('YmdHis').'.xls"');
  		//unduh file
    ...
    public function excel_report_product_price(){
        ...
        $sheet->setCellValue('E'.$NewRow, 'Fitting Price');
  		...
  		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  		//ubah nama file saat diunduh
  		header('Content-Disposition: attachment;filename="PRODUCT_PRICE_'.date('YmdHis').'.xls"');
  		//unduh file
  		$objWriter->save("php://output");
  	}
