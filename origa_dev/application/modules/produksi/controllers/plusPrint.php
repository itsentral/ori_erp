...
function print_planning_produksi($Nama_APP, $no_plan, $koneksi, $printby){
	...
	$sroot 		= $_SERVER['DOCUMENT_ROOT'].'/origa';
	// include $sroot. "/application/libraries/PHPMailer/PHPMailerAutoload.php";
	include $sroot."/application/libraries/MPDF574/mpdf.php";
...
function print_planning_produksi_custom($Nama_APP, $costcenter, $koneksi, $printby, $tgl_awal, $tgl_akhir, $project){
  ...
  $data_header  = "SELECT * FROM produksi_planning WHERE costcenter='".$costcenter."'";
	...
	$projectX = "('".str_replace("-","','",$project)."')";
	// echo $projectX; exit;
	$data_detail  = "SELECT a.* FROM produksi_planning_data a LEFT JOIN ms_inventory_category2 b ON a.product=b.id_category2 WHERE b.id_category1 IN ".$projectX." AND a.no_plan IN ".$dtImplode." AND a.`date` BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."' GROUP BY a.`date` ORDER BY a.`date`";
	// echo $data_detail;
	$result	      = mysqli_query($conn, $data_detail);
  $resultx	    = mysqli_query($conn, $data_detail);
	$resultY	    = mysqli_query($conn, $data_detail);
	$resultZ	    = mysqli_query($conn, $data_detail);
  $data_num	    = mysqli_num_rows($result);

	$data_product = "SELECT a.* FROM produksi_planning_data a LEFT JOIN ms_inventory_category2 b ON a.product=b.id_category2 WHERE b.id_category1 IN ".$projectX." AND a.no_plan IN ".$dtImplode." AND a.`date` BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."'  AND a.qty <> 0 GROUP BY a.product ORDER BY a.product";
  $result_p	    = mysqli_query($conn, $data_product);
	$data_num2	    = mysqli_num_rows($result_p);
// echo $data_product;
  ...
  $date_now = date('Y-m-d', strtotime($tgl_awal));
	...
	<table class="gridtable" border='1' width='100%' cellpadding='2'>
		<tr>
      <td width='70px' rowspan='3' style='padding:0px;'><img src='<?=$sroot;?>/assets/images/ori_logo.jpg' alt="" height='100' width='80' ></td>
			<td align='center'><b>PT. ORIGA MULIA FRP</b></td>
			<td width='15%'>Nomor Dok.</td>
			<td width='15%'></td>
		</tr>
		<tr>
			<td align='center' rowspan='2'><b><h3><?= strtoupper(get_name_field('ms_costcenter', 'nama_costcenter', 'id_costcenter', $header['costcenter']));?> PLANNING</h3></b></td>
			<td>Rev.</td>
			<td></td>
		</tr>
		<tr>
  ...
  <table class="gridtable" width='100%'>
    <thead>
      <tr>
				<th rowspan='2' style='vertical-align:middle; width:40px !important;'>No</th>
        <th rowspan='2' style='vertical-align:middle; width:140px !important;'>Project</th>
        <th rowspan='2' style='vertical-align:middle; width:150px !important;'>Product</th>
				<th rowspan='2' style='vertical-align:middle; width:50px !important;'>Qty Order</th>
        <th rowspan='2' style='vertical-align:middle; width:50px !important;'>Stock</th>
        <?php
        while($value = mysqli_fetch_array($result)){

            echo "<th colspan='3' style='vertical-align:middle; width:150px !important;'>".date('d-M-Y', strtotime($value['date']))."</th>";
						echo "<th rowspan='2' style='vertical-align:middle; width:100px !important;'>Ket</th>";
						echo "<th rowspan='2' style='vertical-align:middle; width:130px !important;'>Man Power</th>";
						echo "<th rowspan='2' style='vertical-align:middle; width:50px !important;'>No Mould</th>";
			  }
        ?>
      </tr>
      <tr>
        <?php
        while($value = mysqli_fetch_array($resultx)){
            echo "<th style='vertical-align:middle; width:40px !important;'>Plan</th>";
            echo "<th style='vertical-align:middle; width:40px !important;'>Aktual</th>";
						echo "<th style='vertical-align:middle; width:120px !important;'>Daycode</th>";
        }
        ?>
      </tr>
    </thead>
    <tbody>
      <?php
        ...
        while($value = mysqli_fetch_array($result_p)){ $nox++;
            ...
            echo "<td align='center'>".$value['qty_order']."</td>";
            echo "<td align='center'>".get_stock($value['product'])."</td>";

            $data_detail2  = "SELECT a.* FROM produksi_planning_data a LEFT JOIN ms_inventory_category2 b ON a.product=b.id_category2 WHERE b.id_category1 IN ".$projectX." AND a.no_plan IN ".$dtImplode." AND a.`date` BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."' GROUP BY a.`date` ORDER BY a.`date`";
            $result2	    = mysqli_query($conn, $data_detail2);
            while($value2 = mysqli_fetch_array($result2)){
                $q_weight = "SELECT a.qty FROM produksi_planning_data a LEFT JOIN ms_inventory_category2 b ON a.product=b.id_category2 WHERE b.id_category1 IN ".$projectX." AND a.`date`='".$value2['date']."' AND a.product='".$value['product']."' AND a.no_plan='".$value['no_plan']."' LIMIT 1 ";
                $wight_h	= mysqli_query($conn, $q_weight);
              	$weight	  = mysqli_fetch_array($wight_h);
                $nil = (!empty($weight))?$weight['qty']:0;
								$enter = "";
            ...
            echo "</tr>";
				...
				echo "<tr>";
					...
					echo "<td align='center'></td>";
					echo "<td align='center'></td>";
					while($value = mysqli_fetch_array($resultY)){
							echo "<td align='center'></td>";
							echo "<td align='center'></td>";
							echo "<td align='center'></td>";
							echo "<td align='center'></td>";
				...
				echo "<tr>";
					...
					echo "<td align='center'></td>";
					echo "<td align='center'></td>";
					while($value = mysqli_fetch_array($resultZ)){
							echo "<td align='center'></td>";
							echo "<td align='center'></td>";
							echo "<td align='center'></td>";
							echo "<td align='center'></td>";
	...
	<p class='foot1'> <?php echo "<i>Printed by : ".ucwords(strtolower(get_name_field('users', 'username', 'id_user', $printby))).", ".$today." / ".strtolower(get_name_field('ms_costcenter', 'nama_costcenter', 'id_costcenter', $header['costcenter']))."</i>"; ?> </p>
	<style type="text/css">
	@page {
		margin-top: 0.5cm;
		margin-left: 0.5cm;
