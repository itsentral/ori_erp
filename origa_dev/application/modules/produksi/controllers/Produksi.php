...
class Produksi extends Admin_Controller{
  ...
  public function get_planning2(){
    ...
    // echo $perbedaan->d;
    // $product    = $this->db->query("SELECT product, SUM(qty_order) AS qty_order FROM sales_order_detail WHERE delivery_date BETWEEN '".$date."' AND '".$date_akhir."' AND product IN ".$dtImplode." GROUP BY product ORDER BY product ")->result_array();
    $product    = $this->db->query("SELECT * FROM produksi_planning_data WHERE no_plan = '".$no_plan."' ORDER BY product, `date`")->result_array();

    $product_date    = $this->db->query("SELECT delivery_date, no_so FROM sales_order_detail WHERE delivery_date BETWEEN '".$date."' AND '".$date_akhir."' GROUP BY no_so ORDER BY delivery_date ")->result_array();
    $product_date_num    = $this->db->query("SELECT no_so FROM sales_order_detail WHERE delivery_date BETWEEN '".$date."' AND '".$date_akhir."' GROUP BY no_so")->num_rows();
