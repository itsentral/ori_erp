...
<script type="text/javascript">
    ...
    if(no_plan != ''){
      ...
      var no_plan     = $("#no_plan").val();
			...
			$.ajax({
				url: base_url+'index.php/'+active_controller+'/get_planning2/'+date_akhir+'/'+costcenter+'/'+date_awal+'/'+no_plan,
				cache: false,
				type: "POST",
				dataType: "json",
    ...
    $(document).on('change', '#date_produksi_plan', function(e){
      ...
      var date      = moment(plan_date).format('YYYY-MM-DD');
			...
			$.ajax({
				url: base_url+'index.php/'+active_controller+'/get_planning2/'+date+'/'+costcenter,
				cache: false,
				type: "POST",
				dataType: "json",
				success: function(data){
...
function get_total_ct(row, col){
  ...
  var sh = getNum($("#sh_"+row).val());
  var SUM_BALA = 0;
  $(".bal_"+row ).each(function() {
    SUM_BALA += getNum(Number($(this).val().split(",").join("")));
  });
  var balc = sh - SUM_BALA;
  $("#balance_"+row).html(balc);
