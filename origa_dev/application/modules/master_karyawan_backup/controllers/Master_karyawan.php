 ...
 *
 * This is controller for Master_karyawan
 */

class Master_karyawan_backup
 extends Admin_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->library(array('Mpdf','upload','Image_lib','PHPExcel'));
        $this->load->model(array('Master_karyawan/Master_karyawan_model',
                                 'Master_karyawan/Divisi_model',
                                 'Aktifitas/aktifitas_model'
                                ));

        $this->template->title('Manage Data Master_karyawan');
    ...
    public function index()
    {
        ...
        $this->template->title('Master_karyawan');
    ...
    //Edit Master_karyawan
    public function edit()
    {
        $this->auth->restrict($this->managePermission);

        $id = $this->uri->segment(3);
        $data  = $this->Master_karyawan_model->find_by(array('id_karyawan' => $id));
        if(!$data)
        {
        ...
        $datdiv     = $this->Divisi_model->pilih_div()->result();
        //$datdiv = $this->Master_karyawan_model->get_div($id);

        $this->template->set('data',$data);
        $this->template->set('datdiv',$datdiv);
        $this->template->title('Edit Data Master_karyawan');
        $this->template->render('karyawan_form');
    }
    ...
    public function save_biodata_ajax(){
        ...
        if($type=="edit")
        {
            ...
            if($id_karyawan!="")
            {
                ...
                //Update data
                $result = $this->Master_karyawan_model->update_batch($data,'id_karyawan');

                $keterangan     = "SUKSES, Edit data Master_karyawan ".$id_karyawan.", atas Nama : ".$nama_karyawan;
                $status         = 1;
                $nm_hak_akses   = $this->addPermission;
            ...
            else
            {
                ...
                $keterangan     = "GAGAL, Edit data Master_karyawan ".$id_karyawan.", atas Nama : ".$nama_karyawan;
                $status         = 1;
                $nm_hak_akses   = $this->addPermission;
                $kode_universal = $id_karyawan;
                $jumlah         = 1;
        ...
        else //Add New
        {
            ...
            //Add Data
            $id = $this->Master_karyawan_model->insert($data);

            if(is_numeric($id))
            {
                $keterangan     = "SUKSES, tambah data Master_karyawan  atas Nama : ".$nama_karyawan;
                $status         = 1;
                $nm_hak_akses   = $this->addPermission;
                $kode_universal = 'NewData';
                $jumlah         = 1;
            ...
            {
                $keterangan     = "GAGAL, tambah data Master_karyawan  atas Nama : ".$nama_karyawan;
                $status         = 0;
                $nm_hak_akses   = $this->addPermission;
                $kode_universal = 'NewData';
                $jumlah         = 1;
    ...
    function hapus_karyawan()
    {
        ...
        if($id!=''){
            ...
            $result = $this->Master_karyawan_model->delete($id);

            $keterangan     = "SUKSES, Delete data Master_karyawan ".$id;
            $status         = 1;
            $nm_hak_akses   = $this->addPermission;
            $kode_universal = $id;
            $jumlah         = 1;
        ...
        {
            $result = 0;
            $keterangan     = "GAGAL, Delete data Master_karyawan ".$id;
            $status         = 0;
            $nm_hak_akses   = $this->addPermission;
            $kode_universal = $id;
            $jumlah         = 1;
    ...
    function print_request($id){
        ...
        $kary_data      =  $this->Master_karyawan_model->find_data('karyawan',$id_karyawan,'id_karyawan');
    ...
    function rekap_pdf(){
        ...
        $data_kar = $this->Master_karyawan_model->rekap_data()->result_array();
    ...
    function downloadExcel()
    {
       ...
       //// $objPHPExcel->getActiveSheet()->getColumnDimension('R')->setWidth(17);
        ...
        $objPHPExcel->getProperties()->setCreator("Yunaz Fandy")
            ...
            ->setTitle("Export Rekap Data Master_karyawan")
            ->setSubject("Export Rekap Data Master_karyawan")
            ->setDescription("Rekap Data Produk for Office 2007 XLSX, generated by PHPExcel.")
            ->setKeywords("office 2007 openxml php")
            ->setCategory("PHPExcel");
        $objPHPExcel->getActiveSheet()->setTitle('Rekap Data Master_karyawan');
        ob_end_clean();
        ...
        header('Content-Disposition: attachment;filename="ExportRekapMaster_karyawan'. date('Ymd') .'.xls"');
