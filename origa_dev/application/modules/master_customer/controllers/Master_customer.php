...
class Master_customer extends Admin_Controller
{
    ...
    public function queryDataJSON($activation, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
    {
        ...
        $sql = "
  			...
  			FROM
                  master_customer a
                  LEFT JOIN child_customer_category b ON a.id_category_customer = b.id_category_customer
  			WHERE 1=1
          " . $where_activation . "
  				AND (
    ...
    public function modal_Process($page = "", $action = "", $id = "")
    {
        ...
        if ($page == 'Customer') {
            if ($action == 'edit' || $action == 'add') {
                ...
                $this->template->render('modal_View_Customer');
            }
        } elseif ($page == 'CustomerCategory') {
    ...
    public function saveCustomer()
    {
        ...
        for ($i = 0; $i < count($data['name_branch']); $i++) {
            ...
            $ArrBranch[$i]['id_branch'] = $data['id_customer'] . '-B' . str_pad($i + 1, 2, "0", STR_PAD_LEFT);
            $ArrBranch[$i]['id_customer'] = $data['id_customer'];
        ...
        if (!empty($data['name_branch'])) {
            $this->db->insert_batch('child_customer_branch', $ArrBranch);
        }
    ...
    public function deleteData()
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $keterangan = 'FAILED, Delete Customer Data ';
            $status = 0;
            $nm_hak_akses = $this->addPermission;
            $kode_universal = $this->auth->user_id();
            $jumlah = 1;
            ...
            $keterangan = 'SUCCESS, Delete Customer Data ';
            $status = 1;
            $nm_hak_akses = $this->addPermission;
            $kode_universal = $this->auth->user_id();
            $jumlah = 1;
