...
class Qc extends CI_Controller
{
	...
	public function query_data_qc($status, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		...
		if ($status == 'cutting') {
			...
			$where = " AND a.sts_cutting='Y' AND (c.id_category='pipe' OR d.id_deadstok IS NOT NULL) ";
			// $group_by = "";
		}
		if ($status == 'fitting') {
	...
	public function server_side_spool()
	{
		...
		foreach ($query->result_array() as $row) {
			...
			$ArrNo_SPK      = [];
			$ArrNo_Drawing  = [];
			$get_split_ipp  = $this->db->select('id_produksi, id_milik, kode_spool, product_code, no_drawing, no_spk')->get_where('spool_group_all',array('spool_induk'=>$row['spool_induk']))->result_array();
			$get_split_ipp2  = $this->db->select('kode_spool, product_code, id_category as product, COUNT(id) AS qty, sts, id_milik, no_spk')->group_by('sts,kode_spool,id_milik')->get_where('spool_group_all',array('spool_induk'=>$row['spool_induk']))->result_array();
	...
	public function query_data_qc_progress($no_ipp, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		...
		$sql .= " ORDER BY z.no_ipp DESC, " . $columns_order_by[$column_order] . " " . $column_dir . " ";
		$sql .= " LIMIT " . $limit_start . " ," . $limit_length . " ";
