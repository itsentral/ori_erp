...
class Finish_good extends CI_Controller
{
	...
	public function server_side_qc()
	{
		...
		foreach ($query->result_array() as $row) {
			...
			$nestedData[]	= "<div align='left'>" . spec_bq2($row['id_milik']) . "-" . $row['length'] . "</div>";
	...
	public function query_data_qc($status, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		...
		if ($status == 'cutting') {
			$where = " AND d.id IS NOT NULL AND (c.id_category='pipe' OR d.id_deadstok IS NOT NULL) AND d.app = 'Y' AND (f.lock_delivery_date IS NULL AND f.spool_induk IS NULL) ";
